...
class NVLinkTransport : public Transport {
 public:
  ...
  std::future<Status> send(Segment::Span src, RequestOptions options = {})
      override;
 ...
 private:
  ...
  Result<const NVLinkRemoteRegistrationHandle*> findRemoteHandle(
      RemoteRegisteredSegment::Span& span) const;

  int deviceId_{-1};
  int peerDeviceId_{-1};
  std::string deviceName_;
  TransportState state_{TransportState::Uninitialized};
