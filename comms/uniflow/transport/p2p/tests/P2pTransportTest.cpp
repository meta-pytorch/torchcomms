...
// neutral event-completion seam (createEvent/recordEvent/queryEvent/
// destroyEvent, which delegate to the vendor-typed event mocks).
  ...
  EXPECT_CALL(
      *mock_,
      memcpyDeviceToDeviceAsync(
          remoteBuf_, localBuf_, sizeof(localBuf_), nullptr))
      .WillOnce(Return(Ok()));
...
TEST_F(P2pTransportPutGetTest, GetCompletesViaEvent) {
  ...
  EXPECT_CALL(
      *mock_,
      memcpyDeviceToDeviceAsync(
          localBuf_, remoteBuf_, sizeof(localBuf_), nullptr))
      .WillOnce(Return(Ok()));
  EXPECT_CALL(*mock_, eventCreate(_))
      .WillOnce(DoAll(SetArgPointee<0>(fakeEvent), Return(Ok())));
  EXPECT_CALL(*mock_, eventRecord(fakeEvent, _)).WillOnce(Return(Ok()));
...
TEST_F(P2pTransportPutGetTest, PutMemcpyErrorDrainsAndFails) {
  ...
  EXPECT_CALL(*mock_, synchronizeStream(nullptr)).WillOnce(Return(Ok()));
