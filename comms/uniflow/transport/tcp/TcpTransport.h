...
class TcpSegmentRegistry {
 public:
  ...
  /// making progress. That is deadlock-free only because the lease holder (the
  /// reader thread) never blocks on an application thread -- its Ack enqueue is
  /// non-blocking and its reply path is exempt from the out-queue cap. A
  /// blocking reader would turn this wait into a hang.
