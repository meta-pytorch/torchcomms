...
TransportInfo RdmaTransport::bind() {
  ...
  info.header.numQps = static_cast<uint8_t>(numQps);
  info.header.numNics = static_cast<uint8_t>(numNics);
  info.header.domainId = domainId_;
  info.qpInfos.reserve(numQps);
  info.nicInfos.reserve(numNics);
  ...
  for (uint32_t i = 0; i < numQps; ++i) {
    ...
    psns_.push_back(psn);
    info.qpInfos.push_back({.qpNum = qp->qp_num, .psn = psn});
  }

  for (const auto& nic : nics_) {
    info.nicInfos.push_back(
        {.lid = nic.lid,
...
std::future<Status> RdmaTransport::put(
    std::span<TransferRequest> requests,
    RequestOptions options) {
  return rdmaTransfer(requests, IBV_WR_RDMA_WRITE, std::move(options));
}

std::future<Status> RdmaTransport::get(
    std::span<TransferRequest> requests,
    RequestOptions options) {
  return rdmaTransfer(requests, IBV_WR_RDMA_READ, std::move(options));
}
...
void RdmaTransport::shutdown() {
  if (state_ == TransportState::Disconnected) {
    UNIFLOW_LOG_INFO("shutdown: already disconnected");
    return;
  }
