...
Status CudaApi::memcpyDeviceToDeviceAsync(
    void* dst,
    const void* src,
    size_t count,
    void* stream) {
...
Status CudaApi::synchronizeStream(void* stream) {
  return streamSynchronize(static_cast<cudaStream_t>(stream));
}
...
Result<void*> CudaApi::createEvent() {
  cudaEvent_t event = nullptr;
  auto st = eventCreate(&event);
  if (st.hasError()) {
...
Status CudaApi::recordEvent(void* event, void* stream) {
  return eventRecord(
      static_cast<cudaEvent_t>(event), static_cast<cudaStream_t>(stream));
}

Result<bool> CudaApi::queryEvent(void* event) {
  return eventQuery(static_cast<cudaEvent_t>(event));
}

Status CudaApi::destroyEvent(void* event) {
  return eventDestroy(static_cast<cudaEvent_t>(event));
}
