...
// CUDA implementation of the cross-platform `discoverTopology()` entry
// point. Selected at build time via Buck `select()` in
// `comms/uniflow/drivers:topology-discovery`. Mirrors the per-platform
// pattern used by DeviceAdapter (see `CudaDeviceAdapter.cpp`).
Status discoverTopology(
  ...
  if (nvmlApi == nullptr) {
    nvmlApi = std::make_shared<NvmlApi>();
  }
  if (ibvApi == nullptr) {
    ibvApi = std::make_shared<IbvApi>();
  }
  if (sysfsApi == nullptr) {
    sysfsApi = std::make_shared<SysfsApi>();
  }

  topology.clear();

  DiscoveryData data;
  CHECK_EXPR(discoverHardware(*cudaApi, *nvmlApi, *ibvApi, *sysfsApi, data));
  buildNodes(topology, data);
  buildP2pMatrix(topology, *cudaApi);
  buildEdges(topology, *sysfsApi, data);
