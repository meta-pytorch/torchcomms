...
struct DmaBuff {
  ...
  uint64_t iova{0};
  /// Original buffer address passed to `exportDmaBuff`.
  void* addr{nullptr};
  /// Pool start address. Only populated by pool-backed backends.
  uint64_t poolStart{0};
  /// Pool total size in bytes. Only populated by pool-backed backends.
  size_t poolTotal{0};
};
...
class DeviceAdapter {
 public:
  ...
  /// Translate a client-supplied device pointer (which may be an
  /// opaque allocation handle such as the MTIA `mtiaAllocationPtr` UID)
  /// to the bare device address the NIC needs to subtract `registrationBase`
  /// from. Default: identity cast — backends that have no separate UID
  /// representation (CPU, CUDA) do not need to override this. MTIA's UID
  /// encodes `device_ordinal` in its top 5 bits, so device 1 buffers
  /// need explicit resolution before any wire-address arithmetic.
  virtual Result<uint64_t> resolveLocalAddr(int /*deviceId*/, void* ptr) {
