...
Under hipify, platform selection is resolved at compile time, so there are no
runtime device-adapter platform-selection unit tests; the device adapter is
covered by `drivers/cuda/tests:cuda_device_adapter_test`.
...
### 6. D107346920 - Peer-to-Peer Transfer Integration Test
**Purpose:** Validate intra-node GPU-to-GPU peer-to-peer transfers over the GPU
interconnect (XGMI on AMD, NVLink on NVIDIA)

**File:** `fbcode/comms/uniflow/transport/nvlink/tests/integration/PeerToPeerTransferTest.cpp`
...
# Peer-to-peer transfer tests (requires 2 GPUs)
buck test @fbcode//mode/opt-amd-gpu -m rocm70 fbcode//comms/uniflow/transport/nvlink/tests/integration:peer_to_peer_transfer_test
```
...
2. **PeerToPeerTransferTest** - Comprehensive P2P transfer validation (builds successfully)
