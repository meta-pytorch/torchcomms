...
static inline int IbCastRecvCommGetNqps(struct ncclIbRecvComm* comm) {
  ...
  // BY_ORDER bypasses the scheduler, so arm the same static QP set as the sender.
  if (comm->base.recvMatchingScheme == BY_ORDER) return IbCastCommBaseGetDefaultNqpsPerRequest(&comm->base);
