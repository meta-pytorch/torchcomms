...
#ifdef ENABLE_ROCSHMEM
RCCL_PARAM_DECLARE(DdaEnable);  // defined in collectives.cc
...
// librocm-core has no Buck target in fbcode, so the `-lrocm-core` upstream's
// src/CMakeLists.txt adds for this symbol is unavailable. Its implementation
// returns the same ROCM_VERSION_* macros this TU already has from
// <rocm-core/rocm_version.h>, included via hip_rocm_version_info.h.
// TODO: add rocm-core to the ROCm tp2 catalog and take a real dep instead.
extern "C" VerErrors getROCmVersion(unsigned int* Major, unsigned int* Minor, unsigned int* Patch) {
...
static ncclResult_t initTransportsRank(struct ncclComm* comm, struct ncclComm* parent, uint64_t timers[TIMERS_INIT_COUNT]) {
  ...
  if (comm->runtimeConn) {
    ...
    if (comm->maxLocalRanks == 1 && ncclParamPatPreconnect() && (ncclParamPatEnable() || comm->forcePatEnable))
...
static ncclResult_t ncclCommInitRankFunc(struct ncclAsyncJob* job_) {
  ...
  // Get device properties to retrieve shared memory info
  // Using hipGetDeviceProperties is more reliable than cudaDeviceGetAttribute
  // for cudaDevAttrMaxSharedMemoryPerBlockOptin on some ROCm versions
  CUDACHECKGOTO(hipGetDeviceProperties(&devProp, cudaDev), res, fail);
  maxSharedMem = devProp.sharedMemPerBlockOptin;
  // Fallback to sharedMemPerBlock if sharedMemPerBlockOptin is 0
  if (maxSharedMem == 0) {
    maxSharedMem = devProp.sharedMemPerBlock;
  }
  ...
  if (rcclParamDdaEnable() && !job->parent && !job->isGrow && comm->nNodes == 1 && comm->nRanks == 8) {
