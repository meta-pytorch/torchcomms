...
namespace {
  ...
  template<typename T, typename RedOp, typename Proto, int RCCLMetadata, int USE_ACC, int COLL_UNROLL, int Pipeline, int UserRegMode = 0>
...
#define rcclAllReduceRunRingSimpleProtoImpl(tid, nthreads, work) \
  if(work->rcclUseOneSlice){ \
    using Proto = ProtoSimple<ALLREDUCE_CHUNKSTEPS/ALLREDUCE_SLICESTEPS_SINGLE_NODE, ALLREDUCE_SLICESTEPS_SINGLE_NODE, USE_ACC, COLL_UNROLL>; \
    runRing<T, RedOp, Proto, RCCL_METADATA_EMPTY, USE_ACC, COLL_UNROLL, Pipeline>(tid, nthreads, work); \
  } \
  else{ \
    using Proto = ProtoSimple<ALLREDUCE_CHUNKSTEPS/ALLREDUCE_SLICESTEPS, ALLREDUCE_SLICESTEPS, USE_ACC, COLL_UNROLL>; \
    runRing<T, RedOp, Proto, RCCL_METADATA_EMPTY, USE_ACC, COLL_UNROLL, Pipeline>(tid, nthreads, work); \
  }
...
struct RunWorkColl<ncclFuncAllReduce, T, RedOp, NCCL_ALGO_RING, NCCL_PROTO_SIMPLE, USE_ACC, COLL_UNROLL, Pipeline, UserRegMode> {
...
template<typename T, typename RedOp, int USE_ACC, int COLL_UNROLL, int Pipeline, int UserRegMode>
struct RunWorkColl<ncclFuncAllReduce, T, RedOp, NCCL_ALGO_TREE, NCCL_PROTO_SIMPLE, USE_ACC, COLL_UNROLL, Pipeline, UserRegMode> {
  __device__ __forceinline__ void run(int tid, int nthreads, struct ncclDevWorkColl* work) {
    using Proto = ProtoSimple<1, 1, USE_ACC, COLL_UNROLL>;
    if (work->acc != nullptr) {
      runTreeSplit<T, RedOp, Proto, USE_ACC, COLL_UNROLL, Pipeline>(tid, nthreads, work);
    } else {
      runTreeUpDown<T, RedOp, Proto, USE_ACC, COLL_UNROLL, Pipeline>(tid, nthreads, work);
    }
    // Check-here
    // #if CUDART_VERSION >= 11020 && CUDART_VERSION < 11040 && __CUDA_ARCH__ >= 800
    //   runTreeUpDown<T, RedOp, ProtoSimple<1, 1, USE_ACC, COLL_UNROLL>, USE_ACC, COLL_UNROLL>(tid, nthreads, work);
    // #else
    //   runTreeSplit<T, RedOp, ProtoSimple<1, 1, USE_ACC, COLL_UNROLL>, USE_ACC, COLL_UNROLL>(tid, nthreads, work);
    // #endif
  }
};

template<typename T, typename RedOp, int USE_ACC, int COLL_UNROLL, int Pipeline, int UserRegMode>
struct RunWorkColl<ncclFuncAllReduce, T, RedOp, NCCL_ALGO_COLLNET_DIRECT, NCCL_PROTO_SIMPLE, USE_ACC, COLL_UNROLL, Pipeline, UserRegMode> {
  __device__ __forceinline__ void run(int tid, int/*nthreads*/, struct ncclDevWorkColl* work) {
...
template<typename T, typename RedOp, int USE_ACC, int COLL_UNROLL, int Pipeline, int UserRegMode>
struct RunWorkColl<ncclFuncAllReduce, T, RedOp, NCCL_ALGO_NVLS, NCCL_PROTO_SIMPLE, USE_ACC, COLL_UNROLL, Pipeline, UserRegMode> {
  __device__ __forceinline__ void run(int tid, int/*nthreads*/, struct ncclDevWorkColl* work) {
...
template<typename T, typename RedOp, int USE_ACC, int COLL_UNROLL, int Pipeline, int UserRegMode>
struct RunWorkColl<ncclFuncAllReduce, T, RedOp, NCCL_ALGO_NVLS_TREE, NCCL_PROTO_SIMPLE, USE_ACC, COLL_UNROLL, Pipeline, UserRegMode> {
  __device__ __forceinline__ void run(int tid, int/*nthreads*/, struct ncclDevWorkColl* work) {
    struct ncclNvls* nvls = &ncclShmem.channel.nvls;
...
template<typename T, typename RedOp, int USE_ACC, int COLL_UNROLL, int Pipeline, int UserRegMode>
struct RunWorkColl<ncclFuncAllReduce, T, RedOp, NCCL_ALGO_COLLNET_CHAIN, NCCL_PROTO_SIMPLE, USE_ACC, COLL_UNROLL, Pipeline, UserRegMode> {
  __device__ __forceinline__ void run(int tid, int nthreads, struct ncclDevWorkColl* work) {
    const int bid = ncclShmem.channelId - work->channelLo;
    const int nChannels = work->channelHi - work->channelLo + 1;
...
template<typename T, typename RedOp, int USE_ACC, int COLL_UNROLL, int Pipeline, int UserRegMode>
struct RunWorkColl<ncclFuncAllReduce, T, RedOp, NCCL_ALGO_RING, NCCL_PROTO_LL, USE_ACC, COLL_UNROLL, Pipeline, UserRegMode> {
  __device__ __forceinline__ void run(int tid, int nthreads, struct ncclDevWorkColl* work) {
    runRing<T, RedOp, ProtoLL, RCCL_METADATA_EMPTY, USE_ACC, COLL_UNROLL, 0>(tid, nthreads, work);
  }
};

template<typename T, typename RedOp, int USE_ACC, int COLL_UNROLL, int Pipeline, int UserRegMode>
struct RunWorkColl<ncclFuncAllReduce, T, RedOp, NCCL_ALGO_TREE, NCCL_PROTO_LL, USE_ACC, COLL_UNROLL, Pipeline, UserRegMode> {
  __device__ __forceinline__ void run(int tid, int nthreads, struct ncclDevWorkColl* work) {
    runTreeSplit<T, RedOp, ProtoLL, USE_ACC, COLL_UNROLL, 0>(tid, nthreads, work);
  }
};

template<typename T, typename RedOp, int USE_ACC, int COLL_UNROLL, int Pipeline, int UserRegMode>
struct RunWorkColl<ncclFuncAllReduce, T, RedOp, NCCL_ALGO_RING, NCCL_PROTO_LL128, USE_ACC, COLL_UNROLL, Pipeline, UserRegMode> {
  __device__ __forceinline__ void run(int tid, int nthreads, struct ncclDevWorkColl* work) {
    ...
    runRing<T, RedOp, ProtoLL128, RCCL_METADATA_EMPTY, USE_ACC, COLL_UNROLL, 0, UserRegMode>(tid, nthreads, work);
  }
};

template<typename T, typename RedOp, int USE_ACC, int COLL_UNROLL, int Pipeline, int UserRegMode>
struct RunWorkColl<ncclFuncAllReduce, T, RedOp, NCCL_ALGO_TREE, NCCL_PROTO_LL128, USE_ACC, COLL_UNROLL, Pipeline, UserRegMode> {
  __device__ __forceinline__ void run(int tid, int nthreads, struct ncclDevWorkColl* work) {
    // See the LL128 ring note above; add_unroll.sh forwards UserRegMode.
    runTreeSplit<T, RedOp, ProtoLL128, USE_ACC, COLL_UNROLL, 0, UserRegMode>(tid, nthreads, work);
  }
};
