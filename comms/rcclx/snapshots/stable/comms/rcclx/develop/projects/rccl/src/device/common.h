...
struct RunWorkBatch<ncclFuncSendRecv, T, RedOp, NCCL_ALGO_RING, NCCL_PROTO_SIMPLE, USE_ACC, COLL_UNROLL, Pipeline, UserRegMode>;

template<typename T, typename RedOp, int Proto, int USE_ACC, int COLL_UNROLL, int Pipeline, int UserRegMode>
struct RunWorkBatch<ncclFuncAllGatherV, T, RedOp, NCCL_ALGO_RING, Proto, USE_ACC, COLL_UNROLL, Pipeline, UserRegMode>;
...
#ifdef ENABLE_WARP_SPEED
      if (tid < subtn) {
        if(ncclShmem.warpComm == 0 || Algo != NCCL_ALGO_RING) RunWorkColl<Fn, T, RedOp, Algo, Proto, USE_ACC, COLL_UNROLL, Pipeline, UserRegMode>().run(tid, subtn, work);
        else if (ncclShmem.warpChannelId[tid / WARP_SIZE] >= 0) RunWorkColl<Fn, T, RedOp, Algo, Proto, USE_ACC, COLL_UNROLL, Pipeline, UserRegMode>().run(tid % WARP_SIZE, WARP_SIZE, work);
      }
#else
      // Coverity reports a possible thread divergence due to not all threads participating in the collective.
      // However, the code ensures that the participation is on a per-warp basis.
      // coverity[device_thread_diverged:FALSE]
      if (tid < subtn) RunWorkColl<Fn, T, RedOp, Algo, Proto, USE_ACC, COLL_UNROLL, Pipeline, UserRegMode>().run(tid, subtn, work);
#endif
    }
  }
};
