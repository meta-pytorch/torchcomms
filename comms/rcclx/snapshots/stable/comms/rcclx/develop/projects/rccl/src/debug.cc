...
#endif
// The alias target is a host function, so it is not emitted during the device
// pass; keeping the attribute there fails with "alias must point to a defined
// variable or function". Only relevant when -fgpu-rdc device-compiles this TU
// (i.e. the parallel device linker is off).
