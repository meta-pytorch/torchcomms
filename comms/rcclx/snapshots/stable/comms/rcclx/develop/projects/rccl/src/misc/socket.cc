...
ncclResult_t ncclSocketBindToDevice(struct ncclSocket* sock, const char* devName) {
  ...
  if (!ncclOsSocketIsValid(sock)) {
    WARN("ncclSocketBindToDevice: socket is invalid");
    return ncclInvalidArgument;
  }
  if (devName == NULL) {
...
#if defined(NCCL_OS_LINUX)
  SYSCHECK(setsockopt(sock->socketDescriptor, SOL_SOCKET, SO_BINDTODEVICE, devName, len + 1),
           "setsockopt SO_BINDTODEVICE");
  // Remembered so ncclOsSocketSetFlags() can reapply the bind when ncclOsSocketResetFd() rebuilds the fd on a connect retry.
  memcpy(sock->bindToDevice, devName, len + 1);
  INFO(NCCL_INIT|NCCL_NET, "ncclSocketBindToDevice: bound socket egress to %s", devName);
  return ncclSuccess;
#else
  WARN("ncclSocketBindToDevice: SO_BINDTODEVICE is only supported on Linux");
