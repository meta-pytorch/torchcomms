 ...
 * Modifications Copyright (c) 2019-2023 Advanced Micro Devices, Inc. All rights
 *reserved.
 *
...
struct ncclKernelPlanner {
  ...
  // mean collecting a different `ncclTasks` per graph and one for non-graph.
  ...
  struct WipPlan {
    struct Channel {
      struct {
        ...
        int p2pRounds[NCCL_MAX_DEV_WORK_P2P_PER_BATCH]; // which rounds are
                                                        // present in this
                                                        // batch.
      } wipBatch; // work-in-progress batch which will be next tail of
                  // workBatchQueue
      int nWorkBatchesP2p; // number of p2p batches for this channel.
      struct ncclIntruQueue<struct ncclWorkBatchList, &ncclWorkBatchList::next>
          workBatchQueue;
      struct ncclIntruQueue<struct ncclProxyOp, &ncclProxyOp::enqNext>
          proxyOpQueue;
...
struct ncclComm {
  ...
  int cpuArch; // architecture - As defined in src/include/graph.h, e.g.
               // x86/arm/ppc/mixed
  ...
  * asynchronous NCCL operations. */
  ...
  int reclaimSteps;
  struct ncclIntruQueueMpsc<struct ncclCommCallback, &ncclCommCallback::next>
      callbackQueue;
  ...
  std::unique_ptr<latency_profiler::CollTrace> ctrace_;
  ...
  // [Added-comment] Change the variable name to make it meaningful
  bool useDebugKernels;
#endif
  ...
  std::unique_ptr<meta::comms::AlgoFactory> algoFactory{nullptr};
  ...
  struct ncclLowPrecisionBufferPool lowPrecisionBufferPool;
...
// Cannot use offsetof on non-standard-layout type
...
inline ncclResult_t ncclCommPollCallbacks(
    struct ncclComm* comm,
    bool waitSome) {
  ncclResult_t result = ncclSuccess;
  struct ncclCommCallback* cb =
      ncclIntruQueueMpscDequeueAll(&comm->callbackQueue, waitSome);
  while (cb != nullptr) {
