...
def get_checksums_from_metadata(snapshots_root: Path) -> Dict[str, str]:
    """
    ...
    Returns:
        Dictionary mapping ROCm version to SHA256 checksum
    """
    ...
    for version_dir in stable_dir.iterdir():
        ...
        if checksum:
            ...
            print(f"Found checksum for ROCm {version}: {checksum}")
        else:
            print(f"Warning: No checksum found for ROCm {version}")

    return checksums


def generate_bzl_file(output_path: Path, checksums: Dict[str, str]) -> None:
    """
    Generate a .bzl file with checksum definitions.

    Args:
        output_path: Path where the .bzl file should be created
        checksums: Dictionary mapping ROCm version to SHA256 checksum
    """
    # Create the .bzl file content
    lines = [
        "# AUTO-GENERATED FILE - DO NOT EDIT MANUALLY",
        "# This file is generated by generate_checksums_bzl.py",
        "# It contains SHA256 checksums for rcclx stable artifacts in Manifold",
        "",
    ...
    for version in sorted(checksums.keys()):
    ...
    print(f"\nGenerated {output_path} with {len(checksums)} checksums")
