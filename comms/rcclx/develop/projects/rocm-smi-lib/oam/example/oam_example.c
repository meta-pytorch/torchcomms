...
static int get_sensor_info(uint32_t device_id, oam_sensor_type_t type, uint32_t num_sensors,
                           char unit[]) {
  uint32_t j;
  oam_sensor_info_t* sensor_info = calloc(num_sensors, sizeof(oam_sensor_info_t));
  if (!sensor_info) {
...
int main() {
  ...
  if (!amd_oam_ops.discover_devices(&dev_cnt)) printf("%u AMD devices are discovered\n", dev_cnt);
  if (!dev_cnt) {
  ...
  for (i = 0; i < dev_cnt; i++) {
    ...
    printf("\t**************************************\n");
    if (amd_oam_ops.get_sensors_count(devs_prop[i].device_id, &sensor_count)) continue;
    ...
    if (get_sensor_info(devs_prop[i].device_id, OAM_SENSOR_TYPE_FAN_SPEED, sensor_count.num_fans,
                        "rpm"))
      goto failure;
  }
