...
# spdlog headers: FetchContent provides spdlog_SOURCE_DIR; find_package provides the target.
if(spdlog_SOURCE_DIR)
  target_include_directories(rccl_device_defs SYSTEM INTERFACE "${spdlog_SOURCE_DIR}/include")
elseif(TARGET spdlog::spdlog)
  get_target_property(_spdlog_inc spdlog::spdlog INTERFACE_INCLUDE_DIRECTORIES)
  if(_spdlog_inc)
    foreach(_p ${_spdlog_inc})
      if(NOT _p MATCHES "^\\$<")
        target_include_directories(rccl_device_defs SYSTEM INTERFACE "${_p}")
      endif()
    endforeach()
...
# Conda headers reach these hand-built compiles only through CPATH, which clang
# treats as non-system. jemalloc.h then declares posix_memalign with nothrow
# from a non-system header, which disqualifies the workaround in
# Sema::CheckEquivalentExceptionSpec that lets clang's mm_malloc.h redeclare it
# without throw(). CPATH also lands the directory in
# CMAKE_CXX_IMPLICIT_INCLUDE_DIRECTORIES, so the guards on the -isystem loops
# below drop it; append it directly instead.
set(DL_CONDA_SYS_INC "")
if(CONDA_INCLUDE_DIR)
  set(DL_CONDA_SYS_INC "-isystem${CONDA_INCLUDE_DIR}")
endif()
...
foreach(DL_GPU_TARGET ${DL_GPU_TARGETS})
  ...
  list(APPEND _link_inc_flags ${DL_CONDA_SYS_INC})
  ...
  foreach(ENTRY ${SPECIALIZED_ENTRIES})
    ...
    add_custom_command(
      ...
      COMMAND ${DL_CLANG}
        ...
        ${DL_HIP_COMPILER_FLAGS}
        -gline-tables-only
        -std=c++${CMAKE_CXX_STANDARD} -w ${DL_OPT_FLAGS}
        -emit-llvm -S
        -o ${IR_OUT}
        ${SRC}
      DEPENDS ${SRC}
...
list(APPEND _host_inc_flags ${DL_CONDA_SYS_INC})
...
# Meta device objects
#
# Same treatment as the dda_* objects above, driven by META_DEVICE_SOURCES
# (set in src/CMakeLists.txt, which also removes these from HIP_SOURCES).
# Compiled monolithically -- no --offload-host-only -- so each TU carries its
# own fatbin rather than leaving an undefined __hip_fatbin_* for the linker.
#
# Object names are path-derived, not NAME_WE: Impl.cu.cpp occurs three times
# under comms/ctran/algos and Get/PutSignal are not unique either.
# ===========================================================================
set(META_FAT_OBJS "")
foreach(_meta_rel IN LISTS META_DEVICE_SOURCES)
  set(_meta_src "${HIPIFY_DIR}/${_meta_rel}")
  string(REGEX REPLACE "\\.cu\\.cpp$" "" _meta_stem "${_meta_rel}")
  string(REPLACE "/" "_" _meta_stem "${_meta_stem}")
  set(_meta_obj "${DEVICE_BUILD_DIR}/meta_${_meta_stem}.o")
  add_custom_command(
    OUTPUT  ${_meta_obj}
    COMMAND ${DL_CLANG}
      -x hip ${DL_OFFLOAD_ARCH_FLAGS}
      ...
      ${_meta_src}
    DEPENDS ${_meta_src}
    COMMENT "DL compile: ${_meta_rel} (Meta device kernels)"
    VERBATIM
  )
  list(APPEND META_FAT_OBJS ${_meta_obj})
endforeach()

# ===========================================================================
# ctran kernel instantiation objects
#
# CTRAN_GEN_HIP_FILES is populated by the genctran block in src/CMakeLists.txt,
# which routes the generated TUs here rather than into the host-only rccl
# target. Basenames are unique within the generated directory, so unlike the
# META_DEVICE_SOURCES loop above these need no path-derived naming.
# ===========================================================================
set(CTRAN_GEN_FAT_OBJS "")
foreach(_ctran_src IN LISTS CTRAN_GEN_HIP_FILES)
  get_filename_component(_ctran_name "${_ctran_src}" NAME)
  string(REGEX REPLACE "\\.cu\\.cpp$" "" _ctran_name "${_ctran_name}")
  set(_ctran_obj "${DEVICE_BUILD_DIR}/ctran_${_ctran_name}.o")
  add_custom_command(
    OUTPUT  ${_ctran_obj}
    COMMAND ${DL_CLANG}
      -x hip ${DL_OFFLOAD_ARCH_FLAGS}
      ...
      ${_ctran_src}
    DEPENDS ${_ctran_src}
    COMMENT "DL compile: ctran ${_ctran_name} (kernel instantiations)"
    VERBATIM
  )
  list(APPEND CTRAN_GEN_FAT_OBJS ${_ctran_obj})
endforeach()
list(LENGTH CTRAN_GEN_FAT_OBJS _ctran_obj_count)
message(STATUS "Device Linker: ${_ctran_obj_count} ctran instantiation objects")

# ===========================================================================
# Top-level target
# ===========================================================================
add_custom_target(device_linker_build ALL
  DEPENDS ${COMMON_FAT_OBJ} ${ONERANK_FAT_OBJ} ${COLLECTIVES_FAT_OBJ} ${DDA_ALL_REDUCE_IPC_FAT_OBJ} ${DDA_REDUCE_SCATTER_IPC_FAT_OBJ} ${DDA_ALL_GATHER_IPC_FAT_OBJ} ${DDA_ALLTOALL_IPC_FAT_OBJ} ${SYM_FAT_OBJS} ${META_FAT_OBJS} ${CTRAN_GEN_FAT_OBJS}
)
