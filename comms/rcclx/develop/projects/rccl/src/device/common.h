...
#ifdef ENABLE_WARP_SPEED
      if (tid < subtn) {
        if(ncclShmem.warpComm == 0 || Algo != NCCL_ALGO_RING) RunWorkColl<Fn, T, RedOp, Algo, Proto, USE_ACC, COLL_UNROLL, Pipeline>().run(tid, subtn, work);
        else if (ncclShmem.warpChannelId[tid / WARP_SIZE] >= 0) RunWorkColl<Fn, T, RedOp, Algo, Proto, USE_ACC, COLL_UNROLL, Pipeline>().run(tid % WARP_SIZE, WARP_SIZE, work);
      }
#else
