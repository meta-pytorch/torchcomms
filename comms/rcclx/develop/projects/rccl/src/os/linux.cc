...
ncclResult_t ncclOsSocketSetFlags(struct ncclSocket* sock) {
  ...
  // Reapply the egress bind: ncclOsSocketResetFd() makes a fresh fd on every connect retry,
  // and an unbound retry would silently fall back to the default route.
  if (sock->bindToDevice[0] != '\0') {
    SYSCHECKGOTO(setsockopt(sock->socketDescriptor, SOL_SOCKET, SO_BINDTODEVICE, sock->bindToDevice,
                           strlen(sock->bindToDevice) + 1), "setsockopt SO_BINDTODEVICE", ret, fail);
    INFO(NCCL_NET, "ncclOsSocketSetFlags: reapplied egress bind to %s after fd reset", sock->bindToDevice);
  }
