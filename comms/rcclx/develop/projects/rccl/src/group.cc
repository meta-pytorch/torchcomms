...
ncclResult_t ncclP2PPreconnectFunc(struct ncclAsyncJob* job_) {
  ...
  NcclScubaEvent p2pPreConnectEvent(&commLogData);
  p2pPreConnectEvent.lapAndRecord(
      "p2pPreconnectFunc START");
...
static ncclResult_t ncclCollPreconnect(struct ncclComm* comm, bool* algoNeedConnect) {
  ...
  CommLogData commLogData = {
      0, comm->commHash, "", comm->rank, comm->nRanks};
  NcclScubaEvent collPreConnectEvent(&commLogData);
#endif
  for (int i = 0; i < NCCL_NUM_ALGORITHMS; ++i) {
    if (algoNeedConnect[i]) {
      ...
      collPreConnectEvent.lapAndRecord(
          collpreStage + " START");
#endif
      switch (i) {
        case NCCL_ALGO_RING: {
