...
def test_single_node(paths, inspector_helpers):
    ...
    dump_dir = os.path.join(paths.INSPECTOR_DUMP_DIR, "allreduce_inspector_dumps", "single_node")
    ...
    log_dir = os.path.join(paths.LOGDIR, "allreduce_ext_inspector_test_logs")
    ...
    assert result.returncode == 0, f"AllReduce inspector test failed, see {log_file}"
    ...
    for dump_file in dump_files:
        ...
        # Verify all records are AllReduce with correct topology
...
def test_single_node_verbose(paths, inspector_helpers):
    ...
    dump_dir = os.path.join(paths.INSPECTOR_DUMP_DIR, "allreduce_inspector_dumps", "single_node_verbose")
    os.makedirs(dump_dir, exist_ok=True)
    ...
    assert result.returncode == 0, f"AllReduce inspector verbose test failed, see {log_file}"
    ...
    for dump_file in dump_files:
        ...
        with open(dump_file, 'r') as f:
            for lineno, line in enumerate(f, 1):
                ...
                assert record["coll_perf"]["coll"] == "AllReduce", \
                    f"Record at line {lineno} in {dump_file} should be AllReduce"
                assert record["header"]["n_ranks"] == 8
                assert record["header"]["nnodes"] == 1
...
def test_multinode(paths, inspector_helpers):
    ...
    dump_dir = os.path.join(paths.INSPECTOR_DUMP_DIR, "allreduce_inspector_dumps", "multinode")
    os.makedirs(dump_dir, exist_ok=True)
    ...
    assert result.returncode == 0, f"Multi-node AllReduce inspector test failed, see {log_file}"
    ...
    for dump_file in dump_files:
        ...
        # Verify all records are AllReduce with correct multi-node topology
        with open(dump_file, 'r') as f:
            for lineno, line in enumerate(f, 1):
                line = line.strip()
                if not line:
...
def test_multinode_verbose(paths, inspector_helpers):
    ...
    dump_dir = os.path.join(paths.INSPECTOR_DUMP_DIR, "allreduce_inspector_dumps", "multinode_verbose")
    os.makedirs(dump_dir, exist_ok=True)
    ...
    assert result.returncode == 0, f"Multi-node AllReduce inspector verbose test failed, see {log_file}"
