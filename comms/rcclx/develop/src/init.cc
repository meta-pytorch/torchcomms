...
ncclResult_t checkHsaEnvSetting() {
  ...
  if (!validHsaScratchEnvSetting(hsaScratchEnv, hipRuntimeVersion, firmwareVersion, devProp.gcnArchName)) {
    WARN("HSA_NO_SCRATCH_RECLAIM=1 must be set. rocm ver:%d", hipRuntimeVersion);
...
// [Added-comment] debug kernels has perf impact, turn it off by default
RCCL_PARAM(UseDebugKernels, "USE_DEBUG_KERNELS", 0);
...
static ncclResult_t commAlloc(struct ncclComm* comm, struct ncclComm* parent, int ndev, int rank) {
  ...
  comm->useDebugKernels = false;
  if ((ncclDebugLevel >= NCCL_LOG_INFO) && rcclParamUseDebugKernels()) {
    comm->useDebugKernels = true;
  }
#endif
  ...
  // Initialize low precision buffer pool for CUDA graph compatibility
  memset(
...
static ncclResult_t initTransportsRank(struct ncclComm* comm, struct ncclComm* parent, uint64_t timers[TIMERS_INIT_COUNT]) {
  ...
  CommLogData commLogData{
        0, comm->commHash, "", comm->rank, comm->nRanks};

  NcclScubaEvent initTransportsRankEvent(&commLogData);
  initTransportsRankEvent.lapAndRecord(
      "InitTransportsRank START");
#endif
  int rank = comm->rank;
  int nranks = comm->nRanks;
  int nNodes = 1;
...
#ifdef BUILD_META_INTERNAL
  initTransportsRankEvent.lapAndRecord(
      "InitTransportsRank Last error: " + std::to_string(ret));
#endif
  return ret;
fail:
  goto exit;
...
static ncclResult_t ncclCommInitRankFunc(struct ncclAsyncJob* job_) {
  ...
  if (job->parent) {
    ...
    INFO(NCCL_INIT, "%s comm %p rank %d nranks %d nnodes %d cudaDev %d nvmlDev %d busId %lx parent %p splitCount %d color %d key %d - Init COMPLETE", job->funcName,
         comm, comm->rank, comm->nRanks, comm->nNodes, comm->cudaDev, comm->nvmlDev, comm->busId, job->parent, job->splitCount, job->color, job->key);
  } else {
    ...
    INFO(NCCL_INIT, "%s comm %p rank %d nranks %d nnodes %d cudaDev %d nvmlDev %d busId %lx commId 0x%llx - Init COMPLETE", job->funcName,
         comm, comm->rank, comm->nRanks, comm->nNodes, comm->cudaDev, comm->nvmlDev, comm->busId, commIdHash);
  }
  sum_timers = 0.0;
...
exit:
  ...
  free(parentRanks);
#ifdef BUILD_META_INTERNAL
  commInitFuncEvent.lapAndRecord("CommInitRankFunc Last error:" + std::to_string(res));
#endif
...
ncclResult_t ncclCommInitRankConfig_impl(ncclComm_t *newcomm, int nranks, ncclUniqueId commId, int myrank, ncclConfig_t *config) {
  ...
  NcclScubaEvent commInitRankConfigEvent(&commLogData);
  commInitRankConfigEvent.lapAndRecord(
      "CommInitRankConfig START");
#endif
  ...
  #ifdef BUILD_META_INTERNAL
    commInitRankConfigEvent.lapAndRecord("CommInitRankConfig COMPLETE");
  #endif
exit:
  ncclGroupErrCheck(ret);
  NCCLCHECK(ncclGroupEndInternal());
  #ifdef BUILD_META_INTERNAL
    commInitRankConfigEvent.lapAndRecord("CommInitRankConfig Last error: " + std::to_string(ret));
  #endif
  if (newcomm && *newcomm) {
...
static ncclResult_t commCleanup(ncclComm_t comm) {
  ...
  // Clean up low precision buffer pool
  if (comm->lowPrecisionBufferPool.initialized) {
    NCCLCHECK(ncclLowPrecisionBufferPoolDestroy(&comm->lowPrecisionBufferPool));
  }
  if (comm->tuner != NULL) {
...
ncclResult_t ncclCommFinalize_impl(ncclComm_t comm) {
...
#ifdef BUILD_META_INTERNAL
  CommLogData commLogData{
      0, comm ? comm->commHash : 0, "", comm ? comm->rank : -1, comm ? comm->nRanks : -1};

  NcclScubaEvent commFinalizeEvent(&commLogData);
  commFinalizeEvent.lapAndRecord(
      "ncclCommFinalize START");
#endif
  ...
  #ifdef BUILD_META_INTERNAL
    commFinalizeEvent.lapAndRecord(
        "ncclCommFinalize COMPLETE");
  #endif
exit:
  #ifdef BUILD_META_INTERNAL
    commFinalizeEvent.lapAndRecord(
        "ncclCommFinalize Last error: " + std::to_string(ret));
  #endif
  ncclGroupErrCheck(ret);
  NCCLCHECK(ncclGroupEndInternal());
  if (comm) {
...
ncclResult_t ncclCommDestroy_impl(ncclComm_t comm) {
  ...
  NcclScubaEvent commDestroyEvent(&commLogData);
  commDestroyEvent.lapAndRecord("Destroy START");
#endif
...
#ifdef BUILD_META_INTERNAL
  commDestroyEvent.lapAndRecord("Destroy COMPLETE");
#endif
exit:
#ifdef BUILD_META_INTERNAL
  commDestroyEvent.lapAndRecord("Destroy Last error: " + std::to_string(res));
#endif
...
ncclResult_t ncclCommAbort_impl(ncclComm_t comm) {
  ...
  NcclScubaEvent commAbortEvent(&commLogData);
  commAbortEvent.lapAndRecord("Abort START");
#endif
  ...
  NCCLCHECKGOTO(ncclAsyncLaunch((struct ncclAsyncJob*)job, commReclaim, NULL, free, comm), res, fail);
#ifdef BUILD_META_INTERNAL
  commAbortEvent.lapAndRecord("Abort COMPLETE");
#endif
exit:
  ncclGroupErrCheck(res);
  NCCLCHECK(ncclGroupEndInternal());
  #ifdef BUILD_META_INTERNAL
    commAbortEvent.lapAndRecord("Abort Last error: " + std::to_string(res));
  #endif
  return ncclSuccess;
fail:
  goto exit;
...
ncclResult_t ncclCommSplit_impl(ncclComm_t comm, int color, int key, ncclComm_t *newcomm, ncclConfig_t *config) {
...
#ifdef BUILD_META_INTERNAL
  CommLogData commLogData{0, 0, "", -1, -1};

  NcclScubaEvent commSplitEvent(&commLogData);
  commSplitEvent.lapAndRecord("CommSplit START");
#endif
  ...
  *newcomm = NCCL_COMM_NULL;
  ...
  #ifdef BUILD_META_INTERNAL
  commSplitEvent.lapAndRecord("CommSplit COMPLETE");
  #endif
exit:
  // for loggin only, not ready for replaying
  ...
  NCCLCHECK(ncclGroupEndInternal());
  #ifdef BUILD_META_INTERNAL
    commSplitEvent.lapAndRecord("CommSplit Last error: " + std::to_string(res));
  #endif
