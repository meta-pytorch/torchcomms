...
NCCL_PARAM(EnableProxyTrace, "ENABLE_PROXY_TRACE", 0);
...
static ncclResult_t ncclProxyOpToArgs(struct ncclProxyOp* op, struct ncclProxyArgs* args, int subIndex) {
  ...
  if (ncclParamEnableProxyTrace()) {
    ...
    META_PROXY_TRACE_INFO_COPY(sub->traceInfo.protocol, op->protocol);
    META_PROXY_TRACE_INFO_COPY(sub->traceInfo.pattern, op->pattern);
    META_PROXY_TRACE_INFO_COPY(sub->traceInfo.totalBytes, op->totalBytes);
    META_PROXY_TRACE_INFO_COPY(sub->traceInfo.chunkSize, op->chunkSize);
  }
...
// [Added-comment] Note, ops are actually uploaded from the uploadProxyOps -> ncclProxySaveOp call chain
