...
std::vector<LifecycleEventRecord> drainAllLifecycleEvents(
  ...
  // Taken before anything else so the requests and every wait come out of one
  // allowance. Reading the clock per feed instead would restart the budget each
  // time, which is the sum this exists to avoid.
  ...
  // Every callable below is registrant code, and the fields are optional: a
  // backend can register with any of them unset, the way one registering
  // without `describeCaptured` does. Calling an empty std::function throws
  // bad_function_call, and a throw out of here would take the drain away from
  // every backend behind the one that raised. Both are treated the same: a feed
  // that cannot answer is a feed with nothing to say.
  ...
  for (const auto& [source, generation] : pending) {
    ...
    // Clamped at zero rather than skipped once the budget is gone: a feed that
    // has already finished flushing answers immediately, and skipping would
    // drop its events for no gain. Only one still waiting is cut off.
    ...
    try {
      if (!source->ops->waitFlush(generation, remaining)) {
        // Not an error on the feed's part, but the same consequence as one: the
        // drain returns without what that feed was about to publish, and
