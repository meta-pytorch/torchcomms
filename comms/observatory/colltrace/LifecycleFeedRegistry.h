...
struct LifecycleFeedOps {
  ...
  // Waits for the generation `requestFlush` returned, giving up after the
  // deadline. Returns whether the flush completed: false means the events it
  // would have carried are not in this drain, not that the feed is broken.
  //
  // The deadline is the caller's, not the feed's, because the drain is
  // sequential: a feed choosing its own would let N feeds sum to N times what
  // any one of them promised.
...
// another, so a per-feed limit would still let a process with several stalled
// backends block for the sum of them; the caller asking for telemetry is
// entitled to a bound on getting back, not on each step. A feed that runs out
// of budget contributes whatever it had already published and is counted in
// `lifecycleFeedFailures()`, so a feed that is always late is visible rather
// than merely slow.
//
// This is on the path a consumer runs at a step boundary and during a hang
// investigation. Waiting forever there means telemetry hangs the trainer at
// exactly the moment someone is trying to find out why it is stuck.
std::vector<LifecycleEventRecord> drainAllLifecycleEvents(
    std::chrono::nanoseconds budget = std::chrono::seconds{1});

// How many times a feed has thrown out of one of its callables, for the life of
// the process.
//
// A feed that cannot answer is skipped so that one backend's bug does not take
// the drain away from the other's. That isolation is deliberate, and on its own
// it is also silent: a feed that throws on every drain contributes nothing and
// says nothing, which is the same subset-with-no-warning this library exists to
// prevent, arriving one feed at a time. Nonzero and growing is the signal.
//
// Counted rather than logged so nothing here needs a logging dependency, and a
// callable a registrant simply left unset is not counted -- that is documented
// as allowed, not a failure.
