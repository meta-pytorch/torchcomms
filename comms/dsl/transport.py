...
def nvl_rendezvous(
    ...
    # The signal pad must hold two int64 regions (tail + head) of
    # world_size * max_blocks_per_peer entries each. The default symm-mem pad
    # (9216 B == 1152 int64) only covers world_size * max_blocks_per_peer <= 576
    # -- i.e. 18 ranks at the default 32 blocks -- so rendezvous would otherwise
    # fail above 18 ranks. Provision the exact capacity this layout needs before
    # allocating; never shrink a larger pad another component may already have set.
    signal_pad_bytes = 2 * world_size * max_blocks_per_peer * torch.int64.itemsize
