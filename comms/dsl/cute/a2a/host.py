...
# vs genai all_to_all_single_non_contig): the STAGED orchestrated variant wins/on-par small+mid
...
def all_to_all_transpose(
    ...
    *,
    ...
    transpose -> tuned zero-copy a2a) for <=32MB/peer (small+mid, where it wins/on-par vs genai:
...
def all_to_all_transpose_fused(
    ...
    *,
    ...
    # 0.33x genai, 32 blocks/peer = 0.99x). UNLIKE the plain a2a's small SM-matched grid, so
    ...
    # num_blocks*ws is a full-SM grid at scale, so vs-genai stays a fair full-device budget.
    # Size ramp: signal/sync overhead scales with block count, so small chunks want few blocks
    # (8xH100 64KB: 4 blocks = 1.88x genai, 32 = 0.54x) while large chunks want a full grid for
