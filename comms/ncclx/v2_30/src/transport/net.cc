...
// Note: Cannot use NCCL_PARAM here because "NCCL_NET_OPTIONAL_RECV_COMPLETION"
// collides with a #define in nccl_net.h. Use the underlying function directly.
static int64_t ncclParamNetOptionalRecvCompletion() {
  static int64_t cache = INT64_MIN;
  static int8_t noCache = -1;
  return ncclLoadParam("NCCL_NET_OPTIONAL_RECV_COMPLETION", 1, INT64_MIN, &cache, &noCache);
}
