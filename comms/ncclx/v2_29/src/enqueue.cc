...
ncclResult_t ncclLaunchPrepare(struct ncclComm* comm) {
  ...
  if (planner->nTasksColl + planner->nTasksP2p + planner->nTasksBcast != 0 ||
    ...
    do {
      ...
      // CollTrace captures the opCount value. Previously opCount was
      // incremented in doLaunches (group.cc) after each kernel launch,
      // but proxy ops are created here in Prepare before any launches,
      // causing colltrace to capture a stale value.
      comm->opCount++;
