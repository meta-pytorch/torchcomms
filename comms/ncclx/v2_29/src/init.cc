...
ncclResult_t ncclCommGrow(ncclComm_t comm, int nRanks, const ncclUniqueId* uniqueId, int rank, ncclComm_t* newcomm, ncclConfig_t* config) {
  ...
  if (config) {
    internalConfigPtr = config;
    NCCLCHECK(ncclxParseCommConfig(internalConfigPtr));
  } else if (isExistingRank) {
    deepCopyCommConfig(&internalConfig, &comm->config);
    internalConfigPtr = &internalConfig;
  } else {
    internalConfig = NCCL_CONFIG_INITIALIZER;
    internalConfigPtr = &internalConfig;
    NCCLCHECK(ncclxParseCommConfig(internalConfigPtr));
  }
  ...
  *newComm->abortFlagRefCount = 1;
  ...
  if (isExistingRank && config == NULL) {
    ...
    NCCLCHECKGOTO(parseCommConfig(newComm, internalConfigPtr), res, fail);
  }
...
ncclResult_t ncclCommGetAsyncError(ncclComm_t comm, ncclResult_t *asyncError) {
  ...
  *asyncError = COMPILER_ATOMIC_LOAD(&comm->asyncResult, std::memory_order_acquire);
  ...
  if (*asyncError == ncclSuccess && comm->sharedRes &&
      comm->sharedRes->ginState.ncclGin && comm->sharedRes->ginState.connected) {
