...
class Primitives<
  ...
  __device__ __forceinline__ void genericOp(
    ...
    if (tid < nworkers && offset < nelem && !isNetOffload) {
      ...
      //   perf_orig = 2*num_slices
      //   perf_new = 2+num_slices
      // So the new code and old code behave the same for num_slices=2, and for
      // num_slices>2 the new code is superior. And note that in the case
      // num_slices=1, the loop is trivially unrollable (single iteration) so we
