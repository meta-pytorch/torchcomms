...
export NCCL_HOME=${NCCL_HOME:-$FBCODE_DIR/comms/ncclx/v2_29}
pushd "${NCCL_HOME}"
...
# Pre-generate nccl_git_version.h (v2_29+) so distcc remote workers can find it
if [ -f src/misc/generate_git_version.py ]; then
  mkdir -p "$BUILDDIR/obj/include"
  python3 src/misc/generate_git_version.py "$BUILDDIR/obj/include/nccl_git_version.h"
fi
