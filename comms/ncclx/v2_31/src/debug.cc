...
static void ncclDebugLogV(ncclDebugLogLevel level, unsigned long flags, const char* file, const char* func, int line,
  ...
  va_list vcopy;
  va_copy(vcopy, vargs);
  const int msgLen = std::vsnprintf(nullptr, 0, fmt, vcopy);
  va_end(vcopy);
  if (msgLen < 0) {
    return;
  }

  std::vector<char> message(static_cast<size_t>(msgLen) + 1); // +1 for null terminator
  va_copy(vcopy, vargs);
  std::vsnprintf(message.data(), message.size(), fmt, vcopy);
  va_end(vcopy);

  ncclx::logging::writeNcclLog(level, file ? file : "", func ? func : "", line,
                               std::string_view(message.data(), static_cast<size_t>(msgLen)));
}
