...
def main():
    ...
    # Timing
    parser.add_argument(
        "-n", "--iters", type=int, default=20, help="Timed iterations (default: 20)"
    )
    parser.add_argument(
        "-w",
        "--warmup_iters",
    ...
    # GPU/Process
    parser.add_argument(
        "-g", "--ngpus", type=int, default=1, help="GPUs per process (default: 1)"
    )
    parser.add_argument(
        "-t", "--nthreads", type=int, default=1, help="Threads per process (default: 1)"
    )
    ...
    parser.add_argument(
        "-o", "--op", default="sum", help="Reduction operation (default: sum)"
    )

    # ubx-specific
    parser.add_argument(
        "--backend", default="all", help="ubx, nccl, all (default: all)"
    )
    parser.add_argument(
        "--kernel",
        default="auto",
        choices=["mc", "uc", "lamport", "lamport_push", "push", "rw", "rr", "auto"],
    ...
    parser.add_argument(
        "--topk", type=int, default=2, help="Top-k routing for a2av_mxfp8 (default: 2)"
    )
    parser.add_argument(
        "--routing-alpha",
        type=float,
    ...
    # Output
    parser.add_argument(
        "-J", "--output_file", default=None, help="JSON output (if .json extension)"
    )
