...
def dequantize_from_symm_tensor(
    output, allocator, blocks_per_token: int
) -> torch.Tensor:
    ...
    scale_bytes = pool[scale_offset : scale_offset + nslots * blocks_per_token].view(
        nslots, blocks_per_token
    )
    ...
    for s in range(nslots):
        for b in range(blocks_per_token):
            ...
            result[s, b * 32 : (b + 1) * 32] = (
                raw.view(torch.float8_e4m3fn).float() * scale
            )
    return result
...
def run_manual_test(async_dispatch=False):
    ...
    tokens_bf16 = torch.full(
        (ntokens, hidden), fill_val, dtype=torch.bfloat16, device=device
    )

    token_offsets = torch.full(
        (ntokens, total_experts), -1, dtype=torch.int32, device=device
    )
    ...
    print(
        f"[rank{rank}] manual test: sending token fill={fill_val} ({sync_mode})",
        flush=True,
    )
    ...
    print(
        f"[rank{rank}] received slot 0, first 8 dequant values: "
        f"{dequant[0, :8].cpu().tolist()}",
        flush=True,
    )
    print(
        f"[rank{rank}] e8m0 scale byte = {scale_byte}, "
        f"scale_f = {2.0 ** (scale_byte - 127):.6g}",
        flush=True,
    )

    try:
        torch.testing.assert_close(
            dequant, torch.full_like(dequant, fill_val), atol=0.005, rtol=0.01
        )
...
def run_random_test(args):
    ...
    sync_mode = "async+wait" if async_dispatch else "sync"
    print(
        f"[rank{rank}] random test: local_ntokens={local_ntokens} hidden={hidden} "
        ...
        for local_t in range(local_ntokens):
            ...
            e8m0_vals = (
                allocator.internal_pool[
                    scale_offset + blk_start : scale_offset
                    + blk_start
                    + blocks_per_token
                ]
                .float()
    ...
    print(
        f"PASS rank={rank} mode=random local_ntokens={local_ntokens} hidden={hidden}",
        flush=True,
    )
    dist.destroy_process_group()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--hidden", type=int, default=128, help="Token hidden size (multiple of 32)"
    )
