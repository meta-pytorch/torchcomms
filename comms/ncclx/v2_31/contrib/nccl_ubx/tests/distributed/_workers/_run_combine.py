...
def reference_combine(
    global_tokens_bf16: torch.Tensor,
    ...
    out = torch.zeros(
        local_ntokens,
        global_tokens_bf16.shape[1],
        dtype=torch.float32,
        device=global_tokens_bf16.device,
    )
    for local_t in range(local_ntokens):
        ...
        for e in range(routing.shape[1]):
            if routing[global_t, e].item() == 0:
                continue
            w = (
                gate_weights_full[global_t, e].item()
                if gate_weights_full is not None
                else 1.0
            )
...
def run_test(args):
    ...
    routing = make_random_routing(
        global_ntokens, total_experts, topk, device, seed=args.seed
    )
    ...
    if args.mode == "gate" or args.mode == "round_trip":
        ...
        gw_full = torch.full(
            (global_ntokens, total_experts), -1e9, dtype=torch.float32, device=device
        )
        for t in range(global_ntokens):
        ...
        gate_weights = gw_full[
            rank * local_ntokens : (rank + 1) * local_ntokens
        ].contiguous()
    ...
    pool_bytes = max(
        64 * 1024 * 1024, 4 * (bytes_per_dispatch + bytes_per_combine_temp)
    )
    ...
    if (use_lamport_push or use_push) and args.wire != "bf16":
        print(
            f"FAIL rank={rank}: --kernel {args.kernel} requires --wire bf16", flush=True
        )
        sys.exit(1)
    ...
    # Reference.
    ref = reference_combine(
        all_tokens, routing, gate_weights_full, rank, local_ntokens
    ).cpu()
    ...
    print(
        f"[rank{rank}] {args.mode}/{args.wire}: local_n={local_ntokens} "
    ...
    if not torch.allclose(got, ref, atol=atol, rtol=rtol):
        ...
        print(
            f"FAIL rank={rank} {args.mode}/{args.wire}: "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mode", required=True, choices=["random", "gate", "round_trip"]
    )
