...
def run_manual_test():
    ...
    output = allocator.create_tensor([total_slots, hidden], torch.bfloat16)
    ...
    print(
        f"[rank{rank}] received slot 0, first 8 values: {received[0, :8].tolist()}",
        flush=True,
    )

    try:
        torch.testing.assert_close(
            received, torch.full_like(received, fill_val), atol=0.0, rtol=0.0
        )
...
def run_random_test(args):
    ...
    if getattr(args, "topk", False):
        ...
        print(
            f"[rank{rank}] topk_max={topk_max} "
            f"topk_expert.shape={list(topk_expert.shape)}",
            flush=True,
        )
