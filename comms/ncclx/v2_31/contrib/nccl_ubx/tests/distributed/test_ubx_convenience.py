...
CONV_WORKER = os.path.join(
    os.path.dirname(__file__), "_workers", "_run_ubx_convenience.py"
)
...
class TestConvenienceFunctions:
    ...
    def test_request_and_get_tensor(self):
        ...
        result = run_distributed_test(
            CONV_WORKER, num_procs=2, args=["--mode", "request_and_get_tensor"]
        )
    ...
    def test_allreduce_convenience(self):
        ...
        result = run_distributed_test(
            CONV_WORKER, num_procs=2, args=["--mode", "allreduce_convenience"]
        )
        assert result.returncode == 0, (
            f"stdout: {result.stdout}\nstderr: {result.stderr}"
        )
    ...
    def test_restore_round_trip(self):
        ...
        result = run_distributed_test(
            CONV_WORKER, num_procs=2, args=["--mode", "restore_round_trip"]
        )
        assert result.returncode == 0, (
            f"stdout: {result.stdout}\nstderr: {result.stderr}"
        )
