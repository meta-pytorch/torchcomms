...
def _emit_inst_file_multi(
    out_path, kernel_name, args_list, params_macro, header_include, namespace_path
):
...
def _emit_extern_decls(out_path, combos, header_include):
    ...
    for _kname, args, params in combos:
        buf.write(
            f"extern template __global__ void scattergather_kernel_p2p<{args}>(\n"
        )
        buf.write(f"    {params}\n")
        buf.write(");\n")
    buf.write("}  // namespace pace\n")
                ...
                for klz in SG_P2P_LOG_Z:
                    ...
                    for nlr, kwp in SG_P2P_PAIRS:
                        ...
                        for kunroll in SG_P2P_KUNROLL:
                            ...
                            all_decls.append(
                                ("scattergather_kernel_p2p", args_cxx, SG_P2P_PARAMS)
                            )
                    fname = (
...
def main():
    ...
    print(f"Generated {len(files)} sg p2p instantiation files under {gen_dir}/sg/")
