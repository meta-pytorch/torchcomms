        ...
        for _t_name, t_cxx in RS_TYPES:
            for k_mul in RS_MUL:
                for k_aligned in RS_ALIGNED:
                    ...
                    all_decls.append(("reduce_scatter", args_cxx, RS_PARAMS))
        fname = f"rs_outmode{out_mode}.cu"
        fpath = os.path.join(kernel_dir, fname)
        _emit_inst_file_multi(
            fpath, "reduce_scatter", args_list, RS_PARAMS, header, ["pace"]
        )
        files.append(fpath)
...
def main():
    ...
    print(f"Generated {len(files)} rs instantiation files under {gen_dir}/rs/")
