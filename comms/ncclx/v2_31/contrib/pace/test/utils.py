...
def bench_kineto(
    ...
    num_kernels_per_period = (
        {name: num_kernels_per_period for name in kernel_names}
        if isinstance(num_kernels_per_period, int)
        else num_kernels_per_period
    )
    # Save chrome traces
    ...
    for name in kernel_names:
        ...
        if match_count < 1 or (match_count != 1 and num_kernels_per_period[name] <= 1):
            print(
                f"Errors of the kernel {name} in the profiling table, {match_count}\n{'\n'.join(prof_lines)}"
            )
    ...
    if max(num_kernels_per_period.values()) > 1:
        ...
        for i, kernel_name in enumerate(kernel_names):
            ...
            if len(durations) % num_kernels_per_period[kernel_name] != 0:
                print(
                    f"Error: {kernel_name} durations len {len(durations)} not divided by {num_kernels_per_period[kernel_name]}",
                    flush=True,
                )
            num_kernel_patterns = len(durations) // num_kernels_per_period[kernel_name]
            kernel_durations[kernel_name] = [
                sum(durations[j :: num_kernels_per_period[kernel_name]])
                / num_kernel_patterns
    ...
    return (
        [kernel_durations[name] for name in kernel_names]
        if is_tuple
        else kernel_durations[kernel_names[0]]
    )
...
def quant_per_token_cast_back(qopt, x_fp8: torch.Tensor, x_scales: torch.Tensor):
    ...
    scale_float = torch.where(
        x_scales_u8 == 0, torch.ones_like(scale_float), scale_float
    )
...
def quantize_bfloat16_to_nvfp4(
    x: torch.Tensor,
    ...
    return (
        x_nvfp4_packed,
        x_scales.to(torch.float8_e4m3fn).view(dtype=torch.int32),
        sf_scales,
    )
...
def pack_8xnvfp4_to_int32(x: torch.Tensor) -> torch.Tensor:
    ...
    NVFP4_TABLE = torch.tensor(
        [0, 0.5, 1, 1.5, 2, 3, 4, 6], dtype=torch.float32, device=x.device
    )
    NVFP4_INTREPL = torch.tensor(
        [0, 1, 2, 3, 4, 5, 6, 7], dtype=torch.int32, device=x.device
    )
    ...
    shift = int32_elm << torch.tensor(
        [28, 24, 20, 16, 12, 8, 4, 0], dtype=torch.int32, device=x.device
    )
...
def int32_to_8floats_lookup(tensor: torch.Tensor, table: torch.Tensor) -> torch.Tensor:
    ...
    assert table.numel() == 16 and table.ndim == 1, (
        "Lookup table must be 1D with length 16"
    )
...
def dequantize_nvfp4_back_to_bfloat16(
    x_nvfp4: torch.Tensor,
    x_scales: torch.Tensor,
    x_sf_scale: torch.Tensor,
    use_ue8m0_for_nvfp4_sf: bool = False,
):
    if x_nvfp4.numel() == 0:
        ratio = int(x_nvfp4.dtype.itemsize * 8 / 4)
        return x_nvfp4.to(torch.bfloat16).view(x_nvfp4.size(0), x_nvfp4.size(1) * ratio)
    NVFP4_TABLE = torch.tensor(
        [0, 0.5, 1, 1.5, 2, 3, 4, 6, 0, -0.5, -1.0, -1.5, -2, -3, -4, -6],
        dtype=torch.float32,
        device=x_nvfp4.device,
    )
