...
from pace.sg import CommConfig, SGComm
from utils import bench, bench_kineto, init_dist
...
def all_to_all_baseline(x, scatter_dim, gather_dim, group=None, **kwargs):
    ...
    if world_size > 1:
        if scatter_dim == 1 and gather_dim == 0:
            ...
            output = output.reshape(s * world_size, h // world_size, d).contiguous()
        elif scatter_dim == 0 and gather_dim == 1:
            s, h, d = x.shape
            x_ = x.reshape(
                world_size, s // world_size, h, d
            ).contiguous()  # [t, s/t, h, d]
            x_ = x_.transpose(1, 2).contiguous()  # [t, h, s/t, d]
            output = torch.empty_like(x_)
            dist.all_to_all_single(output, x_, group=group, **kwargs)
            output = output.reshape(
                h * world_size,
                s // world_size,  # [t*h, s/t, d]
                d,
            ).contiguous()
...
def _run_strided_validator_checks():
    ...
    for S, G, H, D, dtype in [
        (8, 3, 16, 32, torch.float32),
        (8, 3, 16, 16, torch.bfloat16),
    ]:
    ...
    _expect_strided_error(
        v, torch.as_strided(storage, (4, H, D), (H * D - 1, D, 1)), 0, "stride.0."
    )
    # stride(0)*esize not 16-byte aligned rejected
    _expect_strided_error(
        v,
        torch.as_strided(storage, (2, H, D), (H * D + 1, D, 1)),
        0,
    ...
    _expect_strided_error(
        v,
        torch.as_strided(storage, (2, H, D), (2 * H * D, D, 1)),
        0,
        "size.2..element_size",
    )
    # non-3-D rejected
    _expect_strided_error(
        v, torch.zeros((8, 16, 4, 4), dtype=torch.float32)[:, :, :, ::2], 0, "3-D"
    )
...
def test_main(
    ...
    def test_sg(scatter_dim, gather_dim, input_tensor, rand_input, n):
        ...
        if not args.no_check:
            check_func = (
                check_d1_sg if scatter_dim == 1 and gather_dim == 0 else check_d0_sg
            )
            rand_answer = all_to_all_baseline(
                rand_input, scatter_dim, gather_dim, group=group
            )
            ...
            for i in range(extra_rands):
                ...
                rand_o = all_to_all_baseline(
                    rand_i, scatter_dim, gather_dim, group=group
                )
                batch_rand_input.append(rand_i)
                batch_rand_answer.append(rand_o)
            mismatch_i = batch_rand_input[:-1] + [
                torch.rand(
                    (rand_input.size(0), rand_input.size(1), 2 * rand_input.size(2))
                )
            ]
            ...
            assert sg_tensor_out.data_ptr() == sg_out.data_ptr(), (
                f"[rank {rank}]: out= did not write in place"
            )
            assert torch.allclose(sg_out, rand_answer), (
                f"[rank {rank}]: out= random input check failed"
            )
            # async finish
            sg_tensor, event = buffer.scatter_gather(async_finish=True, **kwargs)
            event.current_stream_wait()
            assert torch.allclose(sg_tensor, rand_answer), (
                f"[rank {rank}]: random input check failed"
            )
            ...
            for i in range(extra_rands + 1):
                if not torch.allclose(batch_rand_answer[i], async_outs[i][0]):
                    print(
                        f"[rank {rank}]: {scatter_dim=} {gather_dim=} {i}-th async seq test failed, saved to rand_batch_{rank}_{i}.h5"
                    )
                    torch.save(
                        (batch_rand_answer[i], async_outs[i][0]),
                        f"rand_batch_{rank}_{i}.h5",
                    )
            # batch input
            ...
            for i in range(len(batch_rand_input)):
                if not torch.allclose(outs[i], batch_rand_answer[i]):
                    ...
                    raise Exception(
                        f"[rank {rank}]: batch input check failed, save to {save_file}"
                    )

            # batch input
            outs, event = buffer.scatter_gather(
                batch_rand_input, async_finish=True, **kwargs
            )
            torch.randn((5120, 5120), dtype=torch.bfloat16) @ torch.randn(
                (5120, 5120), dtype=torch.bfloat16
            )
            event.current_stream_wait()
            for i in range(len(batch_rand_input)):
                assert torch.allclose(outs[i], batch_rand_answer[i]), (
                    f"[rank {rank}]: batch input check failed"
                )

            dist.barrier()
            if rank == 0:
                print(
                    f"[rank {rank}]: sg_alltoall_s{scatter_dim}g{gather_dim} check passed"
                )
        ...
        if not args.no_bench:
            ...
            with nvtx.range(f"sg_alltoall_s{scatter_dim}g{gather_dim}"):
                t = bench(
                    lambda: buffer.scatter_gather(input_tensor, **kwargs),
                    post_fn=dist.barrier,
                )[0]
                if rank == 0 or True:
                    print(
                        f"[rank {rank}]: sg_alltoall_s{scatter_dim}g{gather_dim} : bandwidth: {total_bytes / t / 1e9:.2f} GBps"
                    )
                dist.barrier()
            ...
            def kineto_bench():
                if num_sms > 0:
                    ...
                    if num_local_ranks != num_ranks:
                        ...
                        k2p = {
                            ...
                            "Memcpy PtoP (Device -> Device)": (num_ranks - 1)
                            * use_slots,
                        }
                ...
                def tfunc(bundle):
                    if num_local_ranks != num_ranks or num_sms > 0:
                        ...
                        send_t = (
                            (sum(bundle[0]) + sum(bundle[1])) / 2
                        )  # it is not accurate actually, they are on two different streams
                    recv_t = 0
                ...
                with nvtx.range(f"sg_alltoall_s{scatter_dim}g{gather_dim}"):
                    ret = bench_kineto(
                        test_func,
                        kernel_names=tuple(k2p.keys()),
                        barrier_comm_profiling=True,
                        suppress_kineto_output=True,
                        num_kernels_per_period=k2p,
                        trace_path=f"kineto_{rank}_s{scatter_dim}g{gather_dim}.json"
                        if args.save_kineto and seed == 0
                        else None,
                    )
                if all([v is None for v in ret]):
                    ...
                    if rank == 0 or True:
                        print(
                            f"[rank {rank}]: sg_alltoall_s{scatter_dim}g{gather_dim} : total time {total_t * 1e6:.2f} us, bandwidth: {total_bytes / total_t / 1e9:.2f} GBps"
                        )

            if not args.no_compare_baseline:
                with nvtx.range(
                    f"nccl_all_to_all_baseline_s{scatter_dim}g{gather_dim}"
                ):
                    t = bench(
                        lambda: all_to_all_baseline(
                    ...
                    if rank == 0 or True:
                        print(
                            f"[rank {rank}]: nccl_all_to_all_s{scatter_dim}g{gather_dim} within {num_ranks} peers, bandwidth: {total_bytes / t / 1e9:.2f} GBps"
                        )
                    dist.barrier()

                with nvtx.range(f"nccl_all_to_all_only_s{scatter_dim}g{gather_dim}"):
                    t = bench(
                        lambda: all_to_all_only(input_tensor, group=group),
                        post_fn=dist.barrier,
                    )[0]
                    if rank == 0 or True:
                        print(
                            f"[rank {rank}]: nccl_all_to_all_only_s{scatter_dim}g{gather_dim} within {num_ranks} peers, bandwidth: {total_bytes / t / 1e9:.2f} GBps"
                        )
        dist.barrier()
    ...
    if args.strided and not args.no_check:
        ...
        def check_strided(scatter_dim, gather_dim, dtype):
            ...
            storage = (
                torch.arange(S * G * H * D, dtype=dtype, device="cuda").reshape(
                    S, G, H, D
                )
                + rank
            )
            ...
            ref = all_to_all_baseline(
                q.contiguous(), scatter_dim, gather_dim, group=group
            )
            got, _ = buffer.scatter_gather(
                q, scatter_dim=scatter_dim, gather_dim=gather_dim
            )
            assert torch.equal(got, ref), (
                f"[rank {rank}]: strided QKV s{scatter_dim}g{gather_dim} mismatch"
            )
            ...
            if dtype == torch.float32 and scatter_dim == 0:
                ...
                assert torch.equal(got, ref), (
                    f"[rank {rank}]: strided D=24 s0g1 mismatch"
                )
        ...
        if not args.no_check:
            ...
            for scatter_dim, gather_dim in [(0, 1), (1, 0)]:
                ...
                for block_idx in range(num_blocks):
                    ...
                    qkv_list = [q, k, v]
                    answer_list = [
                        all_to_all_baseline(t, scatter_dim, gather_dim, group=group)
                        for t in qkv_list
                    ]
                ...
                for block_idx in range(num_blocks):
                    for qkv_idx in range(3):
                        if not torch.allclose(
                            all_results[block_idx][qkv_idx],
                            all_answers[block_idx][qkv_idx],
                        ):
                            ...
                            print(
                                f"[rank {rank}]: seq_parallel_qkv sync check failed at block {block_idx}, qkv {qkv_idx}, s{scatter_dim}g{gather_dim} "
                            ...
                            torch.save(
                                {
                                    "fused": fused,
                                    "ref": ref,
                                    "input": all_inputs[block_idx][qkv_idx],
                                    "all_fused": all_results[block_idx],
                                    "all_ref": all_answers[block_idx],
                ...
                for block_idx in range(num_blocks):
                    for qkv_idx in range(3):
                        if not torch.allclose(
                            ...
                            rv = ref[idx][:first_n].float()
                            print(
                                f"[rank {rank}]: seq_parallel_qkv async check failed at block {block_idx}, qkv {qkv_idx}, s{scatter_dim}g{gather_dim} "
                                f"MISMATCH: count={mc}/{fused.numel()} shape={tuple(fused.shape)} "
                                f"first_idx={tuple(dim[:first_n].tolist() for dim in idx)} "
                                f"fused_vals={fv.tolist()} ref_vals={rv.tolist()}"
                            )
                ...
                if rank == 0:
                    print(
                        f"[rank {rank}]: seq_parallel_qkv_s{scatter_dim}g{gather_dim} check passed (3 blocks × 3 QKV = 9 tensors)"
                    )


def test_loop(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    rank, num_ranks, group = init_dist(
        local_rank, num_local_ranks, "gloo" if args.no_compare_baseline else "nccl"
    )
    ...
    config = CommConfig(
        slot_unroll=args.unroll,
        nvl_ring_size=args.nvl_ring,
        rdma_ring_size=args.rdma_ring,
        num_sms=args.num_sms,
    )
    ...
    if rank == 0:
        ...
        print(
            f"[rank {rank}]: SG shared memory = {sm / 1e6:.2f} MB (buffer + {4 * 1024 * 1024 / 1e6:.1f} MB signal)",
            flush=True,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--hidden", type=int, default=128, help="Hidden dimension size (default: 128)"
    )
    parser.add_argument(
        "--nhead", type=int, default=8, help="Number of attention heads (default: 8)"
    )
    parser.add_argument(
        "--sep-out-pfx",
        type=str,
    ...
    parser.add_argument(
        "--repeat", type=int, default=1, help="number of benchmark repeats"
    )
    parser.add_argument(
        "--dtype-size",
        type=int,
        default=2,
        choices=[2, 4],
        help="data type size in bytes, 4 : int32, 2: bfloat16. (default: 2)",
    )
    parser.add_argument(
        "--num-sms",
        type=int,
        default=0,
        help="number of SMs to use for SGComm kernels (default: 32)",
    )
    ...
    parser.add_argument(
        "--unroll", type=int, default=32, help="segment length, unrolls of 1024 float4"
    )
    parser.add_argument(
        "--split-intra",
        type=int,
        default=1,
        help="split 1 node into required micro nodes, for simulation use",
    )
    parser.add_argument(
        "--no-compare-baseline",
        action="store_true",
    ...
    parser.add_argument(
        "--save-kineto", action="store_true", help="store kineto trace for profiling"
    )
    parser.add_argument(
        "--replicas",
        type=int,
        default=-1,
        help="batch-size knob: number of extra batch tensors to exercise (-1 = default 5)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="number of local processes for direct launch",
    )
    parser.add_argument(
        "--strided",
        action="store_true",
