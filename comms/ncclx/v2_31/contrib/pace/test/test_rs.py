...
from pace.rs import CommConfig, RSComm
...
def fsdp_rs(
    unsharded_grads: Union[torch.Tensor, List[torch.Tensor]],
    world_size: int,
    ...
    unsharded_grads = (
        [unsharded_grads]
        if isinstance(unsharded_grads, torch.Tensor)
        else unsharded_grads
    )
    ...
    reduce_scatter_output = torch.empty(
        (reduce_scatter_output_numel,), dtype=reduce_dtype, device="cuda"
    )
    ...
    dist.reduce_scatter_tensor(
        reduce_scatter_output, reduce_scatter_input, op=reduce_op, group=group
    )
...
def test_main(
    ...
    def check_func(
        r: torch.Tensor, ans: Union[torch.Tensor, List[torch.Tensor]], in_: torch.Tensor
    ):
        split_r = buffer.get_split_tensors(in_, r)
        split_ans = (
            buffer.get_split_tensors(in_, ans, tight=True)
    ...
    rs_kwargs = {
        "red_op": red_op,
        "extra_mul": args.extra_mul,
        "extra_post_mul": args.extra_post_mul,
    }

    def test_rs(input_tensor, rand_input):
        fix_answer = (
            fsdp_rs(input_tensor, **fsdp_kwargs) * args.extra_mul * args.extra_post_mul
        )
        if not args.no_check:
            rand_answer = (
                fsdp_rs(rand_input, **fsdp_kwargs)
                * args.extra_mul
                * args.extra_post_mul
            )
            # normal
            ...
            check_func(r, fix_answer, input_tensor)
            bf16_r, _ = buffer.reduce_scatter(
                input_tensor, out_dtype=torch.bfloat16, **rs_kwargs
            )
            check_func(bf16_r, fix_answer.to(torch.bfloat16), input_tensor)
            # async finish
            r, event = buffer.reduce_scatter(
                input_tensor, async_finish=True, **rs_kwargs
            )
            ...
            if args.dtype_size == 2:
                ...
                bf16_r, _ = buffer.reduce_scatter(
                    rand_input, out_dtype=torch.bfloat16, **rs_kwargs
                )
            ...
            rs, event = buffer.reduce_scatter(
                batch_input, async_finish=True, **rs_kwargs
            )
            event.current_stream_wait()
            check_func(rs, torch.cat(batch_answer), in_=batch_input[0])
        ...
        if not args.no_bench:
            ...
            with nvtx.range(f"gin_reduce_scatter"):
                t = bench(
                    lambda: buffer.reduce_scatter(
                        input_tensor, out=fix_answer, **rs_kwargs
                    ),
                    post_fn=dist.barrier,
                )[0]
                if rank == 0 or True:
                    print(
                        f"[rank {rank}]: reduce-scatter bandwidth: {total_bytes / t / 1e9:.2f} GBps"
                    )
                dist.barrier()
            ...
            def kineto_bench():
                ...
                def tfunc(bundle):
                    send_t = sum(
                        x if isinstance(x, (int, float)) else sum(x) for x in bundle
                    )
                    recv_t = 0
                ...
                with nvtx.range("gin_reduce_scatter"):
                    ret = bench_kineto(
                        ...
                        trace_path=f"kineto_{rank}.json"
                ...
                if all([v is None for v in ret]):
                    ...
                    if rank == 0 or True:
                        print(
                            f"[rank {rank}]: reduce_scatter : total time {total_t * 1e6:.2f} us, bandwidth: {total_bytes / total_t / 1e9:.2f} GBps"
                        )
                ...
                with nvtx.range(f"nccl_reduce_scatter"):
                    t = bench(
                        lambda: dist.reduce_scatter_tensor(
                            fix_answer, input_tensor, group=group
                        ),
                        post_fn=dist.barrier,
                    )[0]
                    print(
                        f"[rank {rank}]: nccl_reduce_scatter bandwidth: {total_bytes / t / 1e9:.2f} GBps"
                    )
                with nvtx.range(f"nccl_fsdp_rs"):
                    t = bench(
                        lambda: fsdp_rs(input_tensor, **fsdp_kwargs),
                        post_fn=dist.barrier,
                    )[0]
                    print(
                        f"[rank {rank}]: nccl_fsdp_rs bandwidth: {total_bytes / t / 1e9:.2f} GBps"
                    )
...
def test_loop(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    if rank == 0:
        ...
        print(
            f"[rank {rank}]: RS shared memory = {sm / 1e6:.2f} MB (buffer + {4 * 1024 * 1024 / 1e6:.1f} MB signal)",
            flush=True,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dim0", type=int, default=None, help="dim 0, default is num_ranks"
    )
    parser.add_argument(
        "--seq-len",
        type=int,
    ...
    parser.add_argument(
        ...
        default=4,
        choices=[2, 4],
        help="data type size in bytes, 4 : float32, 2: bfloat16. (default: 2)",
    )
    parser.add_argument(
        "--num-sms",
        type=int,
        default=32,
        help="number of SMs to use for PACE kernels (default: 32)",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="extra multiplier for SUM reduce type",
    )
    parser.add_argument(
        "--extra-post-mul",
        type=float,
        default=1.0,
        help="extra post multiplier for SUM reduce type",
    )
    ...
    parser.add_argument(
        "--skip-kineto", action="store_true", help="skip kineto tracing"
    )
    parser.add_argument(
        "--use-wg", action="store_true", help="use warp group for reduce scatter"
    )
