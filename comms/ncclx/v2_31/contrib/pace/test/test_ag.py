...
from pace.ag import AGComm, CommConfig
...
def test_main(
    ...
    def check_func(
        ...
        is_close = [
            all([torch.allclose(kr, ka) for kr, ka in zip(sr, sa)])
            for sr, sa in zip(split_r, split_ans)
        ]
        if not all(is_close):
    ...
    def test_ag(input_tensor, rand_input):
        fix_answer = torch.ones(
            (input_tensor.numel() * num_ranks), dtype=input_tensor.dtype, device="cuda"
        )
        ...
        if not args.no_check:
            dist.all_gather_into_tensor(
                output_tensor=rand_answer, input_tensor=rand_input, group=group
            )
            dist.all_gather_into_tensor(
                output_tensor=fix_answer, input_tensor=input_tensor, group=group
            )
            ...
            if buffer.config.num_sms > 0:
                ...
                r, _ = buffer.all_gather(
                    input_tensor, out_bf16, input_dtype_mapping=imapping
                )
            ...
            list_out = [
                torch.empty(t.numel() * num_ranks, dtype=t.dtype, device="cuda")
                for t in batch_input
            ]
            rs, _ = buffer.all_gather(batch_input, out=list_out)
            assert rs is list_out, (
                f"[rank {rank}]: list out= did not return the caller buffers"
            )
            for i in range(len(batch_input)):
                assert torch.allclose(rs[i], batch_answer[i]), (
                    f"[rank {rank}]: list out= tensor {i} mismatch"
                )
        ...
        if not args.no_bench:
            ...
            with nvtx.range(f"pace_allgather"):
                t = bench(
                    lambda: buffer.all_gather(input_tensor), post_fn=dist.barrier
                )[0]
                if rank == 0 or True:
                    print(
                        f"[rank {rank}]: allgather bandwidth: {total_bytes / t / 1e9:.2f} GBps"
                    )
                dist.barrier()
            ...
            def kineto_bench():
                if args.num_sms > 0:
                    ...
                    if num_local_ranks != num_ranks:
                        ...
                        k2p = {
                            ...
                            "Memcpy PtoP (Device -> Device)": (num_local_ranks - 1)
                            * use_slots,
                        }
                ...
                if all([v is None for v in ret]):
                    print(
                        f"[rank {rank}]: kineto failed, it is normal if you launch program with nsys"
                    )
                else:
                    send_t, recv_t, total_t = tfunc(ret)
                    if rank == 0 or True:
                        print(
                            f"[rank {rank}]: allgather : total time {total_t * 1e6:.2f} us, bandwidth: {total_bytes / total_t / 1e9:.2f} GBps"
                        )
            ...
            if not args.no_compare_baseline:
                with nvtx.range(f"nccl_allgather"):
                    ...
                    print(
                        f"[rank {rank}]: nccl_allgather bandwidth: {total_bytes / t / 1e9:.2f} GBps"
                    )
...
def test_loop(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    if rank == 0:
        ...
        print(
            f"[rank {rank}]: AG shared memory = {sm / 1e6:.2f} MB (buffer + {4 * 1024 * 1024 / 1e6:.1f} MB signal)",
            flush=True,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="num_sms: 0 = zero-SM cord path; >0 = SM-resident ring kernel (default: 0)",
    )
    ...
    parser.add_argument(
        "--use-ring",
        action="store_true",
        help="cord inter-node ring-forward flag (only meaningful when --num-sms 0; num_sms>0 always uses the ring kernel)",
    )
