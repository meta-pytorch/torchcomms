...
def get_extension_gin_cpp():
    ...
    nccl_a_path = None
    if nccl_dir is None:
        nccl_so_path = subprocess.check_output(
            "ldconfig -p | grep libnccl.so", shell=True, text=True
        ).splitlines()
        ...
        if not os.path.exists(nccl_so_path):
            raise Exception(
                f"ldconfig tells libnccl.so is on {nccl_so_path}, but it does not exist"
            )
    else:
    ...
    if major * 100 + minor < 230:
        raise Exception(
            f"requested nccl library (at {nccl_so_path}) version is "
    ...
    if os.environ.get("PACE_KERNEL_DEBUG", None) in [
        "1",
        "true",
        "True",
    ]:  # default off
        nvcc_flags.extend(["-lineinfo", "-Xptxas=-warn-spills"])
    if os.environ.get("PACE_TIMEOUT_DEBUG", "1") not in [
        "0",
        "false",
        "False",
    ]:  # default on
    ...
    ]  # absolute so root-relative includes resolve regardless of ninja cwd
    ...
    extra_link_args.extend(
        ([] if nccl_a_path is None else [f"{nccl_a_path}"])
        + ["-Wl,--no-as-needed", "-lcuda", "-Wl,--as-needed", "-lnuma"]
    )
