...
class AGComm(BaseComm):
    ...
    def get_recommended_config(
        num_local_ranks: int, num_ranks: int, num_sms: int = 0
    ) -> CommConfig:
        """
    ...
    def all_gather(
        ...
        input_dtype_mapping: Optional[Tuple[torch.dtype, torch.dtype]] = None,
        ...
        if storage_type is not None:
            ...
            assert storage_type in [torch.float32], (
                f"storage_type ({storage_type}) must be in [torch.float32]"
            )
            assert comm_type in [torch.float32, torch.bfloat16], (
                f"comm_type ({comm_type}) must be in [torch.float32, torch.bfloat16]"
            )
        ...
        if storage_type is not None:
            for t in tensor:
                assert t.dtype == storage_type, (
                    f"tensor dtype ({t.dtype}) must match mapping requested dtype ({storage_type})"
                )
        ...
        if out is not None:
            if isinstance(out, torch.Tensor):
                ...
                assert not list_input, (
                    "out must be a list when input is a list of tensors"
                )
                output_list = [out]
                single_output = True
            else:
                assert isinstance(out, list), (
                    "out must be a Tensor or a list of tensors"
                )
                output_list = out

            assert len(output_list) == len(tensor), (
                f"output list length ({len(output_list)}) must match input tensor count ({len(tensor)})"
            )
            for i, (inp, o) in enumerate(zip(tensor, output_list)):
                expected_numel = inp.numel() * self.num_ranks
                assert o.numel() == expected_numel, (
                    f"output[{i}] has {o.numel()} elements, expected {expected_numel}"
                )
                if comm_type is not None:
                    assert o.dtype == comm_type, (
                        f"output[{i}] dtype ({o.dtype}) must match mapping requested dtype ({comm_type})"
                    )
                else:
                    assert o.dtype == inp.dtype, (
                        f"output[{i}] dtype ({o.dtype}) must match input dtype ({inp.dtype})"
                    )
        imapping_int = 0
        if storage_type == torch.float32 and comm_type == torch.bfloat16:
            imapping_int = 1
        result, event = self.runtime.all_gather(
            tensor,
            output_list,
    ...
    def get_split_tensors(
        ...
        outs = [
            [
                output[
                    tensor_offsets[i] + j * tensor_numels[i] : tensor_offsets[i]
                    + j * tensor_numels[i]
                    + tensor_numels[i]
                ]
    ...
    def get_gathered_tensors(
        ...
        outs = [
            output[tensor_offsets[i] : tensor_offsets[i + 1]]
            for i in range(len(tensors))
        ]
        return outs
