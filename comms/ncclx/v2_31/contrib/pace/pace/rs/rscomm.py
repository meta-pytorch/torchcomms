...
class RSComm(BaseComm):
    ...
    def get_recommended_config(num_local_ranks: int, num_ranks: int) -> CommConfig:
        ...
        if num_local_ranks == 1:
            ...
            if num_local_ranks != num_ranks:
                ...
                return CommConfig(
                    slot_unroll=128, nvl_ring_size=4, rdma_ring_size=0, num_sms=32
                )
    ...
    def reduce_scatter(
        ...
        red_op: Union[dist.ReduceOp.RedOpType, dist.ReduceOp] = dist.ReduceOp.SUM,
        extra_mul: float = 1.0,
        extra_post_mul: float = 1.0,
        ...
        red_op_type = (
            red_op if isinstance(red_op, dist.ReduceOp.RedOpType) else red_op.op
        )
        assert red_op_type in [
            dist.ReduceOp.SUM,
            dist.ReduceOp.AVG,
            dist.ReduceOp.PREMUL_SUM,
        ], f"RSComm support sum, avg, and premul_sum, but get {red_op}"
        red_op_premul = 1.0
        ...
        red_int = (
            1 if red_op_type in [dist.ReduceOp.SUM, dist.ReduceOp.PREMUL_SUM] else 2
        )
        assert out_dtype in (None, torch.float32, torch.bfloat16), (
            f"RSComm out_dtype must be None, torch.float32, or torch.bfloat16, but got {out_dtype}"
        )
        out_type_int = (
            1 if out_dtype == torch.bfloat16 else 0
        )  # 0 means no cast, just write float
        result, event = self.runtime.reduce_scatter(
            tensor,
            red_int,
            extra_mul,
            extra_post_mul,
        ...
        evt = (
            EventOverlap(event, extra_tensors=(list(tensor), result))
    ...
    def get_call_times(
        ...
        for i, t in enumerate(tensors):
            ...
            ptr_align = (
                16 if t.dtype == torch.float32 else 8
