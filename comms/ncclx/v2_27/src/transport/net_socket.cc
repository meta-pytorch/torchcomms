...
socket_send:
  ...
  NCCLCHECK(ncclCalloc(&comm->inlineData, MAX_REQUESTS * (SOCKET_CTRL_SIZE + NCCL_SOCKET_INLINE)));
  *sendComm = comm;
  return ncclSuccess;
}
...
socket_recv:
  ...
  NCCLCHECK(ncclCalloc(&rComm->inlineData, MAX_REQUESTS * (SOCKET_CTRL_SIZE + NCCL_SOCKET_INLINE)));
  *recvComm = rComm;
...
ncclResult_t ncclNetSocketGetRequest(struct ncclNetSocketComm* comm, int op, void* data, int size, struct ncclNetSocketRequest** req) {
  for (int i=0; i<MAX_REQUESTS; i++) {
    ...
    if (r->used == 0) {
      ...
      r->comm = comm;
      r->nSubs = 0;
      r->inlineData = (uint8_t*)comm->inlineData + i * (SOCKET_CTRL_SIZE + NCCL_SOCKET_INLINE);
      *req = r;
      return ncclSuccess;
    }
  }
...
static int ncclNetSocketInlineSize(int dataSize) { return (dataSize <= NCCL_SOCKET_INLINE) ? dataSize : 0; }
  ...
  *done = 0;
  ...
  if (r->used == 1) { /* try to send/recv size (+ inline data if any) */
    ...
    if (r->comm->nSocks > 0) {
      ...
      int taskSize = std::max(NCCL_SOCKET_MIN_TASKSIZE, DIVUP(r->size - r->offset, r->comm->nSocks));
