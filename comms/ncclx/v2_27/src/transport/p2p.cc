...
struct ncclP2pBuff {
  ...
  size_t size;
  ncclIpcDesc ipcDesc;
  // [Meta]: Used to inform peer if directPtr is not the base address of the buffer
  size_t offset{0};
};
...
// cuMem API support
ncclResult_t ncclP2pAllocateShareableBuffer(size_t size, int refcount, ncclIpcDesc *ipcDesc, void **ptr, ncclComm* comm, const char* callsite) {
  if (ncclCuMemEnable()) {
    ...
    memLogMetaData = comm->logMetaData;
    NCCLCHECK(ncclCuMemAlloc(ptr, &handle, type, size, callsite));
...
// [META]: NCCLX decouples ncclSendMem/ncclRecvMem structure and protocol buffer, allocate sync resources separately
static ncclResult_t ncclxP2pSetupSyncResources(
    struct ncclComm* comm,
    struct ncclPeerInfo* myInfo,
    struct ncclPeerInfo* peerInfo,
    struct p2pConnectInfo* connInfo,
    struct ncclConnector* conn,
    bool isSend,
    int channelId,
    int connIndex) {
  // Only allocates sync resources once
  if (!conn->transportSyncResources) {
    p2pSyncResources* syncResources;
    NCCLCHECK(ncclCalloc(&syncResources, 1));
    conn->transportSyncResources = syncResources;
    void** syncDevMem = nullptr;
    void** syncMemIpc = nullptr;

    if (isSend) {
      syncDevMem = (void**)&syncResources->sendDevMem;
      syncMemIpc = (void**)&syncResources->sendMemIpc;
    } else {
      syncDevMem = (void**)&syncResources->recvDevMem;
      syncMemIpc = (void**)&syncResources->recvMemIpc;
    }
    NCCLCHECK(ncclx::transport::getP2pSyncBufPtr(
        comm,
        isSend,
        channelId,
        connIndex,
        peerInfo->rank,
        &connInfo->syncBuff.directPtr,
        &connInfo->syncBuff.ipcDesc,
        &connInfo->syncBuff.size,
        &connInfo->syncBuff.offset));

    syncResources->comm = comm;
    // Map the sync buffer to be used later
    NCCLCHECK(p2pMap(
        comm,
        &conn->proxyConn,
        myInfo,
        comm->peerInfo + connInfo->rank,
        &connInfo->syncBuff,
        syncDevMem,
        syncMemIpc));

    *syncDevMem = (((char*)*syncDevMem) + connInfo->syncBuff.offset);
  }
...
ncclResult_t p2pSendSetup(struct ncclComm* comm, struct ncclTopoGraph* graph, struct ncclPeerInfo* myInfo, struct ncclPeerInfo* peerInfo,
  ...
  if (send->transportResources) {
    resources = (struct p2pResources*)send->transportResources;
  } else {
  ...
  // [META]: NCCLX can decouple ncclSendMem structure from protocol buffer, no need include its size in such case
  int sendSize = (NCCL_USE_MEM_CACHE) ? 0 : sizeof(struct ncclSendMem);
  ...
  if (intermediateRank == -1) {
    ...
    if (P2P_SAME_PID(myInfo, peerInfo) && NCCL_P2P_DIRECT_DISABLE == 0 && useMemcpy == 0) {
  ...
  // [META]: Allocate ncclSendMem separately, if needed
  if (NCCL_USE_MEM_CACHE) {
    NCCLCHECK(ncclxP2pSetupSyncResources(comm, myInfo, peerInfo, info, send, /*isSend=*/true, channelId, connIndex));
  }
  ...
  if (useMemcpy) {
    ...
    // [META]: NCCLX does not need to allocate protocol buffer for READ protocol
    if (!NCCL_USE_MEM_CACHE || info->read) {
...
ncclResult_t p2pRecvSetup(struct ncclComm* comm, struct ncclTopoGraph* graph, struct ncclPeerInfo* myInfo, struct ncclPeerInfo* peerInfo,
  ...
  struct p2pResources* resources;
  struct ncclP2pRequest req;
  if (recv->transportResources) {
    resources = (struct p2pResources*)recv->transportResources;
  } else {
    NCCLCHECK(ncclCalloc(&resources, 1));
  ...
  // [META]: NCCLX can decouple ncclRecvMem structure from protocol buffer, no need include its size in such case
  int recvSize = (NCCL_USE_MEM_CACHE) ? 0 : sizeof(struct ncclRecvMem);
  // For P2P Read the SIMPLE buffer is tagged on the end of the ncclSendMem structure
  ...
  // [META]: Allocate ncclRecvMem separately, if needed
  if (NCCL_USE_MEM_CACHE) {
    NCCLCHECK(ncclxP2pSetupSyncResources(comm, myInfo, peerInfo, info, recv, /*isSend=*/false, channelId, connIndex));
  }
...
static ncclResult_t p2pSendConnect(struct ncclComm* comm, struct ncclConnect* connectInfo, int nranks, int rank, struct ncclConnector* send) {
  ...
  struct p2pConnectInfo* info = (struct p2pConnectInfo*)connectInfo;
  // [META]: NCCLX may use dedicated sync resources
  struct p2pSyncResources* syncResources = (p2pSyncResources*) send->transportSyncResources;
  ...
  // [META]: Baseline NCCL needs offset to skip the ncclRecvMem structure in p2pBuff
  size_t offset = (syncResources) ? 0 : 1;
  char* buff = (char*)(remDevMem+offset);
  for (int p=0; p<NCCL_NUM_PROTOCOLS; p++) {
    if (info->read && p == NCCL_PROTO_SIMPLE) {
      /* For P2P Read the SIMPLE buffer is local (ncclSendMem) */
      if (resources->sendDevMem == NULL) return ncclInternalError; // We should not use read + memcpy
      send->conn.buffs[p] = (char*)(resources->sendDevMem+offset);
    } else {
  ...
  if (useMemcpy) {
    ...
    // [META]: If a dedicated sync buffer is used, only need to map/import it in first use
    remDevMem = syncResources->recvDevMem;
    if (!remDevMem) {
      NCCLCHECK(p2pMap(comm, &send->proxyConn, comm->peerInfo+rank, comm->peerInfo+info->rank, &info->syncBuff, (void**)&remDevMem, &syncResources->recvMemIpc));
      remDevMem = (struct ncclRecvMem*)(((char*) remDevMem) + info->syncBuff.offset);
      syncResources->recvDevMem = remDevMem;
    }
    send->conn.tail = &remDevMem->tail;
    send->conn.head = &syncResources->sendDevMem->head;
    send->conn.ptrExchange = &syncResources->sendDevMem->ptrExchange;
    send->conn.redOpArgExchange = syncResources->sendDevMem->redOpArgExchange;
  } else {
    send->conn.tail = &remDevMem->tail;
...
ncclResult_t p2pRecvConnect(struct ncclComm* comm, struct ncclConnect* connectInfo, int nranks, int rank, struct ncclConnector* recv) {
  ...
  // [META]: NCCLX may use dedicated sync resources
  struct p2pSyncResources* syncResources = (p2pSyncResources*) recv->transportSyncResources;
  struct p2pConnectInfo* info = (struct p2pConnectInfo*)connectInfo;
  ...
  if (useMemcpy) {
    ...
    struct ncclSendMem* remSyncDevMem = syncResources->sendDevMem;
    // [META]: If a dedicated sync buffer is used, only need to map/import it once
    if (!remSyncDevMem) {
      NCCLCHECK(p2pMap(comm, &recv->proxyConn, comm->peerInfo+rank, comm->peerInfo+info->rank, &info->syncBuff, (void**)&remSyncDevMem, &syncResources->sendMemIpc));
      remSyncDevMem = (struct ncclSendMem*)(((char*) remSyncDevMem) + info->syncBuff.offset);
      syncResources->sendDevMem = remSyncDevMem;
    }

    struct ncclRecvMem* devMem = syncResources->recvDevMem;
    recv->conn.tail = &devMem->tail;
    recv->conn.head = &remSyncDevMem->head;
    recv->conn.ptrExchange = &remSyncDevMem->ptrExchange;
    recv->conn.redOpArgExchange = remSyncDevMem->redOpArgExchange;
    if (info->read) {
  ...
  // [META]: Baseline NCCL needs offset to skip the ncclRecvMem structure in p2pBuff
  size_t offset = (syncResources) ? 0 : 1;
  char* buff = (char*)(resources->recvDevMem+offset);
  for (int p=0; p<NCCL_NUM_PROTOCOLS; p++) {
    if (info->read && p == NCCL_PROTO_SIMPLE) {
      ...
      recv->conn.buffs[p] = (char*)(remDevMem+offset);
    } else {
...
static ncclResult_t p2pSendProxySetup(struct ncclProxyConnection* connection, struct ncclProxyState* proxyState, void* reqBuff, int reqSize, void* respBuff, int respSize, int* done) {
  if (useMemcpy) {
    ...
    auto logKey = ncclx::memory::genKey(
              "ProxySetup",
              /*isP2p=*/true,
              /*isSend=*/true,
              req->channelId,
    ...
    if (proxyState->owner->memCache) {
      NCCLCHECK(ncclx::memory::metaAllocateShareableBuffer(
          ...
          &p2pBuff->directPtr,
          logKey.c_str(),
          proxyState->owner->memCache,
          &proxyState->owner->logMetaData));
      auto hashKey = folly::sformat("{}:{:#x}", logKey, proxyState->owner->commHash);
      proxyState->owner->connSetupBufKeys.push_back(std::move(hashKey));
    } else {
        ...
        *regBufFlag = 1;
        ...
        if (ncclCuMemEnable()) {
          ...
          if (CUPFN(cuMemRetainAllocationHandle(&handle, baseAddr)) != CUDA_SUCCESS) {
            ...
            if (comm->directMode || !NCCL_LEGACY_CUDA_REGISTER) goto fail;
...
ncclResult_t ncclxP2pFreeSyncBuffer(void* transportSyncResources) {
struct p2pSyncResources* resources = (struct p2pSyncResources*) transportSyncResources;
  if (resources) {
    if (ncclCuMemEnable()) {
      if (resources->sendMemIpc) {
        NCCLCHECK(ncclCuMemFreeAddr(resources->sendMemIpc));
      ...
      if (resources->sendMemIpc){
        CUDACHECK(cudaIpcCloseMemHandle(resources->sendMemIpc));
      }
      if (resources->recvMemIpc) {
        CUDACHECK(cudaIpcCloseMemHandle(resources->recvMemIpc));
      }
    }
    NCCLCHECK(ncclx::transport::releaseP2pSyncBuf(resources->comm));
    free(resources);
  }
  return ncclSuccess;
}
