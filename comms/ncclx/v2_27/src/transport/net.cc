...
static ncclResult_t sendSetup(struct ncclComm* comm, struct ncclTopoGraph* graph, struct ncclPeerInfo* myInfo, struct ncclPeerInfo* peerInfo, struct ncclConnect* connectInfo, struct ncclConnector* send, int channelId, int connIndex) {
  ...
  send->conn.shared = req.shared = graph || connIndex == 0 ? 0 : NCCL_NET_SHARED_BUFFERS != -2 ? NCCL_NET_SHARED_BUFFERS : 1;
...
static ncclResult_t recvSetup(struct ncclComm* comm, struct ncclTopoGraph* graph, struct ncclPeerInfo* myInfo, struct ncclPeerInfo* peerInfo, struct ncclConnect* connectInfo, struct ncclConnector* recv, int channelId, int connIndex) {
  ...
  recv->conn.shared = req.shared = graph || connIndex == 0 ? 0 : NCCL_NET_SHARED_BUFFERS != -2 ? NCCL_NET_SHARED_BUFFERS : 1;
  req.channelId = channelId;
  req.connIndex = connIndex;
...
static ncclResult_t sendProxyConnect(struct ncclProxyConnection* connection, struct ncclProxyState* proxyState, void* reqBuff, int reqSize, void* respBuff, int respSize, int* done) {
  ...
  *done = 1;
  ...
  if (ncclGdrCopy && map->sameProcess && NCCL_GDRCOPY_SYNC_ENABLE) {
    ...
    NCCLCHECK(ncclGdrCudaCalloc(&cpuPtr, &gpuPtr, 1, &resources->gdrDesc, proxyState->owner));
...
static ncclResult_t recvProxyConnect(struct ncclProxyConnection* connection, struct ncclProxyState* proxyState, void* reqBuff, int reqSize, void* respBuff, int respSize, int* done) {
  ...
  *done = 1;
  ...
  if (ncclGdrCopy && map->sameProcess) {
    uint64_t *cpuPtr, *gpuPtr;
    NCCLCHECK(ncclGdrCudaCalloc(&cpuPtr, &gpuPtr, 2, &resources->gdrDesc, proxyState->owner));
        ...
        if (connFifo[buffSlot].size != -1 && (*recvTail > tail || p == NCCL_PROTO_LL)) {
          ...
          if (ready) {
            ...
            if(ProxyMockNetSendFailure::mock(sub, sub->transmitted, sub->requests + buffSlot) == false) {
              ...
              if (proxyState->ncclNet->logCollective) {
                NCCLCHECK(proxyState->ncclNet->logCollective(resources->netSendComm, args->coll, proxyState->commHash, true));
              }
            }
...
static ncclResult_t recvProxyProgress(struct ncclProxyState* proxyState, struct ncclProxyArgs* args) {
  ...
  if (args->state == ncclProxyOpProgress) {
    ...
    for (int s=0; s<args->nsubs; s+=args->subs[s].groupSize) {
      ...
      // @yulunwang: Adding this memset because in loop https://fburl.com/code/923lmysd
      // sometimes sizes[i] will access uninitialized memory. This serves as a
      // stop gap to ensure we are not getting crazy result for the receive
      // size.
      ...
      if (subCount) {
        ...
        bool ignoreCompletion = NCCL_NET_OPTIONAL_RECV_COMPLETION_CVAR && ((args->protocol == NCCL_PROTO_LL128) || (args->protocol == NCCL_PROTO_LL)) && (subCount == 1);
        ...
        if (proxyState->ncclNet->logCollective) {
          NCCLCHECK(proxyState->ncclNet->logCollective(resources->netRecvComm, args->coll, proxyState->commHash, false));
        }
        if (*requestPtr) {
