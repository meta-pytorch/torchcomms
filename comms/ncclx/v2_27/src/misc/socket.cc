...
static ncclResult_t socketWait(int op, struct ncclSocket* sock, void* ptr, int size, int* offset) {
  while (*offset < size) {
    ...
    // (meta-ncclx) Wait for the data to be available for read before retrying. Without this the
    // socketWait will perform recv in a loop causing significant CPU usage. See
    // strobelight profiling from emulation run - https://pxl.cl/5ww0g
    if (NCCL_FASTINIT_MODE != NCCL_FASTINIT_MODE::none &&
        (*offset < size) &&
        (op == NCCL_SOCKET_RECV || op == NCCL_SOCKET_SEND)) {
      struct pollfd pfd{
          .fd = sock->fd,
          .events = static_cast<short>((op == NCCL_SOCKET_RECV) ? POLLIN : POLLOUT),
          .revents = 0,
      };
      poll(&pfd, 1, 100 /* in ms */); // Poll with short timeout before retrying.
    }
  }
  return ncclSuccess;
}
...
static int envSocketFamily(void) {
  ...
  const char* env = NCCL_SOCKET_FAMILY.c_str();
...
ncclResult_t ncclFindInterfaces(char* ifNames, union ncclSocketAddress *ifAddrs, int ifNameMaxSize, int maxIfs,
  ...
  *nIfs = 0;
  if (env && strlen(env) > 1) {
    ...
    if (*nIfs == 0) {
      const char* commId = NCCL_COMM_ID.c_str();
...
static ncclResult_t socketTryAccept(struct ncclSocket* sock) {
  ...
  if (sock->fd != -1) {
    sock->state = ncclSocketStateAccepted;
             ...
             errno == ENONET || errno == EHOSTUNREACH || errno == EOPNOTSUPP || errno == ENETUNREACH || errno == EINTR) {
    ...
    if (++sock->errorRetries == NCCL_SOCKET_RETRY_CNT) {
...
static ncclResult_t socketSetFlags(struct ncclSocket* sock) {
  ...
  int rcvBuf = NCCL_SOCKET_RCVBUF, sndBuf = NCCL_SOCKET_SNDBUF;
...
static ncclResult_t socketFinalizeAccept(struct ncclSocket* sock) {
  ...
  if (sock->asyncFlag == 0 || sock->finalizeCounter < sizeof(magic)) {
    ...
    if (magic != sock->magic) {
      WARN("socketFinalizeAccept from %s: wrong magic %lx != %lx", ncclSocketToString(&sock->addr, line), magic, sock->magic);
      socketResetAccept(sock);
      return ncclSuccess;
    }
  }
...
static ncclResult_t socketConnectCheck(struct ncclSocket* sock, int errCode, const char funcName[]) {
  ...
  if (errCode == 0) {
    ...
    if (sock->customRetry == 0) {
      ...
      unsigned int sleepTime = sock->errorRetries * NCCL_SOCKET_RETRY_SLEEP_MSEC;
      INFO(NCCL_NET|NCCL_INIT, "%s: connect to %s returned %s, retrying (%d/%ld) after sleep for %u msec",
           ...
           sock->errorRetries, NCCL_SOCKET_RETRY_CNT, sleepTime);
      msleep(sleepTime);
    }
...
ncclResult_t ncclSocketConnect(struct ncclSocket* sock, const char* localIfName) {
  ...
  if (localIfName != nullptr) {
    ...
    SYSCHECK(setsockopt(sock->fd, SOL_SOCKET, SO_BINDTODEVICE, (void*)&ifr.ifr_name, sizeof(ifr)), "setsockopt");
    TRACE(NCCL_INIT, "ncclSocketConnect bind to interface %s", localIfName);
  }
  ...
  if (addr) {
    ...
    // referenced D77281608
      if (family == AF_INET6) {
        ...
        SYSCHECK(setsockopt(sock->fd, IPPROTO_IP, IP_TOS, (char*)&NCCL_SOCKET_TOS_CONFIG, sizeof(int)), "setsockopt");
      }
    }
  } else {
