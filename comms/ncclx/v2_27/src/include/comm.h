...
struct ncclComm {
  ...
  std::optional<struct ncclDevCommAndChannels*> devCommAndChans{std::nullopt};
...
// There are other ways to do this without triggering -Winvalid-offsetof, but is it worthwhile?
// These conditions are too trivial, and very unlikely ever to be violated.
// We could add compiler+version-specific pragmas to suppress the warnings but not the eventual error.
