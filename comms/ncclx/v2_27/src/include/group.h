...
inline void ncclGroupCommJoin(struct ncclComm* comm, int type) {
  if (comm->groupNext[type] == reinterpret_cast<struct ncclComm*>(0x1)) {
    ...
    /* if ncclGroupCommHead points to a comm that has already been
     * removed by another thread, clean it up from this thread's TLS */
    if (ncclGroupCommHead[type] && ncclGroupCommHead[type]->groupNext[type] == reinterpret_cast<struct ncclComm*>(0x1)) {
