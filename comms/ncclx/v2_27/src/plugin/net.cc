...
static ncclResult_t ncclNetPluginInit(netPluginLib_t* pluginLib) {
  ...
  if (pluginLib->ncclNetPluginState == ncclNetPluginStateInitReady && pluginLib->ncclNet) {
    if (pluginLib->ncclNet->init(ncclMetaDebugLog, ncclProfilerCallback) != ncclSuccess) goto fail;
  ...
  if (pluginLib->ncclCollNetPluginState == ncclNetPluginStateInitReady && pluginLib->ncclCollNet) {
    if (pluginLib->ncclCollNet->init(ncclMetaDebugLog) != ncclSuccess) pluginLib->ncclCollNetPluginState = ncclNetPluginStateDisabled;
...
static void initPluginLibsOnceFunc() {
  ...
  envNetPlugin = NCCL_NET_PLUGIN.c_str();
  if (envNetPlugin) {
    ...
    while(netPluginName) {
      ...
      if((strlen(netPluginName)+1) <= MAX_STR_LEN) {
        ...
        netPluginLibs[pluginCounter].ncclNetPluginRefCount = NCCL_NET_PLUGIN_REF_COUNT;
