...
static ncclResult_t ncclProfilerPluginLoad(void) {
  ...
  profilerPluginLib = ncclOpenProfilerPluginLib(NCCL_PROFILER_PLUGIN.c_str());
...
ncclResult_t ncclProfilerPluginInit(struct ncclComm* comm) {
  ...
  if (__builtin_expect(ncclProfiler != NULL, 0)) {
    int err = ncclProfiler->init(&comm->profilerContext, &ncclProfilerEventMask, comm->config.commName, comm->commHash, comm->nNodes, comm->nRanks, comm->rank, ncclMetaDebugLog);
    if (err) {
