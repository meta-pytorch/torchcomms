...
ncclResult_t ncclCommGroupRegisterSymmetric(struct ncclAsyncJob* job_) {
  ...
  if (comm->baseStride == 0) {
    ...
    if (NCCL_WIN_STRIDE != -1) {
      comm->baseStride = NCCL_WIN_STRIDE;
    } else {
      size_t maxStride = 0;
...
static ncclResult_t groupLaunch(struct ncclAsyncJob *job_, ncclSimInfo_t* simInfo = NULL) {
  ...
  if (!simInfo && groupCommPreconnectHeadMain != nullptr) {
    ...
    do {
      if (ncclx::transport::useTransportProxy(comm)) {
        NCCLCHECKGOTO(
            metaCommToNccl(
              comm->transportProxy_->enqueueP2pPreconnect(comm)),
            ret, fail);
      } else {
  ...
  // If ncclx transport proxy thread is enabled, wait for p2p preconnect to finish
  if (!simInfo && groupCommPreconnectHeadMain != nullptr &&
      ncclx::transport::useTransportProxy(groupCommPreconnectHeadMain)) {
    struct ncclComm* comm = groupCommPreconnectHeadMain;
    NCCLCHECKGOTO(
      metaCommToNccl(
          comm->transportProxy_->waitPreconnect()), 
      ret, fail);
  }
    ...
    do {
      ...
      do {
        ...
        if (comm->cuMemSupport && needConnect) {
          ...
          NCCLCHECKGOTO(
            metaCommToNccl(
              comm->transportProxy_->enqueueCollPreconnect(
                comm, algoNeedConnect)),
            ret, fail);
          } else {
            struct ncclPreconnectJob* job;
            NCCLCHECKGOTO(ncclCalloc(&job, 1), ret, fail);
      ...
      // If ncclx transport proxy thread is enabled, wait for coll preconnect to finish
      comm = cliqueHead;
      do {  
        if (ncclx::transport::useTransportProxy(comm)) { 
          NCCLCHECKGOTO(
