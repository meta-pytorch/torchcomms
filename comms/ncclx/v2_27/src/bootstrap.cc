...
static ncclResult_t netGetDevice(int rank, struct ncclComm* comm, int* dev) {
  ...
  if (devOOB < 0) {
    pthread_mutex_lock(&bootstrapNetLock);
    if (devOOB < 0) {
      const char* userIfEnv = NCCL_OOB_NET_IFNAME.c_str();
...
ncclResult_t bootstrapInit(int nHandles, void* handles, struct ncclComm* comm) { // from bootstrap ring via root
  auto sampleGuardBegin = EVENTS_SCUBA_UTIL_SAMPLE_GUARD("INIT");
  sampleGuardBegin.sample().setCommunicatorMetadata(comm? &comm->logMetaData: nullptr);
  ...
  if (isFastInitRingMode(state->fastInitMode) && !skipFormRingViaTcpStore) {
    ...
    if (nRankRoot > NCCL_UID_STAGGER_THRESHOLD) {
      // for socket the message rate in microsec
      double msg_rate = NCCL_UID_STAGGER_RATE / 1.0e6;
  ...
  if (NCCL_RAS_ENABLE == 1 && performRasAddRanks) {
...
ncclResult_t ncclxBidirRingAllGather(struct ncclSocket* prevSocket, struct ncclSocket* nextSocket, int rank, int nranks, char* data, int size) {
  // BidirectionalBootstrapAllGather provided by ncclx
  ...
  * will do send and receive data to both of its peers. At each step i, we
  ...
  auto forwardLoop = [&]() {
    for (int i=0; i < (nranks / 2); i++) {
      ...
      NCCLCHECK(ncclSocketSend(nextSocket, data + (ssliceNext * size), size));
      NCCLCHECK(ncclSocketRecv(prevSocket, data + (rslicePrev * size), size));
    }
    return ncclSuccess;
  };
  auto backwardLoop = [&]() {
    for (int i=0; i < (nranks / 2); i++) {
      ...
      NCCLCHECK(ncclSocketSend(prevSocket, data + (sslicePrev * size), size));
      NCCLCHECK(ncclSocketRecv(nextSocket, data + (rsliceNext * size), size));
    }
    return ncclSuccess;
  };
  ...
  NCCLCHECK(forwardLoopFuture.get());
  NCCLCHECK(backwardLoopFuture.get());
