    ...
    if (peer) {
      if (ncclAtomicRefCountDecrement(&peer->refCount) == 0) {
        for (int b=0; b<NCCL_MAX_CONNS; b++) {
          ...
          if (peer->send[b].transportSyncResources) {
            NCCLCHECK(ncclxP2pFreeSyncBuffer(peer->send[b].transportSyncResources));
            peer->send[b].transportSyncResources = nullptr;
          }
          if (peer->recv[b].transportSyncResources) {
            NCCLCHECK(ncclxP2pFreeSyncBuffer(peer->recv[b].transportSyncResources));
            peer->recv[b].transportSyncResources = nullptr;
          }
        }
        if (r == nRanks) {
