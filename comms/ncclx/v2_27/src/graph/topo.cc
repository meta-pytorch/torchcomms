...
ncclResult_t ncclTopoMakeVNics(struct ncclXml* xml, ncclResult_t (*makeVDevice)(int*, ncclNetVDeviceProps_t*), ncclResult_t (*getProperties)(int, ncclNetProperties_t*), int physicalDevs) {
  ...
  { // Avoids warnings related to jumping to "out"
    if (!NCCL_NET_MERGE_LEVEL.empty()) kvConvertToInt(NCCL_NET_MERGE_LEVEL.c_str(), &mergeLevel, nicPathKvList);
    NCCLCHECK(ncclCalloc(&placedDevs, physicalDevs));
    memset(placedDevs, 0, sizeof(int)*physicalDevs);

    if (NCCL_NET_FORCE_MERGE.size()) {
      char* forceMerge = strdup(NCCL_NET_FORCE_MERGE.c_str());
...
ncclResult_t ncclTopoGetSystem(struct ncclComm* comm, struct ncclTopoSystem** system, const char* dumpXmlFile) {
  ...
  if (!NCCL_TOPO_FILE.empty()) {
    ...
    if (customTopoFile) {
      INFO(
          NCCL_ENV,
          "NCCL_TOPO_FILE set by environment to %s",
          NCCL_TOPO_FILE.c_str());
    }
    // If default XML topology file is used,
    // do not fail if the file does not exist
    NCCLCHECKGOTO(
        ncclTopoGetXmlFromFile(
            NCCL_TOPO_FILE.c_str(), xml, customTopoFile ? 1 : 0),
        ret,
        fail);
  }
  ...
  if (dumpXmlFile && comm->rank == NCCL_TOPO_DUMP_FILE_RANK) {
