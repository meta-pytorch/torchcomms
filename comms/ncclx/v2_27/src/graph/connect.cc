...
int ncclMaxNchannels() {
  ...
  maxNchannels = std::min(maxNchannels, ncclDevMaxChannelsForArgsBytes(NCCL_WORK_ARGS_BYTES));
...
ncclResult_t ncclTopoPostset(struct ncclComm* comm, int* firstRanks, int* treePatterns, struct ncclTopoRanks** allTopoRanks, int* rings, struct ncclTopoGraph** graphs, struct ncclComm* parent) {
  ...
  if (comm->netDeviceType == NCCL_NET_DEVICE_UNPACK && comm->nNodes > 1 && nChannels < 16 && NCCL_UNPACK_DOUBLE_NCHANNELS) {
     nChannels = comm->nChannels = copyChannels(comm, nChannels, 2*nChannels, ringPrev, ringNext);
  }
  ...
  if (NCCL_FASTINIT_MODE == NCCL_FASTINIT_MODE::none) {
    ...
    NCCLCHECK(ncclxBuildRings(nChannels, rings, comm->rank, comm->nRanks, ringPrev, ringNext));
  }
