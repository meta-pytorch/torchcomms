...
// Thread-safe wrapper for ncclWindow_vidmem -> ncclWin mapping
//
// Design Note: We use a global map because ncclWindow_vidmem is a device-side
...
//
// Usage Pattern:
//   ncclWindow_vidmem* handle = allocate_window(...);
...
class NcclWinMap {
 public:
  ...
  void insert(ncclWindow_vidmem* handle, ncclWin* win) {
    auto locked = map_.wlock();
    (*locked)[handle] = win;
  }

  ncclWin* find(ncclWindow_vidmem* handle) const {
    auto locked = map_.rlock();
    auto it = locked->find(handle);
    return (it != locked->end()) ? it->second : nullptr;
  }

  void erase(ncclWindow_vidmem* handle) {
    auto locked = map_.wlock();
    locked->erase(handle);
  }
 ...
 private:
  ...
  folly::Synchronized<std::unordered_map<ncclWindow_vidmem*, ncclWin*>> map_;
};
