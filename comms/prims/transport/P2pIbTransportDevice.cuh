...
template <typename Proto>
__device__ __forceinline__ void P2pIbTransportDevice::abandon_send_progress(
    ThreadGroup& group) {
  if (type == P2pIbBackendType::IBRC) {
    detail::abandon_send_progress_state<Proto>(*ibrc, group);
  } else {
    detail::abandon_send_progress_state<Proto>(*ibgda, group);
  }
}
...
__device__ __forceinline__ void P2pIbTransportDevice::abandon_recv_progress(
    ThreadGroup& group) {
  if (type == P2pIbBackendType::IBRC) {
    detail::abandon_recv_progress_state<Proto>(*ibrc, group);
  } else {
    detail::abandon_recv_progress_state<Proto>(*ibgda, group);
  }
}
