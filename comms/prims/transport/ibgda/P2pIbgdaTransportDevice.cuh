...
__device__ __forceinline__ bool checkIbgdaSqCapacityAbort(
...
inline constexpr uint32_t kIbgdaSqFailureReport = 1U;
inline constexpr uint32_t kIbgdaSqFailureTrap = 2U;

inline __device__ __noinline__ uint32_t classifyIbgdaSqCapacityPollFailure(
    comms::fault_tolerance::AbortState* abortState,
    comms::fault_tolerance::AbortBehavior abortBehavior) {
  if (abortState == nullptr) {
    return kIbgdaSqFailureReport | kIbgdaSqFailureTrap;
  }
  const bool firstWriter = comms::fault_tolerance::detail::deviceTrySetAbort(
      abortState,
      comms::fault_tolerance::AbortReason::NETWORK_ERROR,
      /*context=*/nullptr);
  uint32_t action = firstWriter ? kIbgdaSqFailureReport : 0U;
  if (abortBehavior == comms::fault_tolerance::AbortBehavior::TRAP) {
    action |= kIbgdaSqFailureReport | kIbgdaSqFailureTrap;
  }
  return action;
}

inline __device__ __noinline__ void reportIbgdaSqCapacityPollFailure(
    uint64_t ticket,
    int status) {
  printf(
...
template <
    ...
    doca_gpu_dev_verbs_sync_scope AcquireScope>
__device__ __forceinline__ int pollIbgdaSqStatusOnce(
    doca_gpu_dev_verbs_cq* cq,
    uint64_t ticket,
    bool collapsedCq) {
  int status;
...
__device__ __forceinline__ IbgdaSqPollResult pollIbgdaSqOnce(
    ...
    bool collapsedCq,
    const AbortDevice& abortDevice) {
  const int status = pollIbgdaSqStatusOnce<CqSharingMode, AcquireScope>(
      cq, ticket, collapsedCq);
  const bool aborted =
      status == EBUSY && checkIbgdaSqCapacityAbort(abortDevice, ticket);
  return {.status = status, .aborted = aborted};
}

template <doca_gpu_dev_verbs_sync_scope AcquireScope>
// A reference here can make nvcc address a `__grid_constant__` kernel argument
// as thread-local storage across this noinline boundary.
__device__ __noinline__ IbgdaWqeReservation waitForIbgdaSqCapacity(
  ...
  uint64_t nextPollCycles =
      comms::fault_tolerance::detail::deviceClock() + pollIntervalCycles;
  int status;
  do {
    status =
        cqSharingMode == DOCA_GPUNETIO_VERBS_RESOURCE_SHARING_MODE_EXCLUSIVE
        ? pollIbgdaSqStatusOnce<
              DOCA_GPUNETIO_VERBS_RESOURCE_SHARING_MODE_EXCLUSIVE,
              AcquireScope>(cq, completionTicket, collapsedCq)
        : pollIbgdaSqStatusOnce<
              DOCA_GPUNETIO_VERBS_RESOURCE_SHARING_MODE_GPU,
              AcquireScope>(cq, completionTicket, collapsedCq);
    if (status == EBUSY && abortState != nullptr) {
      const uint64_t now = comms::fault_tolerance::detail::deviceClock();
      const bool deadlineDue = deadlineCycles != 0 && now >= deadlineCycles;
      if (deadlineDue || now >= nextPollCycles) {
        nextPollCycles = now + pollIntervalCycles;
        const uint32_t slowResult =
            comms::fault_tolerance::detail::deviceCheckExpiredSlow(
                abortState, deadlineDue);
        if ((slowResult &
             comms::fault_tolerance::detail::kAbortSlowCheckAborted) != 0U) {
          const bool flippedHere =
              (slowResult &
               comms::fault_tolerance::detail::kAbortSlowCheckFlippedHere) !=
              0U;
          if (flippedHere) {
            printf(
                FT_ABORT_SITE_
                "IBGDA SQ capacity wait timed out (ticket=%llu)" FT_ABORT_SITE_SUFFIX_,
                static_cast<unsigned long long>(completionTicket),
                __func__);
          } else if (
              abortBehavior == comms::fault_tolerance::AbortBehavior::TRAP) {
            printf(
                "CUDA ABORT ERROR: IBGDA SQ capacity wait timed out "
                "(ticket=%llu)" FT_ABORT_SITE_SUFFIX_,
                static_cast<unsigned long long>(completionTicket),
                __func__);
          }
          if (abortBehavior == comms::fault_tolerance::AbortBehavior::TRAP) {
  ...
  const uint32_t failureAction =
      classifyIbgdaSqCapacityPollFailure(abortState, abortBehavior);
  if ((failureAction & kIbgdaSqFailureReport) != 0U) {
    reportIbgdaSqCapacityPollFailure(completionTicket, status);
  }
  if ((failureAction & kIbgdaSqFailureTrap) != 0U) {
    PIPES_DEVICE_TRAP();
  }
  return {.firstWqe = firstWqe, .acquired = false};
}
...
__device__ __forceinline__ IbgdaWqeReservation tryReserveIbgdaWqes(
  ...
  if (lastWqe < __ldg(&qp->sq_wqe_num)) {
    return {.firstWqe = firstWqe, .acquired = true};
  }
  static_assert(
      CqSharingMode == DOCA_GPUNETIO_VERBS_RESOURCE_SHARING_MODE_EXCLUSIVE ||
      CqSharingMode == DOCA_GPUNETIO_VERBS_RESOURCE_SHARING_MODE_GPU);
  return waitForIbgdaSqCapacity<AcquireScope>(
      qp,
      firstWqe,
      lastWqe,
      collapsedCq,
      CqSharingMode,
      abortDevice.stateForFlag(),
      abortDevice.behavior(),
      abortDevice.deadlineCycles(),
      abortDevice.pollIntervalCycles());
}
...
class P2pIbgdaTransportDevice {
 ...
 private:
  ...
  __device__ IbLocalCompletionTicket put_impl(
    ...
    if (group.is_leader()) {
      ...
      if (hasSignal && hasCounter) {
        ...
        IbgdaPutSignalTickets tickets;
        if (abortDevice != nullptr) {
          tickets = put_signal_single_impl(
              lane,
              localBuf,
              remoteBuf,
              nbytes,
          ...
          const AbortDevice disabledAbort;
          tickets = put_signal_single_impl(
              lane,
              localBuf,
              remoteBuf,
        ...
        const auto ticket = abortDevice != nullptr
            ? try_put_single_impl(
                  lane, localBuf, remoteBuf, nbytes, *abortDevice)
            : IbgdaPutTicket{
  ...
  __device__ uint64_t put_single_impl(
    ...
    return ticket;
#else
    const uint32_t numChunks = maxDataWqeCount(nbytes);
    const uint64_t firstWqe =
        reserve_wqes_mode<DOCA_GPUNETIO_VERBS_RESOURCE_SHARING_MODE_GPU>(
            lane.qp, numChunks);
    return put_single_local(
        lane.qp, remoteAddr, localAddr, nbytes, firstWqe, numChunks);
#endif
  }

  __device__ IbgdaPutTicket try_put_single_impl(
      const IbgdaLane& lane,
      const IbgdaLocalBuffer& localBuf,
      const IbgdaRemoteBuffer& remoteBuf,
      std::size_t nbytes,
...
#ifndef __HIP_PLATFORM_AMD__
  __device__ __forceinline__ static uint32_t maxDataWqeCount(
      std::size_t nbytes) {
    ...
    return numChunks > 1 ? numChunks : 1;
  }
