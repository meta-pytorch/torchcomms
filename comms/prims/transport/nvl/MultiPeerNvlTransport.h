 ...
 *   dataBufferSize = max_num_channels * perChannelSize
 *
...
struct MultiPeerNvlTransportConfig {
  ...
  // When max_num_channels > 0, this is derived from perChannelSize and
  // max_num_channels during host transport construction.
  ...
  // Must be 16-byte aligned when max_num_channels > 0.
...
class MultiPeerNvlTransport {
 ...
 private:
  ...
  // Per-peer NvlChannelState arrays (length = max_num_channels per peer).
  ...
  // Allocated when max_num_channels > 0.
