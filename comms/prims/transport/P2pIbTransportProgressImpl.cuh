 ...
 *
 * Do not pass a kernel-argument aggregate through this boundary. The fused
 * collectives use __grid_constant__ arguments, and taking one by reference in
 * an emitted device function can rematerialize an out-of-bounds local copy.
 * The references below point only to per-thread locals or transport storage.
 */
template <bool HasSignal, typename Transport>
[[nodiscard]] __device__ __noinline__ IbLocalCompletionTicket
