...
class P2pIbrcHostWriter {
 public:
   ...
   *
   * @param timeout bound for each individual wait -- NOT for a whole
   *                operation; see begin_op() when several waits must share one
   *                budget. Zero or negative waits forever (only appropriate
   *                for tests)
  ...
  /**
   * Per-wait bound only. Deliberately separate from the predicate:
  ...
  /**
   * Scopes one absolute deadline across every wait the writer performs while
   * the guard is alive, and restores the previous budget when it dies.
   *
   * waitTimeout_ bounds each wait on its own, restarting for every
   * poll_counter() / fence() / backpressure spin. An operation built from N
   * waits can therefore run to N * timeout, so a caller that forwards a single
   * operation timeout does not get the deadline it asked for. Under a budget
   * the waits share one absolute point instead, whatever each has spent.
   *
   * Nesting narrows and never extends: an inner budget longer than the
   * enclosing one is ignored. A zero or negative budget adds no bound of its
   * own, leaving any enclosing budget in force.
   *
   * The guard borrows the writer, so it must not outlive it, and the writer
   * must not be moved while one is alive.
   */
  class OpBudget {
   public:
    OpBudget(const OpBudget&) = delete;
    OpBudget& operator=(const OpBudget&) = delete;
    // Movable so a caller holding several writers -- P2pIbrcHostLanes puts one
    // budget on every lane -- can keep their guards in a container. A
    // moved-from guard restores nothing; the destination owns the restore.
    OpBudget(OpBudget&& other) noexcept
        : writer_(std::exchange(other.writer_, nullptr)),
          previous_(other.previous_) {}
    OpBudget& operator=(OpBudget&& other) noexcept {
      if (this != &other) {
        restore();
        writer_ = std::exchange(other.writer_, nullptr);
        previous_ = other.previous_;
   ...
   private:
    friend class P2pIbrcHostWriter;
    using Deadline = std::optional<std::chrono::steady_clock::time_point>;

    OpBudget(P2pIbrcHostWriter* writer, Deadline previous)
        : writer_(writer), previous_(previous) {}

    void restore() {
      if (writer_ != nullptr) {
        writer_->opDeadline_ = previous_;
      }
    }

    P2pIbrcHostWriter* writer_;
    Deadline previous_;
  };

  [[nodiscard]] OpBudget begin_op(std::chrono::milliseconds budget) {
    if (budget <= std::chrono::milliseconds::zero()) {
      return OpBudget(this, opDeadline_);
    }
    return begin_op_until(std::chrono::steady_clock::now() + budget);
  }

  /**
   * begin_op() against a deadline the caller already computed.
   *
   * Which one to use decides when an operation's clock starts, and that is a
   * real choice when the waits run on a different thread than the submitter:
   * begin_op() on the executing thread starts it when the work begins, while an
   * absolute deadline computed by the submitter starts it at submission and so
   * charges the operation for any time it spent queued.
   */
  [[nodiscard]] OpBudget begin_op_until(
      std::chrono::steady_clock::time_point deadline) {
    const auto previous = opDeadline_;
    if (!opDeadline_.has_value() || deadline < *opDeadline_) {
      opDeadline_ = deadline;
    }
    return OpBudget(this, previous);
  }
 ...
 private:
  ...
  uint64_t enqueue(const IbrcDesc& desc) {
    ...
    /*
     * One-shot, before anything is claimed. spin_until() reaches the predicate
     * only once the ring is full, so the free-slot fast path needs its own
     * check: a producer on an aborted communicator must leave the wire
     * untouched -- no sequence number taken, no descriptor published. Mirrors
     * P2pIbrcTransportDevice::reserve().
     */
    if (is_aborted()) {
      throw std::runtime_error(
          "P2pIbrcHostWriter: aborted before reserving a ring slot");
    }
  ...
  void spin_until(Ready&& ready, const char* what, uint64_t target) const {
    ...
    const bool bounded = opDeadline_.has_value() ||
        waitTimeout_ > std::chrono::milliseconds::zero();
    // An operation budget is absolute and was computed at begin_op(), so it
    // starts armed and a wait entering it already expired throws on its first
    // check. A bare waitTimeout_ is relative, and is armed on the first failed
    // ready() rather than up front, so the fast path -- fence() per ring step,
    // already satisfied -- never reads the clock.
    std::chrono::steady_clock::time_point deadline =
        opDeadline_.value_or(std::chrono::steady_clock::time_point{});
    bool armed = opDeadline_.has_value();
  ...
  bool is_aborted() const {
    return aborted_ && aborted_();
  }

  std::string timeout_description() const {
    if (opDeadline_.has_value()) {
      return "operation budget expired";
    }
    return fmt::format("timed out after {}ms", waitTimeout_.count());
  }
  ...
  // Absolute, and shared by every wait while a begin_op() guard is alive.
