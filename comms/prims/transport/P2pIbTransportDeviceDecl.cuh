...
// `Aborted` terminalizes local progress but does not prove completion of WQEs
// published before cancellation. Their registered source must remain alive and
// unchanged until teardown or reconfiguration of the owning transport has
// completed.
...
struct P2pIbTransportDevice {
  ...
  template <typename Proto = protocol::Simple>
  __device__ __forceinline__ void abandon_send_progress(ThreadGroup& group);
