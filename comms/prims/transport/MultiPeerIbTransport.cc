...
void MultiPeerIbTransportBase::allocateSendRecvBuffersEager(
  ...
  if (counterStorage == IbCounterStorage::Device) {
    ...
    // CPU proxy writes it on CQE; the device reads via the mapped pointer
    // (IBRC). lkeys are unused (no RDMA target), so wrap with empty keys.
...
void MultiPeerIbTransportBase::allocateSendRecvBufferForPeer(
  ...
  if (deviceCounter) {
    ...
    pb.counter = IbgdaLocalBuffer(alloc.hostPtr, NetworkLKeys{});
