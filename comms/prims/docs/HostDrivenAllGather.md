...
The implementation is split across the host-writer, host-lane, host-collective
...
host lanes, 128 total lanes, and `numNics * MCCL_HOSTRING_QPS_PER_NIC`.
...
The NVL-only path also uses Copy-Engine work. When MCCL can resolve the
caller's receive allocation to a registered multicast VA, the source chunk is
copied once to that multicast VA for the chunk offset. If that multicast mapping
is unavailable, the path opens remote receive slots with fabric handles and uses
unicast Copy-Engine copies; CUDA IPC handles do not cover cross-host peers in
one MNNVL domain.
...
- `MCCL_HOSTRING_DIRECT_HOST_WAIT=0` keeps the Copy-Engine bridge on the
  forwarding path instead of polling immediate-mode arrival counters directly
  from the host.
...
- `MCCL_HOSTRING_NVL_MULTIMEM=0` disables multicast-VA resolution for the
  NVL-only path, forcing the fabric-handle unicast fallback.
- `MCCL_HOSTRING_NVL_CE_MULTICAST=0` disables the `cudaMemcpyAsync` to
  multicast VA optimization. This only matters when
  `MCCL_HOSTRING_NVL_MULTIMEM=1`; with multimem disabled, the path is already
  unicast.
