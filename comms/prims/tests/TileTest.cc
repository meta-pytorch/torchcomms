...
void run_partial_accumulate_fp8_test(TileTestDataType dtype) {
  constexpr int kFp8TileElems = kTileTestByteTileElems;
  constexpr int kValidElems = kFp8TileElems - 13; // 4083, remainder 3 of 16
  ...
  std::vector<std::uint8_t> base_h(kFp8TileElems), addend_h(kFp8TileElems);
  for (int i = 0; i < kFp8TileElems; ++i) {
    base_h[i] = fromFloat(1.0f + static_cast<float>(i % 4) * 0.5f);
    addend_h[i] = fromFloat(0.5f + static_cast<float>(i % 3) * 0.5f);
  }

  DeviceBuffer baseBuf(kFp8TileElems), addendBuf(kFp8TileElems),
      outBuf(kFp8TileElems);
  auto* base_d = static_cast<std::uint8_t*>(baseBuf.get());
  auto* addend_d = static_cast<std::uint8_t*>(addendBuf.get());
  auto* out_d = static_cast<std::uint8_t*>(outBuf.get());
  CUDACHECK_TEST(
      cudaMemcpy(base_d, base_h.data(), kFp8TileElems, cudaMemcpyHostToDevice));
  CUDACHECK_TEST(cudaMemcpy(
      addend_d, addend_h.data(), kFp8TileElems, cudaMemcpyHostToDevice));

  for (TileTestReduceOp op :
       {TileTestReduceOp::kSum,
        TileTestReduceOp::kMax,
        TileTestReduceOp::kMin}) {
    CUDACHECK_TEST(cudaMemset(out_d, 0, kFp8TileElems));
    test_tile_partial_load_accumulate_dtype(
        dtype, op, base_d, addend_d, out_d, kValidElems);
    CUDACHECK_TEST(cudaGetLastError());
    CUDACHECK_TEST(cudaDeviceSynchronize());

    std::vector<std::uint8_t> out_h(kFp8TileElems);
    CUDACHECK_TEST(
        cudaMemcpy(out_h.data(), out_d, kFp8TileElems, cudaMemcpyDeviceToHost));
...
TEST_F(TileTestFixture, Fp8SpecialValuesSaturate) {
  ...
  for (const Case& c : cases) {
    std::vector<std::uint8_t> a_h(kFp8TileElems, c.a), b_h(kFp8TileElems, c.b);
    DeviceBuffer aBuf(kFp8TileElems), bBuf(kFp8TileElems),
        outBuf(kFp8TileElems);
    auto* a_d = static_cast<std::uint8_t*>(aBuf.get());
    auto* b_d = static_cast<std::uint8_t*>(bBuf.get());
    auto* out_d = static_cast<std::uint8_t*>(outBuf.get());
    CUDACHECK_TEST(
        cudaMemcpy(a_d, a_h.data(), kFp8TileElems, cudaMemcpyHostToDevice));
    CUDACHECK_TEST(
        cudaMemcpy(b_d, b_h.data(), kFp8TileElems, cudaMemcpyHostToDevice));
    ...
    CUDACHECK_TEST(
        cudaMemcpy(out_h.data(), out_d, kFp8TileElems, cudaMemcpyDeviceToHost));
    EXPECT_EQ(out_h[kFp8TileElems - 14], c.want) << c.what << " (scalar tail)";
  }
  ...
  std::vector<std::uint8_t> a_h(kFp8TileElems, kNaN), b_h(kFp8TileElems, kOne);
  DeviceBuffer aBuf(kFp8TileElems), bBuf(kFp8TileElems), outBuf(kFp8TileElems);
  auto* a_d = static_cast<std::uint8_t*>(aBuf.get());
  auto* b_d = static_cast<std::uint8_t*>(bBuf.get());
  auto* out_d = static_cast<std::uint8_t*>(outBuf.get());
  CUDACHECK_TEST(
