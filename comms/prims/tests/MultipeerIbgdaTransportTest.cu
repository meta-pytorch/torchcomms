...
struct IbgdaWarpProxyTestAccess {
  template <typename Proxy>
  __device__ static bool runServiceStep(
      typename Proxy::SharedState& sharedState,
      const ThreadGroup& fullBlock,
      WarpProxyRefusalStep step,
      const AbortDevice& abortDevice) {
    using ServiceStepResult = typename Proxy::ServiceStepResult;
    const ServiceStepResult result = step == WarpProxyRefusalStep::RecvReadiness
        ? Proxy::publish_recv_readiness(sharedState, abortDevice)
        : Proxy::post_send_once(sharedState, fullBlock, abortDevice);
    return result == ServiceStepResult::Stop;
  }
};
...
__device__ void initializeWarpProxyRefusalState(WarpProxyRefusalState* state) {
  ...
  // These types contain non-assignable spans, so construct them in the raw
  // cudaMalloc storage after their backing arrays and layout are initialized.
  ...
  state->layout.numProtocolSlots = kNumProtoSlots;
...
__global__ void warpProxyStepRefusalKernel(
  ...
  const bool directStep = step == WarpProxyRefusalStep::RecvReadiness ||
      step == WarpProxyRefusalStep::SendRecvCredit ||
      step == WarpProxyRefusalStep::SendSlotFree;
  if (directStep) {
    // Bypass run_service's leading poll and leave the handle unstarted so this
    // first check must report the pre-set abort from the step under test.
    __shared__ WarpProxyRefusalTest::SharedState sharedState;
    if (block.is_leader()) {
      sharedState.send.tail = 0;
      sharedState.send.posted = 0;
      sharedState.recv.tail = 0;
      sharedState.recv.ready = 0;
      sharedState.recv.copied = 0;
      sharedState.recv.credited = 0;
      if (step == WarpProxyRefusalStep::RecvReadiness) {
        sharedState.recv.commands[0] = {
            .transport = &state->transport,
            .protocolBytes = sizeof(state->source),
            .channel = 0,
        };
        sharedState.recv.tail = 1;
      } else {
        sharedState.send.commands[0] = {
            ...
            .protocolBytes = sizeof(state->source),
            .slotFreeExpected =
                step == WarpProxyRefusalStep::SendSlotFree ? 1U : 0U,
            .generation = 0,
            .requiredRecvCredit =
                step == WarpProxyRefusalStep::SendRecvCredit ? 1U : 0U,
            .channel = 0,
            .slot = 0,
        };
        sharedState.send.tail = 1;
      }
      const bool stepStopped =
          IbgdaWarpProxyTestAccess::runServiceStep<WarpProxyRefusalTest>(
              sharedState, block, step, abortDevice);
      state->result = WarpProxyRefusalResult{
          ...
          .recvReady = sharedState.recv.ready,
