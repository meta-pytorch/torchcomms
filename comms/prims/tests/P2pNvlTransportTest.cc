...
TEST_F(P2pNvlTransportTestFixture, ProgressSendUnwindsOnAbortWithSkipBehavior) {
  ...
  // is ever returned -- so reporting Done can only mean the poll observed the
  // abort and unwound, which is what the fault-tolerance contract requires of
  // a wait. Without abort the same kernel reports not-completed, so the two
  // tests bracket the behavior.
  const int peerRank = (globalRank == 0) ? 1 : 0;
  const size_t nbytes = 4 * 1024 * 1024;
  ...
  if (globalRank == 0) {
    ...
    EXPECT_EQ(counters.sendCompleted, 1)
        << "aborted send should report Done rather than spinning on credit "
...
TEST_F(P2pNvlTransportTestFixture, ProgressAbortThenReinitSameKernel) {
  ...
  // The abort path clears `stage` so the channel can be reused. Nothing
  // previously exercised abort and re-init together in one kernel, which is
...
TEST_F(P2pNvlTransportTestFixture, ProgressRecvUnwindsOnAbortWithSkipBehavior) {
  ...
  if (globalRank == 0) {
    EXPECT_EQ(counters.recvCompleted, 1)
        << "aborted recv should report Done rather than spinning";
  }
}
