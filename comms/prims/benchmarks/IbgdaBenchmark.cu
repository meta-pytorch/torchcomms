...
__global__ void ibgdaPutSignalWaitCounterKernel(
  ...
  if (group.is_global_leader()) {
    ...
    transport.put(
        localBuf, remoteBuf, nbytes, abortDevice, signalId, 1, counterId, 1);
    transport.wait_counter(counterId, 1, abortDevice);
  }
}
...
__global__ void ibgdaPutWaitCounterBatchKernel(
  ...
  if (group.is_global_leader()) {
    ...
    for (int i = 0; i < 10; i++) {
      transport.put(
          localBuf, remoteBuf, nbytes, abortDevice, -1, 0, counterId, 1);
      transport.wait_counter(counterId, expected, abortDevice);
      expected++;
    }
  ...
  if (group.is_global_leader()) {
    ...
    for (int i = 0; i < 10; i++) {
      const auto ticket = transport.put(
          localBuf, remoteBuf, nbytes, IbgdaRemoteBuffer{}, abortDevice, 0);
      transport.wait_local(solo, ticket, abortDevice);
    }
...
__global__ void ibgdaPutSignalFlushBatchKernel(
  ...
  if (group.is_global_leader()) {
    ...
    for (int i = 0; i < 10; i++) {
      transport.put(
          localBuf, remoteBuf, nbytes, resolvedSignalBuf, abortDevice, 1);
...
__global__ void ibgdaThreadScopeMultiBlockPutFlushBatchKernel(
  ...
  if (threadIdx.x == 0) {
    ...
    for (int i = 0; i < 10; i++) {
      transport->put(
          blockLocalBuf, blockRemoteBuf, nbytesPerBlock, abortDevice);
      transport->flush(abortDevice);
    }
...
__global__ void ibgdaMultiPeerSerialCounterFanOutBatchKernel(
  ...
  if (group.is_global_leader()) {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      for (int p = 0; p < numPeers; p++) {
        T(p).wait_counter(perPeerCounter(p), expected, abortDevice);
      }
      expected++;
    }
...
__global__ void ibgdaMultiPeerCounterFanOutBatchKernel(
  ...
  if (group.is_global_leader()) {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      T(0).wait_counter(resolvedCounterBuf, expected, abortDevice);
      expected += numPeers;
    }
