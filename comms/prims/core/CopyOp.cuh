...
// the per-piece header + 256-byte alignment padding) still fits
...
// fetch_and_reset_ans_compress_stats() call. Defined inline so the symbol
// has a single definition across the (sole) .cu translation unit that
// includes this header with PIPES_ENABLE_ANS_COMPRESSION (today:
// AllToAllvTileCompressed.cu via :alltoallv_tile_compressed_obj).
...
struct AnsCompress {
 public:
  // ANS produces a variable-length payload, so the transport must size the
  // RDMA-put off the on-chip header (`*staging[0..8) + 8`) instead of the
  // uncompressed chunk size. See P2pIbgdaTransportDevice::send().
...
#error "PIPES_ENABLE_ANS_COMPRESSION requires sm_80 or higher"
 ...
 public:
 private:
  // nvCOMPDx decompress input pointer alignment requirement. Each
  // per-piece compressed payload must start at a `kAnsInputAlignment`-
  // byte aligned offset within the staging slot so the cooperative
  // decompressor can vector-load it without faulting. Also used by
  // `AnsCompress::send` as the runtime `src` alignment guard (the
  // realign-via-alignedAuxBuf path kicks in when this check fails).
  static constexpr std::size_t kAnsInputAlignment = 256ULL;
  ...
  // under-report across all `NumWarps ∈ {1, 2, 4, 8, 16, 32, 64}`
 ...
 public:
   ...
   * stay `kAnsInputAlignment`-aligned via the `alignInputBufferSize`
   ...
   * (`alignInputBufferSize(max_comp_chunk_size())`) is materialized once
   * per template instantiation in `kWorstCompPaddedPerChunk` so the
  ...
  worst_case_chunk_stride(std::size_t chunkSize) {
    ...
    const std::size_t worstTotal =
        headerPadded + numChunks * kWorstCompPaddedPerChunk;
  ...
  max_safe_chunk_size_for_slot(std::size_t perBlockSlot) {
    ...
    std::size_t numChunks = perBlockSlot / kWorstCompPaddedPerChunk;
    if (numChunks == 0) {
 ...
 private:
  // Materialized worst-case per-piece padded payload size (compressed
  // worst-case payload, rounded up to `kNvcompInputAlignment` so that
  // chunk[i+1] starts at a 256-byte-aligned offset for nvCOMPDx's
  // decompress input-alignment requirement). Cached once per (NumWarps,
  // MaxUncompBytes, kSrcAligned, kAnsArch) instantiation. We use
  // `inline static const` rather than `constexpr` because nvcompdx's
  // `max_comp_chunk_size()` isn't guaranteed `constexpr` across versions;
  // `inline static const` still gives one-time-init across all TUs that
  // see this header, and the `__forceinline__` accessor above folds
  // away the lookup at the call site.
  inline static const std::size_t kWorstCompPaddedPerChunk = []() {
    const std::size_t worstCompPayload =
        AnsCompressorType().max_comp_chunk_size();
    return alignInputBufferSize(worstCompPayload);
  }();
 ...
 public:
  ...
  __device__ __forceinline__ static std::size_t send(
    ...
    const std::size_t headerBytes = numChunks * sizeof(std::size_t);
    // Padded header section so chunk[0] starts at a 256-byte aligned
    ...
    // captures the misalignment state for the whole loop.
    // `kAnsInputAlignment` is the class-level 16-byte constant.
    bool needRealign = false;
    if constexpr (!kSrcAligned) {
      needRealign = (reinterpret_cast<uintptr_t>(src) &
                     (kAnsInputAlignment - 1ULL)) != 0ULL;
      if (needRealign) {
    ...
    // Inner loop: compress each sub-piece in serial, advancing
    // writeOffset by the (256-byte-aligned) per-piece compressed size.
    // All threads in the cooperating group iterate identically — the
    // returned `writeOffset` is therefore the same on every thread.
    ...
    // column). All sizeHeaders[] writes are already visible to the
    // leader thanks to the group.sync() at the bottom of each loop
    // iteration above.
    if (group.is_leader()) {
...
#ifdef DEBUG
      if (group.is_leader()) {
        assert(__ans_outSize == pieceBytes);
      }
#else
      (void)pieceBytes;
#endif
  ...
  __device__ __forceinline__ static std::size_t recv_forward(
    ...
    if (dst != nullptr) {
      __shared__ size_t __ans_fwd_outSize;
