...
TEST_F(SendRecvTileBenchmarkFixture, IbSweepCompressed) {
  ...
  MultiPeerTransportConfig ib_config{
      ...
      .ibgdaConfig =
          {
              .cudaDevice = localRank,
              .dataBufferSize = 128ULL * 1024 * 1024,
              .sendRecv =
                  MultipeerIbgdaTransportConfig::SendRecvConfig{
                      // 256 to cover grid_blocks=256 on the one-way path
                      // (active_blocks == grid_blocks there); the two-way
                      // path needs only grid_blocks/2 but shares the config.
                      .maxGroups = 256,
                      .pipelineDepth = 2,
                  },
              .numQpsPerPeerPerNic = 32,
  ...
  const int grid_blocks = 256;

  constexpr std::size_t kCompressChunkBytes = 256 * 1024;
  constexpr int kNumWarpsPerBlock = 8; // blockDim.x / 32 = 512 / 32 = 16
      ...
      auto launch_one = [&](void** ka) {
        return comms::common::launchKernel(
            reinterpret_cast<void*>(
                sendrecv_tile_compressed_kernel<kNumWarpsPerBlock>),
            dim3(grid_blocks),
