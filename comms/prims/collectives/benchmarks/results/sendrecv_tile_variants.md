...
mpirun -np 2 -host <host>:2 --allow-run-as-root --map-by ppr:2:node \
    ...
    -x NCCL_SOCKET_IFNAME=eth1 --prefix <mpi-install-prefix> \
...
| `PIPES_SENDRECV_BENCH_DATA_BUFFER_MB` | IB staging buffer, in MiB |
| `PIPES_SENDRECV_BENCH_MAX_SIGNAL_BYTES` | signal granularity; 0 = auto-safe |

## Results

Recorded 2026-09-11, single node, 2x H100 80GB HBM3, mlx5 RoCE,
