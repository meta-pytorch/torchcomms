 ...
 * regions.
 *
 * Templated on `NumWarps = blockDim.x / 32`. Explicit instantiations
 * are emitted in `AnsCopyOpBench.cu` for `NumWarps ∈ {1, 2, 4, 8,
 * 16}` (blockDim.x ∈ {32, 64, 128, 256, 512}); extend the macro
 * block there if a wider range is needed.
 *
 * Launch requirements:
...
/**
 * Map a runtime-chosen `threads_per_block` (= `NumWarps * 32`) to
 * the matching explicit instantiation of the bench kernels in
 * `AnsCopyOpBench.cu`. Supported values: 32, 64, 128, 256, 512
 * (NumWarps ∈ {1, 2, 4, 8, 16}). Returns `{nullptr, nullptr}` for
 * unsupported values.
 */
