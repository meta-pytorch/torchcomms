...
TEST_F(AnsCopyOpBenchFixture, AnsCopyOpStandalone) {
  ...
  // `AnsCopyOpBench.cu`); other values are rejected at launch.
  ...
  const AnsCopyOpBenchKernels bench_kernels =
      pick_ans_copy_op_bench_kernels(kCopyOpThreads);
  ...
  for (int sparsityPct = 0; sparsityPct <= 100; sparsityPct += 10) {
    ...
    for (std::size_t inputBytes : sizes) {
      ...
      PIPES_CUDA_CHECK(cudaEventRecord(c_start.get()));
      PIPES_CUDA_CHECK(launch_compress(measureIters));
      PIPES_CUDA_CHECK(cudaEventRecord(c_stop.get()));
      ...
      PIPES_CUDA_CHECK(cudaEventRecord(d_start.get()));
      PIPES_CUDA_CHECK(launch_decompress(measureIters));
      PIPES_CUDA_CHECK(cudaEventRecord(d_stop.get()));
