...
std::string describe_max_signal(
  ...
  if (requested == 0) {
    ...
    if (pipelineDepth <= 0) {
      os << " -- slot unknown (pipelineDepth=" << pipelineDepth << ")";
    } else if (requested > perBlockSlot) {
...
SplitGreenCtx makeSplitGreenCtxStreams(int totalSms) {
  ...
  PIPES_CU_CHECK(cuDevSmResourceSplitByCount(
      groups, &nGroups, &sm, &remaining, /*flags=*/0, perPartition));
  if (nGroups < 2) {
    COMMS_LOG_STREAM(FATAL)
      ...
      .ibConfig =
          {
              ...
              // counters, which are posted on the companion QP.
...
TEST_F(SendRecvTileVariantsBenchmarkFixture, IbSweepCompressed) {
  ...
  // The sweep below indexes this one-per-size, so pin the invariant here rather
  // than letting a short vector turn into an out-of-bounds read per sparsity.
  ASSERT_EQ(nccl_results.size(), sizes.size());
  ...
  for (int sparsityPct = 0; sparsityPct <= 100; sparsityPct += 10) {
    ...
    std::size_t bppIdx = 0;
    for (std::size_t bytes : sizes) {
      auto nccl_r = nccl_results.at(bppIdx++);
...
TEST_F(SendRecvTileVariantsBenchmarkFixture, IbSweepCompressedTwoWay) {
  ...
  // caller-supplied PIPES_SENDRECV_BENCH_DATA_BUFFER_MB is too small.
  ...
  //
  // The helper cannot simply be called from here: it lives in
  // `core/CopyOp.cuh`, which only parses with PIPES_ENABLE_ANS_COMPRESSION and
  // __CUDACC__, and this is a host `.cc` TU compiled by clang.
  // `ans_worst_case_chunk_stride()` is the thin nvcc TU that re-exports the
  // value behind a plain host signature.
  const std::size_t kWorstCaseChunk = ans_worst_case_chunk_stride(
  ...
  }
  {
    ...
    if (globalRank == 0) {
      COMMS_LOG_STREAM(INFO)
          << "ANS worst-case chunk stride (num_warps=" << kNumWarpsPerBlock
          << "): " << kWorstCaseChunk << " bytes ("
    ...
    ASSERT_GE(slot, kWorstCaseChunk)
        ...
        << "PIPES_SENDRECV_BENCH_DATA_BUFFER_MB to at least "
        << ((kMinDataBuf + 1024ULL * 1024ULL - 1) / (1024ULL * 1024ULL))
        << " (MiB)";
  }
  MultiPeerTransportConfig ib_config{
      .nvlConfig =
  ...
  // Same one-per-size invariant as the one-way sweep.
  ASSERT_EQ(nccl_results.size(), sizes.size());
