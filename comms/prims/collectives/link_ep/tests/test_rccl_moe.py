...
Run with the same wrapper as the intranode test:
  HSA_NO_SCRATCH_RECLAIM=1 NCCL_NET=Socket NCCL_SOCKET_IFNAME=lo \\
    ...
    //comms/prims/collectives/link_ep/tests:test_rccl_moe -- --num-processes 8
"""
...
sys.modules["deep_ep"] = _link_ep
sys.modules["deep_ep_cpp"] = _link_ep_cpp

# Bind to the in-house link_ep module (sys.modules shim above) — no import of
# a separate deep_ep package.
deep_ep = _link_ep
import torch  # noqa: E402
import torch.distributed as dist  # noqa: E402
from comms.prims.collectives.link_ep.tests.common_utils import (  # noqa: E402  # noqa: E402
    bench,
    init_dist,
    inplace_unique,
    use_rocm,
...
def bench_e2e_breakdown(
    ...
    INCLUDES the topk-derived pre-computation, for both our link_ep kernel and
    ...
    The existing per-iteration numbers (cached link_ep dispatch / RCCL with
    ...
    link_ep breakdown (uses cached-vs-non-cached differencing to isolate the
    ...
    if local_rank == 0:
        ...
        # With --tune, report the link_ep data legs at the best-of-sweep config
        ...
        print(f"---- link_ep (NVLink IPC){cfg_note} ----", flush=True)
        ...
        print(f"  >> link_ep e2e total         : {u(me_e2e_show):9.1f}", flush=True)
        print(
        ...
        print(
            f"  e2e speedup (RCCL/link_ep)   : {rccl_e2e / me_e2e_show:.2f}x",
            flush=True,
        )
        print(
        ...
        print(f"  {'leg':<16}{'link_ep':>10}{'RCCL':>10}", flush=True)
        print(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="breakdown",
        help="breakdown: Benchmark B e2e link_ep-vs-RCCL with per-part timing "
        "(includes pre-computation). rccl-only: legacy RCCL data-move bench.",
    )
    parser.add_argument(
        "--tune",
        action="store_true",
        help="Also sweep nvl_chunk_size for the link_ep data legs and print a "
        "config x precompute 2x2 of the speedup (same RCCL baseline). Shows that "
        "including pre-computation RAISES link_ep's lead at fixed config.",
    )
