...
# Alias in-tree modules before any `import link_ep` / `import link_ep_cpp`.
...
link_ep = _link_ep  # noqa: F401
import torch
import torch.distributed as dist
from comms.prims.collectives.link_ep.tests.common_utils import (  # noqa: F401
    bench,
    calc_diff,
    init_dist,
    per_token_cast_back,
...
def bench_kineto(*_args, **_kwargs):
    if _kwargs.get("num_kernels_per_period", 1) > 1:
        return ((0.001, 0.001), (0.001, 0.001))
    return (0.001, 0.001)


def hash_tensor(t: torch.Tensor) -> int:
    return int(t.flatten().to(torch.int64).sum().item())
...
def test_main(
    ...
    for current_x in x_list:
        for return_recv_hook in (False, True):
            for dispatch_use_fp8 in (False,):  # FP8 not yet supported in LL kernel
