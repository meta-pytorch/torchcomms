...
# Intranode EP dispatch/combine test. The link_ep adaptations are: the
# `link_ep`/`link_ep_cpp` import shims (this module runs directly as
...
# Install the import shims BEFORE the first `import link_ep` / `import link_ep_cpp`
# so the test body below runs unchanged.
import comms.prims.collectives.link_ep._cpp as _link_ep_cpp  # noqa: E402
import comms.prims.collectives.link_ep.link_ep as _link_ep  # noqa: E402

sys.modules["link_ep"] = _link_ep
sys.modules["link_ep_cpp"] = _link_ep_cpp

# Bind `link_ep` / `link_ep_cpp` to the in-house link_ep modules (installed in
# sys.modules above) so the test body runs against our implementation
# without importing a separate link_ep package.
link_ep = _link_ep
link_ep_cpp = _link_ep_cpp
import torch  # noqa: E402
import torch.distributed as dist  # noqa: E402
from comms.prims.collectives.link_ep.tests.common_utils import (  # noqa: E402
    bench,
    calc_diff,
    init_dist,
    inplace_unique,
...
def test_main(
    ...
    x_e4m3 = (
        ...
        if (use_rocm or link_ep.Buffer.is_sm90_compiled())
        else None
    )
    x_e4m3 = (x_e4m3[0], x_e4m3[1].T.contiguous().T) if x_e4m3 is not None else None
    x_e4m3 = None  # link_ep: FP8 dispatch path not yet wired in the kernel (num_scales=0); skip FP8 cases.
    scores = (
        torch.randn((num_tokens, num_experts), dtype=torch.float32, device="cuda").abs()
        + 1
    )
    topk_idx = torch.topk(scores, num_topk, dim=-1, largest=True, sorted=False)[1]
    topk_idx = topk_idx.to(link_ep.topk_idx_t)
    topk_weights = (
    ...
    # Config
    nvl_buffer_size = 256
    config = link_ep.Config(num_sms, 8, nvl_buffer_size)
    ...
    for current_x in filter(lambda elem: elem is not None, (x_e4m3, x)):
        ...
        for nvl_chunk_size in tuple(range(4, 33, 2)) + (0,):
            if nvl_chunk_size > 0:
                config = link_ep.Config(num_sms, nvl_chunk_size, nvl_buffer_size)
            else:
                # Test default config as well
                link_ep.Buffer.set_num_sms(num_sms)
                config = link_ep.Buffer.get_dispatch_config(num_ranks)
    ...
    dispatch_config = link_ep.Config(
    ...
    for nvl_chunk_size in tuple(range(1, 17, 1)) + (0,):
        if nvl_chunk_size > 0:
            ...
            # Test default config as well
            link_ep.Buffer.set_num_sms(num_sms)
            config = link_ep.Buffer.get_combine_config(num_ranks)
...
def test_loop(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    if test_ll_compatibility:
        ...
        num_rdma_bytes = link_ep.Buffer.get_low_latency_rdma_size_hint(
    ...
    if test_ll_compatibility:
        ...
        from comms.prims.collectives.link_ep.tests import test_low_latency
