...
// UNROLLED_WARP_COPY + system-scope atomics, matching the prims NVLink
      ...
      if (dst_expert_idx >= 0) {
        ...
        if (buffer_ptrs[dst_rank] != nullptr) {
          ...
          if (device_transport != nullptr && local_rdma_x_buf != nullptr &&
            ...
            if (lane_id == 0) {
              printf(
                  "link_ep LL dispatch: cross-node peer with no IBGDA "
        ...
        // VMEM stores still in-flight (the IPC `LINK_EP_UNROLLED_WARP_COPY`
    ...
    if (sub_warp_id == 0 && lane_id == 0) {
      ...
      while ((recv_val = *slot_ptr) == 0) {
        ...
        if (elapsed > NUM_TIMEOUT_CYCLES) {
          printf(
              "link_ep LL dispatch recv timeout rank=%d expert=%d "
