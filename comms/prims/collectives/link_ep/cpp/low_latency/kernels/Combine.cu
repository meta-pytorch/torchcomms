...
// with direct peer-mapped IPC buffer writes via prims NVLink primitives.
  ...
  if (responsible_expert_idx < num_experts) {
    ...
    for (int token_idx = offset + sub_warp_id;
      ...
      if (buffer_ptrs[dst_rank] != nullptr) {
        ...
        if (lane_id == 0) {
          printf(
              "link_ep LL combine: cross-node peer with no IBGDA transport "
              "(rank=%d, dst_rank=%d).\n",
              rank,
              dst_rank);
          trap_kernel();
    ...
    if (sub_warp_id == 0 && lane_id == 0) {
      ...
      while (*reinterpret_cast<volatile int64_t*>(
        ...
        if (wall_clock64_compat() - start > NUM_TIMEOUT_CYCLES) {
          printf(
              "link_ep LL combine recv timeout rank=%d expert=%d\n",
              rank,
