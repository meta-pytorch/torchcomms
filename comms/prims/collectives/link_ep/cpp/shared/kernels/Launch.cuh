...
//     cooperative-launch attributes. We don't use cooperative launches on the
//     intranode path (notify_dispatch is the only place
//     that touches `task_fifo_ptrs`, and we replace that with prims
//     `barrier_all` which doesn't require cooperative launch).
//
//  2) `SWITCH_RANKS` is restricted to the values the intranode kernels
//     instantiate (2 / 4 / 8); the internode path extends this with
//     rdma-rank variants.

namespace comms::prims::link_ep::kernels {

#ifdef __HIP_PLATFORM_AMD__
#define LINK_EP_GPU_R_16BF HIP_R_16BF
#define LINK_EP_GPU_R_32F HIP_R_32F
using gpu_bfloat16_t = hip_bfloat16;
#else
#define LINK_EP_GPU_R_16BF CUDA_R_16BF
#define LINK_EP_GPU_R_32F CUDA_R_32F
...
#define SETUP_LAUNCH_CONFIG(num_sms, num_threads, stream) \
  ::comms::prims::link_ep::kernels::LaunchConfig cfg = {  \
...
#define SWITCH_TYPES(case_macro)                                    \
  switch (type) {                                                   \
    case LINK_EP_GPU_R_16BF:                                        \
      case_macro(::comms::prims::link_ep::kernels::gpu_bfloat16_t); \
      break;                                                        \
    case LINK_EP_GPU_R_32F:                                         \
      case_macro(float);                                            \
      break;                                                        \
    default:                                                        \
