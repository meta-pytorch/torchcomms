    ...
    for (int64_t token_idx = token_start_idx; token_idx < token_end_idx;) {
      ...
      while (lane_id == 0) {
        ...
        if (elapsed > NUM_TIMEOUT_CYCLES) {
          printf(
              "link_ep combine sender timeout, rank %d channel %d\n",
              rank,
              responsible_channel);
          trap_kernel();
        }
...
#pragma unroll
      ...
      for (int64_t token_idx = token_start_idx + recv_warp_id - 1;
        ...
        while (true) {
          ...
          if (elapsed > NUM_TIMEOUT_CYCLES) {
            printf(
                "link_ep combine receiver timeout rank %d channel %d expect %d\n",
                rank,
                responsible_channel,
                expected_head);
            trap_kernel();
...
void intranode_combine(
  ...
  // Only bf16 is supported today; FP32 + the DType switch land in a
  // follow-up patch.
