...
#include "comms/prims/collectives/link_ep/cpp/shared/kernels/EpBuffer.cuh"
...
#include "comms/prims/collectives/link_ep/cpp/shared/kernels/KernelUtils.cuh"
#include "comms/prims/collectives/link_ep/cpp/shared/kernels/Launch.cuh"
...
//     `LINK_EP_UNROLLED_WARP_COPY` (NVLink → IPC-mapped peer pointer).
    ...
    for (int64_t token_idx = token_start_idx; token_idx < token_end_idx;) {
      ...
      while (send_lane_id == 0) {
        ...
        if (elapsed > NUM_TIMEOUT_CYCLES) {
          printf(
              "link_ep dispatch sender timeout, rank %d, channel %d\n",
              rank,
              responsible_channel);
          trap_kernel();
        }
...
#pragma unroll
    ...
    while (recv_lane_id == 0 &&
      ...
      if ((now > offset_wait_start ? now - offset_wait_start : 0) >
          NUM_TIMEOUT_CYCLES) {
        printf(
            "link_ep dispatch receiver start-offset timeout rank %d channel %d\n",
            rank,
            responsible_channel);
        trap_kernel();
      }
    ...
    while (recv_lane_id == 0 &&
      ...
      if ((now > offset_wait_start ? now - offset_wait_start : 0) >
          NUM_TIMEOUT_CYCLES) {
        printf(
            "link_ep dispatch receiver end-offset timeout rank %d channel %d\n",
            rank,
            responsible_channel);
        trap_kernel();
      }
    ...
    while (num_tokens_to_recv > 0) {
      while (recv_thread_id_in_rank == 0) {
        ...
        if (elapsed > NUM_TIMEOUT_CYCLES) {
          printf(
              "link_ep dispatch receiver timeout rank %d channel %d remain %d\n",
              rank,
              responsible_channel,
              num_tokens_to_recv);
          trap_kernel();
