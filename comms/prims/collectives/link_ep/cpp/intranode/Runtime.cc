...
IntranodeRuntime::IntranodeRuntime(
  ...
  // 2. Prims NVL transport — per-peer P2P state, signal slots, barrier
  ...
  comms::prims::MultiPeerNvlTransportConfig nvlConfig{
      ...
      // numSignalSlots is dimensioned by callers; for intranode dispatch +
  ...
  for (int i = 0; i < numRanks_; ++i) {
    ...
    // FIFO barrier slots. The dispatch / combine kernels start their own
    // region after `kNumRanks * kNumRanks * sizeof(int)`, so reusing the same
    // region is a TODO for production multi-rank: the current layout
    // colocates barrier slots with the rank prefix matrix because
