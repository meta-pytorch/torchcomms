...
class SendRecvTileTestFixture : public MpiBaseTestFixture {
 protected:
  ...
  // IB staging geometry. Named because the signal-bytes tests below derive the
  // per-block slot from these; a literal in both places could drift and turn
  // the clamp control into a test that asserts nothing.
  // Warp count of the compressed kernel these tests launch:
  // `compressed_kernel()` picks the 512-thread shape, and the encoded-size
  // query below must be asked about that same shape.
  static constexpr int kNumWarps = 512 / 32;

  static constexpr std::size_t kIbPerChannelSize = 1024 * 1024;
  static constexpr int kIbPipelineDepth = 2;

  // What `P2pIbgdaTransportDevice::pipeline_chunk()` computes: the per-channel
  // window divided by the pipeline depth, floored to the 512 B NIC burst.
  static constexpr std::size_t ib_per_block_slot() {
    return (kIbPerChannelSize / static_cast<std::size_t>(kIbPipelineDepth)) &
        ~511ULL;
  }
        ...
        .ibConfig =
            {
                ...
                .perChannelSize = kIbPerChannelSize,
                .max_num_channels = 32,
                .pipelineDepth = kIbPipelineDepth,
                .maxGroups = 32,
                // The tile transport signals completion through NIC_DONE local
                // counters, which are posted on the companion QP. Ignored by
                // IBRC, so it is safe under either ibMode.
  ...
  void run_sendrecv_tile_compressed_twoway_test(
    ...
    // As in the one-way helper: a byte-exact round trip does not prove the
    // compressed path ran, because the plain Memcpy fallback is byte-exact
    // too. Every rank sends here, so unlike the one-way case there is no
    // send-side guard -- each rank must see its own compressor traffic.
    const SendRecvAnsCompressStats stats =
        fetch_and_reset_sendrecv_ans_compress_stats(nullptr);
    if (plain_block_fraction < 1.0f) {
      EXPECT_GT(stats.uncompressed_bytes, 0ULL)
          << "no bytes went through the ANS compressor -- the bidirectional "
             "compressed path did not run (plain_block_fraction="
          << plain_block_fraction << ")";
      EXPECT_GT(stats.compressed_bytes, 0ULL);
    } else {
      EXPECT_EQ(stats.uncompressed_bytes, 0ULL)
...
TEST_F(SendRecvTileTestFixture, IbCompressedClampsUnsafeSignalBytes) {
  constexpr std::size_t kRequested = 384 * 1024;
  // The premise of this case: 384 KiB is under the slot, so the transport's own
  // `value < perBlockSlot` check passes, but its ENCODED form is not -- which
  // is exactly the gap that used to trap. Assert that here, otherwise this is
  // indistinguishable from the safe case below and neither test proves
  // anything.
  EXPECT_GT(
      ans_worst_case_chunk_stride(kNumWarps, kRequested), ib_per_block_slot())
      << "384 KiB no longer exceeds the slot once encoded; this case is no "
         "longer exercising the clamp";
  EXPECT_LT(kRequested, ib_per_block_slot());
  run_sendrecv_tile_compressed_test(
      4 * 1024 * 1024,
      16,
      /*plain_block_fraction=*/0.0f,
      /*max_signal_bytes=*/kRequested);
}

TEST_F(SendRecvTileTestFixture, IbCompressedHonoursSafeSignalBytes) {
  constexpr std::size_t kRequested = 256 * 1024;
  // The control for the case above: same shape, but this value's encoded form
  // DOES fit the slot, so it must be honoured rather than clamped.
  EXPECT_LE(
      ans_worst_case_chunk_stride(kNumWarps, kRequested), ib_per_block_slot())
      << "256 KiB no longer fits the slot once encoded; this case is no longer "
         "a control for the clamp";
  run_sendrecv_tile_compressed_test(
      4 * 1024 * 1024,
      16,
      /*plain_block_fraction=*/0.0f,
