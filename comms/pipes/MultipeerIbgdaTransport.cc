...
constexpr int kBufferExchTag = 0x5678;

// Sysfs paths for device discovery
constexpr std::string_view kSysClassInfiniband = "/sys/class/infiniband/";
constexpr std::string_view kSysBusPciDevices = "/sys/bus/pci/devices/";
...
std::string getCudaPciBusId(int cudaDevice) {
  ...
  cudaError_t err =
      cudaDeviceGetPCIBusId(busId.data(), busId.size(), cudaDevice);
  if (err != cudaSuccess) {
...
int getNumaNodeForPcie(const std::string& pciBusId) {
  auto numaPath = fmt::format("{}{}/numa_node", kSysBusPciDevices, pciBusId);
...
int getNumaNodeForIbDev(std::string_view devName) {
  auto numaPath =
      fmt::format("{}{}/device/numa_node", kSysClassInfiniband, devName);
  std::ifstream numaFile(numaPath);
  if (!numaFile.is_open()) {
    return -1;
  }
...
void MultipeerIbgdaTransport::exchange() {
  ...
  // Build local exchange info (only signal buffer in initial exchange)
  IbgdaExchInfo localInfo{};
  localInfo.signal.addr = reinterpret_cast<uint64_t>(signalBuffer_);
  localInfo.signal.rkey = HostRKey(signalMr_->rkey);
  memcpy(
      localInfo.transport.gid, localGid_.raw, sizeof(localInfo.transport.gid));
  localInfo.transport.gidIndex = gidIndex_;

  // Query port for LID (IB only)
  ibv_port_attr portAttr{};
  ibv_query_port(ibvContext_, 1, &portAttr);
  localInfo.transport.lid = portAttr.lid;
  ...
  for (int peerIndex = 0; peerIndex < numPeers; peerIndex++) {
    ...
    localInfo.transport.qpn = doca_verbs_qp_get_qpn(qpHlList_[peerIndex]->qp);

    LOG(INFO) << "MultipeerIbgdaTransport: rank " << myRank_
              << " exchanging with peer " << peerRank << " (index " << peerIndex
              << ")"
              << " local qpn=" << localInfo.transport.qpn;
    ...
    if (myRank_ < peerRank) {
      bootstrap_->send(&localInfo, sizeof(localInfo), peerRank, tag).get();
      bootstrap_
          ->recv(
              &peerExchInfo_[peerIndex], sizeof(IbgdaExchInfo), peerRank, tag)
          .get();
    } else {
      bootstrap_
          ->recv(
...
std::vector<IbgdaRemoteBuffer> MultipeerIbgdaTransport::exchangeBuffer(
  ...
  // Build local buffer info for exchange (using shared IbgdaBufferExchInfo)

  IbgdaBufferExchInfo localInfo{
      reinterpret_cast<uint64_t>(localBuf.ptr),
      HostRKey(mr->rkey),
  };

  std::vector<IbgdaBufferExchInfo> peerInfo(numPeers);

  // Exchange with all peers
  for (int peerIndex = 0; peerIndex < numPeers; peerIndex++) {
    int peerRank = peerIndexToRank(peerIndex);
    int tag = kBufferExchTag + peerIndex;

    if (myRank_ < peerRank) {
      bootstrap_->send(&localInfo, sizeof(localInfo), peerRank, tag).get();
      bootstrap_
          ->recv(&peerInfo[peerIndex], sizeof(IbgdaBufferExchInfo), peerRank, tag)
          .get();
    } else {
      bootstrap_
          ->recv(&peerInfo[peerIndex], sizeof(IbgdaBufferExchInfo), peerRank, tag)
  ...
  // Convert to IbgdaRemoteBuffer vector using toRemoteBuffer()
  std::vector<IbgdaRemoteBuffer> result(numPeers);
  for (int i = 0; i < numPeers; i++) {
    result[i] = peerInfo[i].toRemoteBuffer();
  }
