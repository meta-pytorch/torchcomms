...
MultiPeerNvlTransport::MultiPeerNvlTransport(
  ...
  perPeerSignalBufferSize_ = config_.signalCount * sizeof(Signal);
  ...
  signalBufferHandler_ = std::make_unique<meta::comms::IpcMemHandler>(
      mpiBootstrap_, myRank, nRanks_);
  ...
  auto signalPtr = static_cast<Signal*>(signalBuffer_d_->get());
  std::vector<Signal> signalInitStates(config_.signalCount * (nRanks_ - 1));
  cudaErr = cudaMemcpy(
      signalPtr,
...
P2pNvlTransportDevice MultiPeerNvlTransport::getP2pTransportDevice(
  ...
  auto* localSignalPtr = static_cast<Signal*>(signalBuffer_d_->get());
  ...
  LocalState localState{
      ...
      .stateBuffer = DeviceSpan<ChunkState>(
          ...
          numChunksPerPeer),
      .signalBuffer = DeviceSpan<Signal>(
          reinterpret_cast<Signal*>(localSignalPtr + localSignalBufferOffset),
          config_.signalCount),
  };
  ...
  auto* remoteSignalPtr =
      static_cast<Signal*>(signalBufferHandler_->getPeerDeviceMemPtr(peerRank));
  ...
  RemoteState remoteState{
      ...
      .stateBuffer = DeviceSpan<ChunkState>(
          ...
          numChunksPerPeer),
      .signalBuffer = DeviceSpan<Signal>(
          reinterpret_cast<Signal*>(remoteSignalPtr + remoteSignalBufferOffset),
          config_.signalCount),
  };
