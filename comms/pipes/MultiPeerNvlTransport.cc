...
MultiPeerNvlTransport::MultiPeerNvlTransport(
  ...
  const std::size_t totalSignalStateBufferSize =
      sizeof(SignalState) * (nRanks_ - 1);
  ...
  chunkStateBufferHandler_ = std::make_unique<meta::comms::IpcMemHandler>(
      mpiBootstrap_, myRank, nRanks_);
  chunkStateBufferHandler_->addSelfDeviceMemPtr(chunkStateBuffer_d_->get());

  signalStateBuffer_d_ =
      std::make_unique<meta::comms::DeviceBuffer>(totalSignalStateBufferSize);
  signalStateBufferHandler_ = std::make_unique<meta::comms::IpcMemHandler>(
      mpiBootstrap_, myRank, nRanks_);
  signalStateBufferHandler_->addSelfDeviceMemPtr(signalStateBuffer_d_->get());
  ...
  auto signalStatePtr = static_cast<SignalState*>(signalStateBuffer_d_->get());
  std::vector<SignalState> signalInitStates(nRanks - 1);
  cudaErr = cudaMemcpy(
      signalStatePtr,
      signalInitStates.data(),
      totalSignalStateBufferSize,
      cudaMemcpyDefault);
  if (cudaErr != cudaSuccess) {
    throw std::runtime_error(
...
void MultiPeerNvlTransport::exchange() {
  ...
  signalStateBufferHandler_->exchangeMemPtrs();
}
...
P2pNvlTransportDevice MultiPeerNvlTransport::getP2pTransportDevice(
  ...
  auto* localSignalStatePtr =
      static_cast<SignalState*>(signalStateBuffer_d_->get());
  ...
  LocalState localState{
      ...
      .stateBuffer = DeviceSpan<ChunkState>(
          ...
          numChunksPerPeer),
      .signalState = localSignalStatePtr + localPeerIndex,
  };
  ...
  auto* remoteSignalStatePtr = static_cast<SignalState*>(
      signalStateBufferHandler_->getPeerDeviceMemPtr(peerRank));

  RemoteState remoteState{
      .dataBuffer = remoteDataPtr + remoteDataBufferOffset,
      .stateBuffer = DeviceSpan<ChunkState>(
          reinterpret_cast<ChunkState*>(
              remoteChunkStatePtr + remoteStateBufferOffset),
          numChunksPerPeer),
      .signalState = remoteSignalStatePtr + remotePeerIndex,
  };
