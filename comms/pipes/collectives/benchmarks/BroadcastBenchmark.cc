...
TEST_F(BroadcastBenchmarkFixture, OptimalConfigs) {
  ...
  // 4MB with 16 blocks, chunkSize = 32KB (256 warps, 128 chunks/step = 50%)
  // Ring topology limits small message performance to ~0.70x NCCL.
  ...
  // === 64-block optimized (1 chunk/warp, 16MB buffer) ===
  // Confirmed winner across all sizes ≥64MB. 1024 warps, 1024 chunks = 1/warp.
  ...
  // === 64-block with pipeline depth 4 (A/B test vs depth 2) ===
