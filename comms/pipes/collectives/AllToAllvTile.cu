...
// NVL 2D: partition across all peers first (including self), then split
// send/recv within each peer's group. Self gets all its blocks for memcpy;
// remote peers split their blocks into send/recv halves.
__device__ __forceinline__ void nvl_2d_path(
  ...
  auto [peerIdx, peerGroup] = transportGroup.partition(nRanks);

  if (peerIdx == myRank) {
    std::size_t bytes = args.send_counts[myRank];
    if (bytes > 0) {
      TiledBuffer<char> sendTile(args.send_ptrs[myRank], bytes, peerGroup);
      TiledBuffer<char> recvTile(args.recv_ptrs[myRank], bytes, peerGroup);
      memcpy_vectorized(
          recvTile.data(), sendTile.data(), sendTile.bytes(), peerGroup);
    }
    return;
  }

  auto [role, halfGroup] = peerGroup.partition_interleaved(2);
  const int tileIdx = halfGroup.group_id;
  const int blocksPerPeer = halfGroup.total_groups;

  if (role == 0) {
    std::size_t sendBytes = args.send_counts[peerIdx];
    if (sendBytes > 0) {
      TiledBuffer<char> tiles(
          args.send_ptrs[peerIdx], sendBytes, blocksPerPeer);
      send_to_peer(
          handle,
          peerIdx,
          halfGroup,
    ...
    std::size_t recvBytes = args.recv_counts[peerIdx];
    if (recvBytes > 0) {
      TiledBuffer<char> tiles(
          args.recv_ptrs[peerIdx], recvBytes, blocksPerPeer);
      recv_from_peer(
          handle,
          peerIdx,
          halfGroup,
