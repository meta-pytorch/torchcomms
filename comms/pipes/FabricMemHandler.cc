...
#include "comms/pipes/FabricMemHandler.h"
...
void GpuMemHandler::cleanupCudaIpc() {
  ...
  for (int32_t rank = 0; rank < nRanks_; ++rank) {
    ...
    if (cudaIpcPeerPtrs_[rank] != nullptr) {
      // Ignore errors in destructor - nothing we can do
      (void)cudaIpcCloseMemHandle(cudaIpcPeerPtrs_[rank]);
  ...
  if (cudaIpcLocalPtr_ != nullptr) {
    // Ignore errors in destructor - nothing we can do
    (void)cudaFree(cudaIpcLocalPtr_);
