...
TEST_F(P2pIbgdaTransportDistributedTest, BidirectionalTransfer) {
  // Both ranks exchange data with each other
  // Serialized to avoid race condition where both read and write
  // to the same buffer simultaneously

  constexpr int kSignalId0 = 0; // Rank 0 -> Rank 1
  constexpr int kSignalId1 = 1; // Rank 1 -> Rank 0
  constexpr uint64_t kSignalVal = 1;
  constexpr uint8_t kPattern0 = 0xAA; // Rank 0's data pattern
  constexpr uint8_t kPattern1 = 0xBB; // Rank 1's data pattern
  const size_t nbytes = dataBufferSize_;

  resetSignalBuffers();

  uint8_t myPattern = (globalRank == 0) ? kPattern0 : kPattern1;

  // Fill local data with our pattern
  runFillDataKernel(dataBuffer_, nbytes, myPattern);
  syncRanks();

  // Phase 1: Rank 0 sends to Rank 1
  if (globalRank == 0) {
    XLOGF(INFO, "Rank 0: Sending {} bytes to Rank 1", nbytes);
    runPutSignalKernel(
        deviceTransport_,
        localDataBuf_,
        remoteDataBuf_,
    ...
    // Rank 1 waits for data from Rank 0
    bool* d_success = nullptr;
    CUDACHECK_TEST(cudaMalloc(&d_success, sizeof(bool)));
    CUDACHECK_TEST(cudaMemset(d_success, 0, sizeof(bool)));

    XLOG(INFO) << "Rank 1: Waiting for data from Rank 0...";
    runWaitSignalKernel(deviceTransport_, kSignalId0, kSignalVal, d_success);
    XLOG(INFO) << "Rank 1: Received data from Rank 0";

    // Verify received data
    runVerifyDataKernel(dataBuffer_, nbytes, kPattern0, d_success);
    ...
    CUDACHECK_TEST(cudaFree(d_success));
    EXPECT_TRUE(success)
        << "Rank 1: Data verification failed for Rank 0's data";
  }

  // Sync before phase 2
  syncRanks();

  // Reset signal buffers for phase 2
  resetSignalBuffers();

  // Re-fill Rank 1's buffer with its pattern (it was overwritten in phase 1)
  if (globalRank == 1) {
    runFillDataKernel(dataBuffer_, nbytes, myPattern);
  }
  syncRanks();
  ...
  if (globalRank == 1) {
    XLOGF(INFO, "Rank 1: Sending {} bytes to Rank 0", nbytes);
    runPutSignalKernel(
        deviceTransport_,
        localDataBuf_,
        remoteDataBuf_,
        ...
        kSignalId1,
        kSignalVal);
    XLOG(INFO) << "Rank 1: put_signal complete";
  } else {
    // Rank 0 waits for data from Rank 1
    bool* d_success = nullptr;
    CUDACHECK_TEST(cudaMalloc(&d_success, sizeof(bool)));
    CUDACHECK_TEST(cudaMemset(d_success, 0, sizeof(bool)));

    XLOG(INFO) << "Rank 0: Waiting for data from Rank 1...";
    runWaitSignalKernel(deviceTransport_, kSignalId1, kSignalVal, d_success);
    XLOG(INFO) << "Rank 0: Received data from Rank 1";

    // Verify received data
    runVerifyDataKernel(dataBuffer_, nbytes, kPattern1, d_success);
    ...
    CUDACHECK_TEST(cudaFree(d_success));
    EXPECT_TRUE(success)
        << "Rank 0: Data verification failed for Rank 1's data";
  }
