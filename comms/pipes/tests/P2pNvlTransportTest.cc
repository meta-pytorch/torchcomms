...
void runWriteTest(
  ...
  if (globalRank == 0) {
    ...
    // Device barrier: wait for rank 1 to be ready
    test::testBarrier(p2p, numBlocks, blockSize);

    // Write data to peer's buffer
    test::testWrite(p2p, remoteDst, localSrc, nbytes, numBlocks, blockSize);

    // Signal peer we're done writing
    test::testBarrier(p2p, numBlocks, blockSize);
    ...
    // Device barrier: signal rank 0 we're ready
    test::testBarrier(p2p, numBlocks, blockSize);

    // Device barrier: wait for rank 0's write to complete
    test::testBarrier(p2p, numBlocks, blockSize);
    CUDACHECK_TEST(cudaDeviceSynchronize());
