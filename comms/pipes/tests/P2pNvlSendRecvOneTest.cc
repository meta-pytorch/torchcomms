...
TEST_P(P2pNvlSendRecvOneSingleCallTest, SendRecvOne) {
  ...
  const size_t totalChunks = params.nbytes ? 1 : 0; // 1 chunk if nbytes > 0
  ...
  DeviceBuffer totalChunksBuffer(sizeof(size_t));
  ...
  auto totalChunks_d = static_cast<size_t*>(totalChunksBuffer.get());
  ...
  if (globalRank == 0) {
    ...
    XLOGF(
        INFO,
        "Rank {}: Received metadata - total_chunks={}, offset={}, nbytes={}",
        globalRank,
        receivedTotalChunks,
        receivedOffset,
        receivedNbytes);

    EXPECT_EQ(receivedTotalChunks, totalChunks)
        << "total_chunks mismatch: expected " << totalChunks << ", got "
        << receivedTotalChunks;
...
TEST_P(P2pNvlSendRecvOneMultipleCallsTest, SendRecvOneMultipleCalls) {
  ...
  DeviceBuffer totalChunksBuffer(num_calls * sizeof(size_t));
  ...
  std::vector<size_t> totalChunks(num_calls, 1); // Each call sends 1 chunk
  CUDACHECK_TEST(cudaMemcpy(
      nbytes_d,
      params.nbytes.data(),
  ...
  DeviceBuffer recvTotalChunksBuffer(num_calls * sizeof(size_t));

  auto recvNbytes_d = static_cast<size_t*>(recvNbytesBuffer.get());
  auto recvOffset_d = static_cast<size_t*>(recvOffsetBuffer.get());
  auto recvTotalChunks_d = static_cast<size_t*>(recvTotalChunksBuffer.get());
  ...
  if (globalRank == 0) {
    ...
    std::vector<size_t> recvTotalChunks(num_calls);
    ...
    for (size_t i = 0; i < num_calls; i++) {
      XLOGF(
          INFO,
          "Rank {}: Call {} metadata - nbytes={}, offset={}, total_chunks={}",
          globalRank,
          i,
          recvNbytes[i],
          recvOffset[i],
      ...
      EXPECT_EQ(recvTotalChunks[i], 1)
          << "Call " << i << " total_chunks mismatch";
    }
