...
__global__ void testSendOneKernel(
  ...
  p2p.send_one(group, src_d, nbytes, 0, offset_in_output, total_chunks);
}
...
__global__ void testRecvOneKernel(
  ...
  p2p.recv_one(group, dst_base_d, 0, &nbytes, &offset, &total_chunks);
...
void testSendOne(
  ...
  testSendOneKernel<<<numBlocks, blockSize>>>(
      p2p, src_d, nbytes, offset_in_output, total_chunks);
...
void testRecvOne(
  ...
  testRecvOneKernel<<<numBlocks, blockSize>>>(
      p2p, dst_base_d, nbytes_d, offset_d, total_chunks_d);
  PIPES_KERNEL_LAUNCH_CHECK();
}
...
__global__ void testSendOneMultipleTimesKernel(
    ...
    const size_t* total_chunks_array,
    size_t num_calls) {
  auto group = make_warp_group();
  ...
  for (size_t i = 0; i < num_calls; i++) {
    p2p.send_one(
        ...
        src_d_array[i],
        nbytes_array[i],
        static_cast<int32_t>(i), // call_index increments for each call
        offset_array[i],
        total_chunks_array[i]);
  }
}
...
__global__ void testRecvOneMultipleTimesKernel(
  ...
  for (size_t i = 0; i < num_calls; i++) {
    ...
    if (group.is_leader()) {
      ...
      total_chunks_array_d[i] = total_chunks;
    }
  }
}
