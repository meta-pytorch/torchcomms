...
__global__ void waitSignalFromAllKernel(
  ...
  if (myRank == targetRank) {
    ...
    // Signal the target rank (convert targetRank to peer index)
    int targetPeerIndex = transport.rank_to_peer_index(targetRank);
    transport.signal_peer(
        targetPeerIndex, group, signalIdx, SignalOp::SIGNAL_ADD, 1);
  }
