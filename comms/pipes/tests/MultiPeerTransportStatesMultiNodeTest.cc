...
class MultiPeerTransportStatesMultiNodeFixture : public MpiBaseTestFixture {
 protected:
  void SetUp() override {
   ...
   * verify that MultiPeerTransportStates made the correct topology
   * decisions.
   */
  void detectPlatform() {
    struct RankLocation {
    ...
    CUDACHECK_TEST(
        cudaDeviceGetPCIBusId(busId, detail::kNvmlBusIdLen, localRank));
    myLoc.fabricInfo = query_nvml_fabric_info(busId);
...
// peers.
TEST_F(MultiPeerTransportStatesMultiNodeFixture, TopologyDiscoveryMultiNode) {
...
TEST_F(MultiPeerTransportStatesMultiNodeFixture, ExchangeMultiNode) {
  ASSERT_GE(numRanks, 4) << "Requires >= 4 ranks (nnodes=2, ppn=2)";
...
TEST_F(MultiPeerTransportStatesMultiNodeFixture, DeviceHandleMultiNode) {
  ASSERT_GE(numRanks, 4) << "Requires >= 4 ranks (nnodes=2, ppn=2)";
  ...
  // NVL transports are always populated (at minimum same-node peers).
  EXPECT_FALSE(handle.nvlTransports.empty());

  if (isMnnvl_) {
    EXPECT_EQ(handle.nvlTransports.size(), static_cast<uint32_t>(numRanks - 1))
        << "MNNVL: all peers should be in nvlTransports";
  } else {
    EXPECT_EQ(
        handle.nvlTransports.size(), static_cast<uint32_t>(localSize_ - 1))
        << "Non-MNNVL: nvlTransports should match same-node peers";
  }

  // IBGDA transports are always populated for all peers (universal).
...
TEST_F(MultiPeerTransportStatesMultiNodeFixture, HostAccessorsMultiNode) {
  ASSERT_GE(numRanks, 4) << "Requires >= 4 ranks (nnodes=2, ppn=2)";
