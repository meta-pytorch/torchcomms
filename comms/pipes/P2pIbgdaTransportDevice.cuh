...
class P2pIbgdaTransportDevice {
 public:
  ...
  /**
   * reset_signal - Reset remote peer's signal buffer to zero (non-blocking)
   *
   ...
   * 3. Sender: reset_signal() - reset for next iteration
   *
   * @param signalId Index into the signal buffer array
   *
   * @return IbgdaWork for tracking local completion via wait_local()
   */
  __device__ IbgdaWork reset_signal(int signalId) {
    checkSignalId(signalId, "reset_signal");
    // Use signal with SET operation to write 0 to the remote signal
    // Note: Currently using a workaround since DOCA doesn't support SET yet.
    // We store 0 locally first, then use put() to write it to the remote
    // signal.
