 ...
 *   put_group(), put_signal_group()
 *   - Accept a ThreadGroup and partition a single data chunk across group lanes
...
class P2pIbgdaTransportDevice {
 public:
  ...
  /**
   * put_group - Group-collaborative RDMA Write (single chunk, partitioned)
   *
   * Accepts a ThreadGroup and a single data chunk, partitions the data
   * across group threads, and issues RDMA writes.
   *
   ...
   * @param localBuf Source buffer in local GPU memory (full chunk)
   * @param remoteBuf Destination buffer in remote GPU memory (full chunk)
   * @param nbytes Total number of bytes to transfer (partitioned across lanes)
   *
   * @return IbgdaWork for tracking local completion via wait_local() (per-lane)
   */
  __device__ IbgdaWork put_group(
      ThreadGroup& group,
      const IbgdaLocalBuffer& localBuf,
      const IbgdaRemoteBuffer& remoteBuf,
      std::size_t nbytes) {
  ...
  /**
   * put_signal_group - Group-collaborative RDMA Write with fenced signal
   *                     (single chunk, partitioned, adaptive routing safe)
   *
   * Accepts a ThreadGroup and a single data chunk, partitions the data
   * across group threads, issues collaborative RDMA writes, and
   * the leader issues a fenced atomic signal. The signal ticket is
   * broadcast to all threads via group.broadcast_64().
   *
   * All ThreadGroup sizes are supported:
   ...
   */
  __device__ IbgdaWork put_signal_group(
      ThreadGroup& group,
      const IbgdaLocalBuffer& localBuf,
      const IbgdaRemoteBuffer& remoteBuf,
      std::size_t nbytes,
      int signalId,
      uint64_t signalVal) {
    checkSignalId(signalId, "put_signal_group");
 ...
 private:
   ...
   * be computed by the caller (put_group / put_signal_group).
  ...
  __device__ IbgdaWork put_group_impl(
    ...
    base_wqe_idx = group.broadcast_64(base_wqe_idx);
