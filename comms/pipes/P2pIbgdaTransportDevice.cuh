...
class P2pIbgdaTransportDevice {
 public:
  ...
  __device__ __forceinline__ void send(
    ...
    const uint64_t iteration = *sendIterationCounter_;
    const size_t totalSteps = (nbytes + dataBufferSize_ - 1) / dataBufferSize_;

    const int completionSlot = remoteSignalId_;
    const int backPressureSlot = localSignalId_ + 1;

    for (size_t stepId = 0; stepId < totalSteps; stepId++) {
      const size_t pipelineIdx = (iteration + stepId) % pipelineDepth_;
      const size_t stagingOffset = pipelineIdx * dataBufferSize_;
      const size_t stepOffset = stepId * dataBufferSize_;
      const size_t stepBytes = (stepOffset + dataBufferSize_ <= nbytes)
          ? dataBufferSize_
          : (nbytes - stepOffset);
    ...
    if (group.is_global_leader()) {
      atomicAdd(
          reinterpret_cast<unsigned long long int*>(sendIterationCounter_),
  ...
  __device__ __forceinline__ void recv(
    ...
    const uint64_t iteration = *recvIterationCounter_;
    const size_t totalSteps = (nbytes + dataBufferSize_ - 1) / dataBufferSize_;

    const int completionSlot = localSignalId_;
    const int backPressureSlot = remoteSignalId_ + 1;
    ...
    if (group.is_global_leader()) {
      atomicAdd(
          reinterpret_cast<unsigned long long int*>(recvIterationCounter_),
          static_cast<unsigned long long int>(totalSteps));
    }
#endif
  }
