...
constexpr int kIbgdaWarmupIters = 20;
constexpr int kIbgdaBenchmarkIters = 30;
...
class IbgdaBenchmarkFixture : public MpiBaseTestFixture {
 protected:
  ...
  // Sender issues kIbgdaBenchmarkIters put_signal operations (each adds 1 to
  // signal) Receiver waits for final signal value
  float runPutSignalBenchmark(
    ...
    // Warmup iterations
    for (int warmup = 0; warmup < kIbgdaWarmupIters; warmup++) {
      ...
      if (globalRank == 0) {
        ...
        launchIbgdaWaitSignal(
            ...
            kSignalId,
            IbgdaCmpOp::GE,
            warmup + 1, // Wait for cumulative signal value
            config.numBlocks,
            config.numThreads,
            stream_);
      }
    ...
    if (globalRank == 0) {
      ...
      for (int i = 0; i < kIbgdaBenchmarkIters; i++) {
        launchIbgdaPutSignalWaitLocal(
            deviceTransportPtr,
            localBuf,
            remoteBuf,
      ...
      launchIbgdaWaitSignal(
          ...
          kSignalId,
          IbgdaCmpOp::GE,
          kIbgdaBenchmarkIters, // Wait for all signals
          config.numBlocks,
          config.numThreads,
          stream_);
    }
    ...
    float avgTime_ms = totalTime_ms / kIbgdaBenchmarkIters;
  ...
  void printResultsTable(const std::vector<IbgdaBenchmarkResult>& results) {
    ...
    ss << "Warmup iterations: " << kIbgdaWarmupIters
       << ", Benchmark iterations: " << kIbgdaBenchmarkIters << "\n";
    ss << "================================================================================\n\n";
...
TEST_F(IbgdaBenchmarkFixture, PutSignalWaitLocal) {
  ...
  try {
    // Create transport (no data buffer - user manages buffers)
    ...
    XLOGF(
        ...
        globalRank,
        fmt::ptr(localDataBuf.ptr),
        localDataBuf.lkey.value,
        fmt::ptr(remoteDataBuf.ptr),
...
TEST_F(IbgdaBenchmarkFixture, PutWaitLocal) {
  ...
  // Test configurations - various message sizes (same as PutSignalWaitLocal)
  std::vector<IbgdaBenchmarkConfig> configs;
  ...
  try {
    ...
    XLOGF(
        INFO,
        "Rank {}: Transport initialized for PutWaitLocal benchmark",
        globalRank);
    ...
    for (const auto& config : configs) {
      ...
      // Warmup iterations - only rank 0 sends (one-way benchmark)
      for (int warmup = 0; warmup < kIbgdaWarmupIters; warmup++) {
        MPI_CHECK(MPI_Barrier(MPI_COMM_WORLD));
        if (globalRank == 0) {
          launchIbgdaPutWaitLocal(
              deviceTransportPtr,
              localDataBuf,
              remoteDataBuf,
              config.nBytes,
      ...
      // Benchmark - sender issues all operations
      ...
      float avgTime_ms = totalTime_ms / kIbgdaBenchmarkIters;
      result.latency = avgTime_ms * 1000.0f;

      // Unidirectional bandwidth
      result.bandwidth = (config.nBytes / 1e9f) / (avgTime_ms / 1000.0f);
  ...
  // Print results table (only rank 0)
  if (globalRank == 0) {
    std::stringstream ss;
    ss << "\n";
    ss << "================================================================================\n";
    ss << "                    IBGDA Put+WaitLocal Benchmark Results\n";
    ss << "                    (RDMA Write without signaling)\n";
    ss << "================================================================================\n";
    ss << std::left << std::setw(20) << "Test Name" << std::right
       << std::setw(12) << "Msg Size" << std::right << std::setw(10) << "Blocks"
       << std::right << std::setw(10) << "Threads" << std::right
    ...
    ss << "================================================================================\n";
    ss << "BW = Bandwidth (message size / time), Latency = Average time per transfer\n";
    ss << "Note: This measures raw RDMA write without signaling overhead\n";
    ss << "Warmup iterations: " << kIbgdaWarmupIters
       << ", Benchmark iterations: " << kIbgdaBenchmarkIters << "\n";
    ss << "================================================================================\n\n";
...
TEST_F(IbgdaBenchmarkFixture, SignalOnlyLatency) {
  ...
  try {
    ...
    for (int warmup = 0; warmup < kIbgdaWarmupIters; warmup++) {
      MPI_CHECK(MPI_Barrier(MPI_COMM_WORLD));
      if (globalRank == 0) {
        launchIbgdaSignalOnly(deviceTransportPtr, kSignalId, 1, 1, 32, stream_);
      } else {
        launchIbgdaWaitSignal(
            deviceTransportPtr,
            kSignalId,
    ...
    CUDA_CHECK_VOID(
        cudaEventElapsedTime(&totalTime_ms, start.get(), stop.get()));
    float avgLatencyUs = (totalTime_ms / kIbgdaBenchmarkIters) * 1000.0f;
