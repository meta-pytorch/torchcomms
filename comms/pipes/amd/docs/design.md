# AMD support in `comms/pipes/` — design

This document describes how `comms/pipes/` supports AMD GPUs (HIP/ROCm). For
...
`comms/pipes/` builds from the same `.h` / `.cc` / `.cuh` / `.cu` sources
...
All under `comms/pipes/amd/`. Together they replace the legacy parallel
...
Under `comms/pipes/amd/pipes_gda/`:
...
gpu_cpp_library(
    ...
    deps = [...] + select({
        ...
        "ovr_config//gpu:amd": ["//comms/pipes:hip_compat"],
    }),
)
```
...
### AMD-only support targets in `comms/pipes:`
...
### AMD `pipes_gda` library targets in `comms/pipes/amd:`
...
#include "comms/pipes/amd/HipHostCompat.h"  // unconditional — provides DeviceBuffer/CudaEvent on AMD
