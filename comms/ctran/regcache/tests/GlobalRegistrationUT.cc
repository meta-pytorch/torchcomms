 ...
 * 1. ctranGlobalRegisterWithPtr can be called without a comm
...
TEST_F(GlobalRegistrationTest, RegisterWithoutCommOrMapper) {
  ...
  commResult_t result = ctranGlobalRegisterWithPtr(buf, bufSize, cudaDev);
  EXPECT_EQ(result, commSuccess)
      << "Global registration should succeed without comm/mapper";

  // Deregister
  result = ctranGlobalDeregisterWithPtr(buf, bufSize, cudaDev);
...
TEST_F(GlobalRegistrationTest, MultipleRegistrationsBeforeComm) {
  ...
  for (int i = 0; i < numBuffers; i++) {
    commResult_t result =
        ctranGlobalRegisterWithPtr(buffers[i], bufSize, cudaDev);
    EXPECT_EQ(result, commSuccess)
  ...
  for (int i = 0; i < numBuffers; i++) {
    commResult_t result =
        ctranGlobalDeregisterWithPtr(buffers[i], bufSize, cudaDev);
    EXPECT_EQ(result, commSuccess)
...
TEST_F(GlobalRegistrationTest, MultiSegmentDisjointRegistration) {
  ...
  commResult_t result =
      ctranGlobalRegisterWithPtr(disjointBuf, totalSize, cudaDev);
  EXPECT_EQ(result, commSuccess)
  ...
  // Deregister
  result = ctranGlobalDeregisterWithPtr(disjointBuf, totalSize, cudaDev);
  EXPECT_EQ(result, commSuccess)
