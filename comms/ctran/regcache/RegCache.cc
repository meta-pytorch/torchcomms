...
void ctran::RegCache::init() {
  ...
  CLOGF_SUBSYS(
      ...
      "IB={} NVL={} SOCKET={} TCPDM={}",
      globalBackends_[CommBackend::IB],
      globalBackends_[CommBackend::NVL],
      globalBackends_[CommBackend::SOCKET],
      globalBackends_[CommBackend::TCPDM]);
...
commResult_t
ctran::RegCache::globalRegister(const void* buf, size_t len, int cudaDev) {
  ...
  // Register if in eager mode (async mode will defer to async thread)
...
commResult_t
ctran::RegCache::globalDeregister(const void* buf, size_t len, int cudaDev) {
  if (buf == nullptr || len == 0) {
    return commSuccess;
  }
  ...
  // Use pinRange to discover all physical segments
  std::vector<ctran::regcache::SegmentRange> ranges;
  FB_COMMCHECK(
      ctran::regcache::SegmentRange::pinRange(buf, cudaDev, len, ranges));

  // Free each segment
  size_t totalSegmentsFreed = 0;
  for (const auto& range : ranges) {
    void* segHdl =
        segmentsAvl_.rlock()->search(const_cast<void*>(range.buf), range.len);
    if (segHdl) {
      bool freed = false;
      bool ncclManaged = false;
      std::vector<std::unique_ptr<ctran::regcache::RegElem>> regElems;
      FB_COMMCHECK(freeSegment(segHdl, freed, ncclManaged, regElems));

      if (freed) {
        totalSegmentsFreed += regElems.size();
      }
    } else {
      CLOGF_TRACE(
          ALLOC,
          "CTRAN-REGCACHE globalDeregister: segment not found for buf={} len={}, "
          "may have already been freed or was never registered",
          range.buf,
          range.len);
    }
  }

  // TODO need to release remote mem
