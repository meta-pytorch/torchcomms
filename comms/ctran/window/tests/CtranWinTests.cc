...
TEST_F(CtranWinTest, multiSegmentWindowRegister) {
  ...
  NCCLCHECK_TEST(
      ncclMemAllocDisjoint(&disjointBuf, segSizes, disjointSegments));
  ...
  CtranWin* win = nullptr;
  meta::comms::Hints hints;
  auto res = ctranWinRegister(
      disjointBuf, totalSize, comm->ctranComm_.get(), &win, hints);
  ASSERT_EQ(res, commSuccess);
  ...
  NCCLCHECK_TEST(ncclMemFreeDisjoint(disjointBuf, segSizes));
