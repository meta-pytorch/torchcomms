...
| `MultiGraph`         | Multiple independent graph captures and replays on the same communicator. AllGather ctgraph variants are disabled for now because the current pattern captures graph 2 while graph 1 replay may still have same-comm GPE/mapper work in flight; that replay-plus-capture overlap is not a supported production use case. |
