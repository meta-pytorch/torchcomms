...
def parse_sweep_output(raw: str) -> Dict[int, List[Tuple[str, float]]]:  # noqa: C901
...
def run_sweep_only() -> int:
    ...
    for ri, (lo, hi, iters, warmup) in enumerate(size_ranges):
        ...
        for ms in sorted(best):
            config_name, latency_us = best[ms]
            print(f"{format_size(ms):>10s} | {config_name:>14s} | {latency_us:>10.2f}")
        print(f"{'=' * 80}\n", flush=True)
...
def run_full_sweep() -> int:  # noqa: C901
