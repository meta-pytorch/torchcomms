...
class SweepConfig:
    warmup_iters: int = 5
    bench_iters: int = 50
    msg_sizes: List[int] = field(default_factory=lambda: list(_DEFAULT_MSG_SIZES))
    blocks_per_peer_values: List[int] = field(
        default_factory=lambda: list(_DEFAULT_BPP)
    )
    num_warps_values: List[int] = field(default_factory=lambda: list(_DEFAULT_WARPS))
    chunk_size_values: List[int] = field(default_factory=lambda: list(_DEFAULT_CHUNKS))
...
class IBAutoTuneSweep:
    ...
    def __init__(self, comm: TorchComm, max_msg_size: int = 16 * 1024 * 1024) -> None:
        from comms.ctran.prims.collectives.triton import alloc_comms_buffer
    ...
    def benchmark_triton(
        ...
        from comms.ctran.prims.collectives.triton import (
