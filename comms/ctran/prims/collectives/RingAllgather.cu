...
#include "comms/ctran/prims/collectives/RingAllgather.cuh"
...
template __global__ void ring_allgather_kernel<1, 512>(
    const PIPES_GRID_CONSTANT RingAllgatherArgs<1>,
    Timeout);
template __global__ void ring_allgather_kernel<2, 512>(
    const PIPES_GRID_CONSTANT RingAllgatherArgs<2>,
    Timeout);
template __global__ void ring_allgather_kernel<4, 512>(
    const PIPES_GRID_CONSTANT RingAllgatherArgs<4>,
    Timeout);
