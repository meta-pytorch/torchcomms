...
#include "comms/ctran/prims/collectives/benchmarks/IbSendRecvBenchmarkKernels.cuh"
#include "comms/ctran/prims/collectives/ib/SendRecv.cuh"
 ...
 protected:
  ...
  BenchResult run_put_benchmark(size_t total_bytes, int num_blocks) {
    ...
    // Warmup
    {
      ...
      ctran::prims::ib::benchmark::launch_put_bw_kernel(
          s.dev_win,
          s.src_buf,
          total_bytes,
          dst_rank,
  ...
  BenchResult run_send_recv_benchmark(
    ...
    // Warmup
    {
      ...
      for (int i = 0; i < kWarmupIters; i++) {
        ctran::prims::ib::launch_send_recv_kernel(
            dev_win,
            staging_buf,
            src_ptr,
            staging_ptr,
