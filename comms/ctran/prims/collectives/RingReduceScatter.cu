...
#include "comms/ctran/prims/collectives/RingReduceScatter.cuh"
...
ring_reduce_scatter_kernel<1, float, SumOp, 16384, 512>(
    const PIPES_GRID_CONSTANT RingReduceScatterArgs<1, float>,
    Timeout);
template __global__ void
ring_reduce_scatter_kernel<2, float, SumOp, 16384, 512>(
    const PIPES_GRID_CONSTANT RingReduceScatterArgs<2, float>,
    Timeout);
template __global__ void
ring_reduce_scatter_kernel<4, float, SumOp, 16384, 512>(
    const PIPES_GRID_CONSTANT RingReduceScatterArgs<4, float>,
    Timeout);
