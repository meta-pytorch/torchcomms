...
class P2pLl128BenchmarkFixture : public meta::comms::BenchmarkTestFixture {
 protected:
  ...
  float runSimpleBenchmark(
    ...
    void* kernelFunc = isSend ? (void*)ctran::prims::benchmark::p2pSend
                              : (void*)ctran::prims::benchmark::p2pRecv;
  ...
  float runSimpleBidirectionalBenchmark(
    ...
    void* kernelFunc = (void*)ctran::prims::benchmark::p2pBidirectional;
    ...
    for (const auto& cfg : configs) {
      ...
      if (cfg.nBytes <= kSimpleChunkThreshold) {
        ctran::prims::MultiPeerNvlTransportConfig p2pConfig{
            ...
            .pipelineDepth = 2,
            .ll128BufferSize = ctran::prims::ll128_buffer_size(cfg.nBytes),
        };
        ...
        ctran::prims::MultiPeerNvlTransportConfig ll128Config{
            .dataBufferSize = cfg.nBytes,
            .chunkSize = cfg.nBytes,
            .pipelineDepth = 2,
            .ll128BufferSize = ctran::prims::ll128_buffer_size(cfg.nBytes),
        };
        ctran::prims::MultiPeerNvlTransport ll128Transport(
            globalRank, worldSize, bootstrap, ll128Config);
        ll128Transport.exchange();

        ctran::prims::MultiPeerNvlTransportConfig simpleConfig{
            .dataBufferSize = cfg.nBytes,
            .chunkSize = kSimpleChunkSize,
            .pipelineDepth = 2,
        };
        ctran::prims::MultiPeerNvlTransport simpleTransport(
  ...
  void run_chunked_ll128_sweep(
    ...
    for (const auto& cfg : configs) {
      ...
      std::size_t ll128BufSize = (cfg.bufferNumPackets > 0)
          ? cfg.bufferNumPackets * ctran::prims::kLl128PacketSize
          : ctran::prims::ll128_buffer_size(cfg.nBytes);
...
TEST_F(P2pLl128BenchmarkFixture, AutoTunedBenchmark) {
  ...
  for (auto nBytes : kAutoTuneMessageSizes) {
    auto cfg = ctran::prims::ll128_auto_tune(nBytes);
    configs.push_back(
...
TEST_F(P2pLl128BenchmarkFixture, AutoTunedBidirectionalBenchmark) {
  ...
  std::vector<Ll128Config> configs;
  for (auto nBytes : kAutoTuneMessageSizes) {
    auto cfg = ctran::prims::ll128_auto_tune_bidirectional(nBytes);
    configs.push_back(
