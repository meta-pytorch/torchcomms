...
#include "comms/ctran/prims/MultiPeerNvlTransport.h"
#include "comms/ctran/prims/benchmarks/BenchmarkKernel.cuh"
#include "comms/ctran/prims/benchmarks/BenchmarkMacros.h"
#include "comms/ctran/prims/benchmarks/P2pNvlBenchmarkUtils.h"
#include "comms/ctran/prims/ll/LlPacket.cuh"
#include "comms/ctran/prims/ll128/Ll128AutoTune.cuh"
...
class P2pLlBenchmarkFixture : public meta::comms::BenchmarkTestFixture {
 protected:
  ...
  float runNcclBenchmark(std::size_t nBytes, float& timeUs) {
    ...
    // Benchmark — pre-enqueue all groups, then measure GPU-side time only
    // to avoid including host-side ncclGroupEnd() overhead in the measurement.
  ...
  float runLl128Benchmark(
      ctran::prims::P2pNvlTransportDevice& p2p,
    ...
    void* kernelFunc = isSend ? (void*)ctran::prims::benchmark::p2pLl128Send
                              : (void*)ctran::prims::benchmark::p2pLl128Recv;
  ...
  float runLlBenchmark(
    ...
    Timeout timeout;
    void* args[] = {&p2p, &devicePtr, &nBytes, &timeout};
    void* kernelFunc = isSend ? (void*)ctran::prims::benchmark::p2pLlSend
                              : (void*)ctran::prims::benchmark::p2pLlRecv;
  ...
  float runLl128BidirectionalBenchmark(
    ...
    void* kernelFunc = (void*)ctran::prims::benchmark::p2pLl128Bidirectional;
  ...
  float runLlBidirectionalBenchmark(
    ...
    Timeout timeout;
    void* args[] = {&p2p, &sendPtr, &recvPtr, &nBytes, &timeout};
    void* kernelFunc = (void*)ctran::prims::benchmark::p2pLlBidirectional;
  ...
  void run_unidirectional_sweep(
    ...
    for (const auto& cfg : configs) {
      ...
      ctran::prims::MultiPeerNvlTransportConfig p2pConfig{
          .dataBufferSize = 0,
          .chunkSize = 0,
          .pipelineDepth = 1,
          .tile_max_groups = 0,
          .ll128BufferSize =
              ll128Eligible ? ctran::prims::ll128_buffer_size(cfg.nBytes) : 0,
          .llBufferSize = ctran::prims::ll_buffer_size(cfg.nBytes),
      };

      ctran::prims::MultiPeerNvlTransport transport(
...
TEST_F(P2pLlBenchmarkFixture, UnidirectionalBenchmark) {
  ...
  // LL is non-cooperative: each call uses a single warp (32 threads).
  // All configs use numBlocks=1, numThreads=32.
  std::vector<LlConfig> configs = {
      ...
      {4 * 1024, 1, 32, "LL_4KB"},
      {8 * 1024, 1, 32, "LL_8KB"},
      {16 * 1024, 1, 32, "LL_16KB"},
      {32 * 1024, 1, 32, "LL_32KB"},
      {64 * 1024, 1, 32, "LL_64KB"},
      {128 * 1024, 1, 32, "LL_128KB"},
      {256 * 1024, 1, 32, "LL_256KB"},
  };
...
TEST_F(P2pLlBenchmarkFixture, BidirectionalBenchmark) {
  ...
  // Bidirectional uses partition_interleaved(2): 64 threads = 2 warps
  // (1 sender + 1 receiver). All configs use numBlocks=1, numThreads=64.
  std::vector<LlConfig> configs = {
      {8, 1, 64, "Bidir_8B"},
      {16, 1, 64, "Bidir_16B"},
      {32, 1, 64, "Bidir_32B"},
      ...
      {8 * 1024, 1, 64, "Bidir_8KB"},
      {16 * 1024, 1, 64, "Bidir_16KB"},
      {32 * 1024, 1, 64, "Bidir_32KB"},
      {64 * 1024, 1, 64, "Bidir_64KB"},
      {128 * 1024, 1, 64, "Bidir_128KB"},
      {256 * 1024, 1, 64, "Bidir_256KB"},
  };
