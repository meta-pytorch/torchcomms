...
#include "comms/ctran/prims/TiledBuffer.cuh"
...
#include "comms/ctran/prims/benchmarks/TileSendRecv.cuh"
...
class P2pSendRecvBenchmarkFixture : public meta::comms::BenchmarkTestFixture {
 protected:
  ...
  bool verifyP2pCorrectness(
      ctran::prims::P2pNvlTransportDevice* p2pDevicePtr,
  ...
  float runTileBenchmark(
    ...
    ctran::prims::TiledBuffer<char> sendTiles(
        static_cast<char*>(sendBuff.get()), config.nBytes, numSendBlocks);
    ctran::prims::TiledBuffer<char> recvTiles(
    ...
    std::size_t maxSignalBytes = config.chunksPerSlot > 1
        ? static_cast<std::size_t>(
            ...
            ~15ULL
        : 0;
    void* kernelFunc = (void*)ctran::prims::benchmark::p2pTileSendRecv;
    void* args[] = {
        p2pDevicePtr,
        &sendTiles,
        &recvTiles,
