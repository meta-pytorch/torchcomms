...
#include "comms/ctran/prims/tests/P2pNvlTransportTest.cuh"
...
TEST_F(P2pNvlTransportTestFixture, TileSendRecvMultiCall) {
  ...
  for (int iter = 0; iter < nIters; iter++) {
    ...
    CUDACHECK_TEST(cudaLaunchKernel(
        (void*)ctran::prims::benchmark::p2pTileSendRecv,
        grid,
        block,
        args,
        0,
...
TEST_F(P2pNvlTransportTestFixture, TileSendRecvDynamicBlockCount) {
  ...
  for (size_t roundIdx = 0; roundIdx < rounds.size(); roundIdx++) {
    ...
    MPI_CHECK(MPI_Barrier(MPI_COMM_WORLD));
    CUDACHECK_TEST(cudaLaunchKernel(
        (void*)ctran::prims::benchmark::p2pTileSendRecvDynamic,
        dim3(totalBlocks),
        dim3(256),
        args,
        0,
...
static void runTileForwardTest(
  ...
  for (int iter = 0; iter < nIters; iter++) {
    ...
    if (globalRank == 0) {
      ...
      ctran::prims::TiledBuffer<char> dstTiles(dstPtr, nBytes, numSendBlocks);
      int numBlocksArg = numSendBlocks;
      std::size_t maxSignalBytes = 0;
      void* args[] = {
          &p2pHost,
      ...
      CUDACHECK_TEST(cudaLaunchKernel(
          (void*)ctran::prims::benchmark::p2pTileForward,
          dim3(numSendBlocks),
          dim3(threadCount),
          args,
          0,
