...
#include "comms/ctran/prims/TimeoutUtils.h"
#include "comms/ctran/prims/amd/HipHostCompat.h"
#include "comms/ctran/prims/tests/Checks.h"
#include "comms/ctran/prims/tests/P2pIbgdaTransportDeviceTest.cuh"

namespace ctran::prims::tests {
...
__global__ void testPutCooperativePartitioning(bool* success) {
  *success = true;
  ...
  if (group.group_size != ctran::prims::kWarpSize) {
    *success = false;
    return;
  }

  constexpr std::size_t kTotalBytes = 1024; // 1KB
  constexpr std::size_t kChunkSize = kTotalBytes / ctran::prims::kWarpSize;
  ...
  ctran::prims::IbgdaLocalBuffer baseBuf(
      basePtr, ctran::prims::NetworkLKeys{ctran::prims::NetworkLKey(0x1111)});
  ctran::prims::IbgdaLocalBuffer laneBuf = baseBuf.subBuffer(expectedOffset);
...
void runTestPutCooperativePartitioning(bool* d_success) {
  testPutCooperativePartitioning<<<1, ctran::prims::kWarpSize>>>(d_success);
  PIPES_KERNEL_LAUNCH_CHECK();
}

void runTestPutSignalGroupBroadcast(bool* d_success) {
  testPutSignalGroupBroadcast<<<1, ctran::prims::kWarpSize>>>(d_success);
  PIPES_KERNEL_LAUNCH_CHECK();
}
...
__global__ void testBroadcast64Block(bool* success) {
  auto group = ctran::prims::make_block_group();
...
__global__ void testBroadcast64Multiwarp(bool* success) {
  auto group = ctran::prims::make_multiwarp_group();
