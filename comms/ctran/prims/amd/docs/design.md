# AMD support in `comms/ctran/prims/` — design

This document describes how `comms/ctran/prims/` supports AMD GPUs (HIP/ROCm). For
...
`comms/ctran/prims/` builds from the same `.h` / `.cc` / `.cuh` / `.cu` sources
...
All under `comms/ctran/prims/amd/`. Together they replace the legacy parallel
...
Under `comms/ctran/prims/amd/pipes_gda/`:
...
Under `comms/ctran/prims/amd/nic/`:
...
gpu_cpp_library(
    ...
    deps = [...] + select({
        ...
        "ovr_config//gpu:amd": ["//comms/ctran/prims:hip_compat"],
    }),
)
```
...
### AMD-only support targets in `comms/ctran/prims:`
...
### AMD `pipes_gda` library targets in `comms/ctran/prims/amd:`
...
```cpp
#include "comms/ctran/prims/MultipeerIbgdaTransport.h"
#include "comms/ctran/prims/amd/HipHostCompat.h"  // unconditional — provides DeviceBuffer/CudaEvent on AMD
