...
static commResult_t initResult;
IbvSymbols ibvSymbols;

commResult_t wrap_ibv_symbols(void) {
  pthread_once(&initOnceControl, []() {
    initResult =
        buildIbvSymbols(ibvSymbols) == 0 ? commSuccess : commInternalError;
  });
  return initResult;
}
...
#define IBV_INT_CHECK_RET_ERRNO(                          \
    container, internal_name, call, success_retval, name) \
  CHECK_NOT_NULL(container, internal_name);               \
  int ret = container.call;                               \
...
#define IBV_MQP_RETRY_ERRNO_ALL(e) \
  (NCCL_IB_MQP_RETRY_ALL ? (e != 0) : IBV_MQP_RETRY_ERRNO(e))

commResult_t wrap_ibv_fork_init() {
  IBV_INT_CHECK(
      ibvSymbols,
...
commResult_t wrap_ibv_free_device_list(struct ibv_device** list) {
  IBV_PASSTHRU(
      ibvSymbols,
      ibv_internal_free_device_list,
...
commResult_t wrap_ibv_open_device(
  ...
  IBV_PTR_CHECK(
      ...
      ibv_internal_open_device(device),
      *ret,
      nullptr,
      "ibv_open_device");
}

commResult_t wrap_ibv_close_device(
    struct ibv_context* context) { /*returns 0 on success, -1 on failure*/
  IBV_INT_CHECK(
      ibvSymbols,
      ibv_internal_close_device,
      ibv_internal_close_device(context),
      -1,
      "ibv_close_device");
}
...
commResult_t wrap_ibv_query_device(
  ...
  IBV_INT_CHECK_RET_ERRNO(
      ibvSymbols,
      ibv_internal_query_device,
      ibv_internal_query_device(context, device_attr),
      0,
      "ibv_query_device");
}
...
commResult_t wrap_ibv_alloc_pd(
  ...
  IBV_PTR_CHECK_ERRNO(
      ibvSymbols,
      ibv_internal_alloc_pd,
      ibv_internal_alloc_pd(context),
      *ret,
      nullptr,
      "ibv_alloc_pd");
}

commResult_t wrap_ibv_dealloc_pd(
    struct ibv_pd* pd) { /*returns 0 on success, or the value of errno on
                            failure (which indicates the failure reason)*/
  IBV_INT_CHECK_RET_ERRNO(
      ibvSymbols,
      ibv_internal_dealloc_pd,
      ibv_internal_dealloc_pd(pd),
      0,
      "ibv_dealloc_pd");
}
...
commResult_t wrap_ibv_reg_mr(
  ...
  IBV_PTR_CHECK_ERRNO(
      ibvSymbols,
      ibv_internal_reg_mr,
      ibv_internal_reg_mr(pd, addr, length, access),
      *ret,
      nullptr,
      "ibv_reg_mr");
}
...
commResult_t wrap_ibv_reg_dmabuf_mr(
  ...
  IBV_PTR_CHECK_ERRNO(
      ibvSymbols,
      ibv_internal_reg_dmabuf_mr,
      ibv_internal_reg_dmabuf_mr(pd, offset, length, iova, fd, access),
      *ret,
      nullptr,
      "ibv_reg_dmabuf_mr");
}
...
struct ibv_mr* wrap_direct_ibv_reg_dmabuf_mr(
  ...
  return ibvSymbols.ibv_internal_reg_dmabuf_mr(
      pd, offset, length, iova, fd, access);
}

commResult_t wrap_ibv_dereg_mr(
    struct ibv_mr* mr) { /*returns 0 on success, or the value of errno on
                            failure (which indicates the failure reason)*/
  IBV_INT_CHECK_RET_ERRNO(
      ibvSymbols,
      ibv_internal_dereg_mr,
      ibv_internal_dereg_mr(mr),
      0,
      "ibv_dereg_mr");
}
...
commResult_t wrap_ibv_destroy_cq(struct ibv_cq* cq) {
  IBV_INT_CHECK_RET_ERRNO(
      ibvSymbols,
      ibv_internal_destroy_cq,
      ibv_internal_destroy_cq(cq),
      0,
      "ibv_destroy_cq");
}

commResult_t wrap_ibv_destroy_qp(struct ibv_qp* qp) {
  IBV_INT_CHECK_RET_ERRNO(
      ibvSymbols,
      ibv_internal_destroy_qp,
      ibv_internal_destroy_qp(qp),
      0,
      "ibv_destroy_qp");
}
...
commResult_t wrap_ibv_create_qp(
  ...
  IBV_PTR_CHECK_ERRNO(
      ibvSymbols,
      ibv_internal_create_qp,
      ibv_internal_create_qp(pd, qp_init_attr),
      *ret,
      nullptr,
      "ibv_create_qp");
}
...
static void
ibvQpStateName(enum ibv_qp_state state, char* msg, const size_t len) {
  switch (state) {
    case (IBV_QPS_RESET):
...
#define QP_ATTR(attr, userAttr, userFlag, mask) \
  ((userFlag & mask) ? (userAttr) : (attr))

static void ibvModifyQpLog(
    struct ibv_qp* qp,
    enum ibv_qp_state qpState,
    struct ibv_qp_attr* userAttr,
    int userFlag,
    char* msg,
    size_t msgLen) {
  commResult_t res;
  ...
  const char *localGidRes = nullptr, *remoteGidRes = nullptr;
  ...
  snprintf(
      ...
      sizeof(devName),
      "%s",
      (qp->pd->context) ? wrap_ibv_get_device_name(qp->pd->context->device)
                        : "N/A");
  ...
  struct ibv_qp_attr* qpAttr = (res == commSuccess) ? &attr : nullptr;

  // port info, portAttr can be nullptr if not given by the user and query_qp
  // failed
  struct ibv_qp_attr* portAttr =
      QP_ATTR(qpAttr, userAttr, userFlag, IBV_QP_PORT);
  portNum = portAttr ? portAttr->port_num : -1;

  // address info, avAttr can be nullptr if not given by the user and query_qp
  // failed
  ...
  if (avAttr && avAttr->ah_attr.is_global) {
    ...
    FB_COMMCHECKGOTO(
        wrap_ibv_query_gid(qp->pd->context, portNum, gidIndex, &localGid),
        res,
        print);
...
print:
  snprintf(
      msg,
      msgLen,
      "on dev %s:%d, curr state %s, next state %s, local GID index %d, local GID %s, remote GID %s",
      devName,
      portNum,
      currState,
      nextState,
      gidIndex,
      localGidRes ? localGidName : "N/A",
      remoteGidRes ? remoteGidName : "N/A");
  return;
}

commResult_t
wrap_ibv_modify_qp(struct ibv_qp* qp, struct ibv_qp_attr* attr, int attr_mask) {
  char qpMsg[1024];
  int ret = 0, attempts = 0;
  int maxCnt = (int)NCCL_IB_MQP_RETRY_CNT +
      1; // number of attempts = number of retry + 1
  int timeOut = (int)NCCL_IB_MQP_RETRY_SLEEP_MSEC;
  ...
  do {
    if (attempts > 0) {
      ...
      struct timespec tv = {
          .tv_sec = sleepTime / 1000,
          .tv_nsec = (sleepTime % 1000) * ((long)1e6)};
...
commResult_t wrap_ibv_query_ece(
    struct ibv_qp* qp,
    struct ibv_ece* ece,
    int* supported) { /*returns 0 on success, or the value of errno on failure
                         (which indicates the failure reason)*/
  IBV_INT_CHECK_RET_ERRNO_OPTIONAL(
      ibvSymbols,
      ibv_internal_query_ece,
      ibv_internal_query_ece(qp, ece),
      0,
      "ibv_query_ece",
      supported);
}

commResult_t wrap_ibv_set_ece(
    struct ibv_qp* qp,
    struct ibv_ece* ece,
    int* supported) { /*returns 0 on success, or the value of errno on failure
                         (which indicates the failure reason)*/
  IBV_INT_CHECK_RET_ERRNO_OPTIONAL(
      ibvSymbols,
      ibv_internal_set_ece,
      ibv_internal_set_ece(qp, ece),
      0,
      "ibv_set_ece",
      supported);
}

commResult_t wrap_ibv_event_type_str(char** ret, enum ibv_event_type event) {
  *ret = (char*)ibvSymbols.ibv_internal_event_type_str(event);
  return commSuccess;
}

commResult_t wrap_ibv_is_fork_initialized(enum ibv_fork_status* ret) {
  CHECK_NOT_NULL(ibvSymbols, ibv_internal_is_fork_initialized);
  *ret = ibvSymbols.ibv_internal_is_fork_initialized();
  return commSuccess;
}
} // namespace ctran::ibvwrap
