...
// orchestrator lives in AllReduceFusedOrchestrator.cuh
 ...
 *
 * `runAllReduceFused<T, NvlOps, Phase2Fn>` dispatches the NVL phases through
...
struct Ops {
  template <typename T>
  __device__ __forceinline__ static void phase1(
      const common::CommonKernArgs& args,
      comms::prims::ThreadGroup& group) {
    phase1ReduceScatter<T>(args, group);
  }
  template <typename T>
  __device__ __forceinline__ static void phase3(
      const common::CommonKernArgs& args,
      comms::prims::ThreadGroup& group) {
