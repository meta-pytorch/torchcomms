...
commResult_t directGpeFn(
  ...
  // Issue all PUT operations to IB peers
  std::vector<std::pair<int, std::unique_ptr<CtranMapperRequest>>> putReqs;
  for (int p = 1; p < nRanks; p++) {
    const int peer = (rank + p) % nRanks;
    ...
    FB_COMMCHECK(mapper->iput(
        ...
        &req));
    putReqs.emplace_back(peer, std::unique_ptr<CtranMapperRequest>(req));
  }

  // Wait for each PUT to complete and signal the peer immediately
  for (auto& [peer, req] : putReqs) {
    ...
    FB_COMMCHECK(mapper->atomicSet(
        ...
        &signalReq));
    FB_COMMCHECK(mapper->waitRequest(&signalReq));
  }
