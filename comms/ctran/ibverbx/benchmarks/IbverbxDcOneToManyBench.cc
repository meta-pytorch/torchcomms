...
static void dcScalabilityCore(
  ...
  int K = multiNic ? static_cast<int>(g_dcCapableDevices.size()) - 1 : 1;
  if (K < 1) {
    counters["error"] = folly::UserMetric(1, folly::UserMetric::Type::METRIC);
    return;
  }
  ...
  if (!transitionDCIToRts(*dci, kPortNum, ibverbx::IBV_MTU_4096)) {
    counters["error"] = folly::UserMetric(1, folly::UserMetric::Type::METRIC);
    return;
  }
  ...
  for (int n = 0; n < K; ++n) {
    ...
    if (!nic.res.init(g_dcCapableDevices[n + 1])) {
      counters["error"] = folly::UserMetric(1, folly::UserMetric::Type::METRIC);
      return;
    }
    ...
    if (!transitionDCTToRtr(*nic.dct, kPortNum, ibverbx::IBV_MTU_4096)) {
      counters["error"] = folly::UserMetric(1, folly::UserMetric::Type::METRIC);
      return;
    }
    ...
    DcBusinessCard ahCard{};
    ahCard.mtu = 5;
    ahCard.dctNum = nic.dct->qp()->qp_num;
    ahCard.port = kPortNum;
    ahCard.subnetPrefix = nic.res.gid.global.subnet_prefix;
    ahCard.interfaceId = nic.res.gid.global.interface_id;
  ...
  if (sorted && K > 1) {
    ...
    for (int n = 0; n < K; ++n) {
      for (int r = 0; r < receiversPerNic[n]; ++r) {
        auto& nic = nics[n];
        DcBusinessCard& card = recvCards[idx];
        card.mtu = 5;
        card.dctNum = nic.dct->qp()->qp_num;
        card.port = kPortNum;
        card.subnetPrefix = nic.res.gid.global.subnet_prefix;
        card.interfaceId = nic.res.gid.global.interface_id;
        card.rank = idx;
        card.remoteAddr = reinterpret_cast<uint64_t>(nic.buf.data()) +
            static_cast<uint64_t>(r) * msgSize;
        card.rkey = nic.mr->mr()->rkey;
        ahIndex[idx] = n;
        idx++;
      }
    }
    ...
    for (int i = 0; i < numReceivers; ++i) {
      int n = i % K;
      auto& nic = nics[n];
      DcBusinessCard& card = recvCards[i];
      card.mtu = 5;
      card.dctNum = nic.dct->qp()->qp_num;
      card.port = kPortNum;
      card.subnetPrefix = nic.res.gid.global.subnet_prefix;
...
static void dcMultiDciCore(
  ...
  DcBusinessCard ahCard{};
  ahCard.mtu = 5;
  ahCard.dctNum = dcts[0]->qp()->qp_num;
  ahCard.port = kPortNum;
  ahCard.subnetPrefix = receiver.gid.global.subnet_prefix;
  ahCard.interfaceId = receiver.gid.global.interface_id;
  auto ahResult = createAddressHandle(*sender.pd, ahCard);
  if (!ahResult) {
    counters["error"] = folly::UserMetric(1, folly::UserMetric::Type::METRIC);
    return;
