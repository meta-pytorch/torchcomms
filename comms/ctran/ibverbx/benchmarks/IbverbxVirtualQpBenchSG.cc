...
struct ScatterGatherBenchmarkSetup {
  ...
  // Populate sgBufs/sgBufLens (used by IbvVirtualSendWr) for the SG payload.
  void populateSgBuffers(
      std::vector<void*>& sgBufs,
      std::vector<size_t>& sgBufLens) const {
    sgBufs.reserve(kNumScatterGatherBuffers);
    sgBufLens.reserve(kNumScatterGatherBuffers);
    for (uint32_t i = 0; i < kNumScatterGatherBuffers; ++i) {
      sgBufs.push_back(sendBuffers[i]);
      sgBufLens.push_back(perBufferSize);
    }
  }
...
static void BM_Ibverbx_VirtualQp_ScatterGather_RdmaWrite(
  ...
  try {
    ...
    // Build scatter-gather payload + per-buffer device keys for the typed
    // postSend(IbvVirtualSendWr&) entry point.
    auto perBufferKeys = setup.buildPerBufferKeys();
    int32_t senderDeviceId = setup.sender->qp.getQpsRef().at(0).getDeviceId();

    int wr_id = 0;
    IbvVirtualSendWr sendWr;
    sendWr.wrId = wr_id;
    sendWr.remoteAddr = reinterpret_cast<uint64_t>(setup.recvBuffer);
    sendWr.opcode = IBV_WR_RDMA_WRITE;
    sendWr.sendFlags = IBV_SEND_SIGNALED;
    setup.populateSgBuffers(sendWr.sgBufs, sendWr.sgBufLens);
    // Fallback deviceKeys: rkey is for the destination, lkey is unused for
    // SG fragments because perBufferDeviceKeys covers every source buffer.
    sendWr.deviceKeys[senderDeviceId] = MemoryRegionKeys{
        .lkey = setup.sendMrs[0]->mr()->lkey, .rkey = setup.recvMr->mr()->rkey};
    sendWr.perBufferDeviceKeys = std::move(perBufferKeys);

    // Benchmark the typed SG postSend operation
    for (auto _ : state) {
...
static void BM_Ibverbx_VirtualQp_ScatterGather_RdmaWriteWithImm(
  ...
  try {
    ...
    // Construct typed recv WR (zero-length notify recv on the receiver)
