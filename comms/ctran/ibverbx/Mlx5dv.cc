...
Status Mlx5dv::initObj(mlx5dv_obj* obj, uint64_t obj_type) {
...
Expected<mlx5dv_context> Mlx5dv::queryDevice(
    ibv_context* ctx,
    uint64_t compMask) {
  if (ctx == nullptr) {
  ...
  if (rc != 0) {
    return makeUnexpected(Error(rc, "mlx5dv_query_device failed"));
  }
