...
/**
 * This class defines common interface for all ReduceScatter Algorithms
 * subclasses are expected to provide actual implementation
 */
class AlgoReduceScatter {
 public:
  // NOTE: acc is not applied to reduce-scatter operation
  AlgoReduceScatter(
      const void* sendbuff,
...
class AlgoReduceScatterDdaIpc : public AlgoReduceScatter {
 public:
  using AlgoReduceScatter::AlgoReduceScatter;

  void reduceScatter() override;
 ...
 private:
  ...
  void launchKernel() {
    ...
    ASSIGN_FUNC_NRANKS(func, ddaReduceScatterIpc, nRanks_, false /* hasAcc */);
