...
// @generated SignedSource<<e83dd514476a800f0345b7ca4785b3bc>>
...
std::vector<std::string> NCCL_SOCKET_IFNAME;
std::vector<std::string> NCCL_SOCKET_IFNAME_DEFAULTCVARVALUE;
...
static void validateCvarEnv() {
  auto containsBeth = [](const std::vector<std::string>& list) {
    for (const auto& s : list) {
      if (s.find("beth") != std::string::npos)
        return true;
    }
    return false;
  };
  if (containsBeth(NCCL_SOCKET_IFNAME) !=
      (NCCL_CLIENT_SOCKET_IFNAME.find("beth") != std::string::npos)) {
    CVAR_ERROR(
        "CVAR incompatibility: NCCL_SOCKET_IFNAME ({}) vs NCCL_CLIENT_SOCKET_IFNAME ({}); mismatched beth interface usage",
        folly::join(",", NCCL_SOCKET_IFNAME),
        NCCL_CLIENT_SOCKET_IFNAME);
  }
  if (NCCL_SOCKET_IFNAME.size() > 1) {
    CVAR_ERROR(
        "NCCL_SOCKET_IFNAME should specify only one interface, but {} were specified",
        NCCL_SOCKET_IFNAME.size());
  }
}
...
static void readCvarEnv() {
  ...
  NCCL_SOCKET_IFNAME.clear();
  NCCL_SOCKET_IFNAME = env2strlist("NCCL_SOCKET_IFNAME", "");
  NCCL_SOCKET_IFNAME_DEFAULTCVARVALUE.clear();
  NCCL_SOCKET_IFNAME_DEFAULTCVARVALUE = env2strlist("NCCL_ENV_DO_NOT_SET", "");
