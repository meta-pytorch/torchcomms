...
// @generated SignedSource<<52498230a7fb14b1870d2b918763a2c6>>
...
static std::vector<std::string> tokenizer(std::string str) {
  ...
  auto tokens = ncclx::cvar::detail::split(
      ',', str, folly::SplitOptions{}.ignore_empty(true));
...
parseConfigTokens(const std::string& inputString) {
  ...
  auto tokenList = ncclx::cvar::detail::split(
      ';', inputString, folly::SplitOptions{}.ignore_empty(true));

  // Trim white space & check for duplicates
  std::unordered_set<std::string> uniqueTokens;
  for (auto& token : tokenList) {
    token = folly::trimWhitespace(token);
    auto keyValuePairs = ncclx::cvar::detail::split(
        ':', token, folly::SplitOptions{}.ignore_empty(true));
...
static void updateNcclConfig(const std::string& fname) {
  ...
  while (std::getline(in, line)) {
    ...
    auto tokens = ncclx::cvar::detail::split_noexact_2('=', line);
    if (tokens[0].empty()) {
