...
std::string_view getCollectiveStateStr(const CollTraceEvent& curEvent) {
...
WatchdogPluginConfig normalizeWatchdogConfig(WatchdogPluginConfig config) {
  if (!config.funcScheduleAsyncError) {
    config.funcScheduleAsyncError = [](std::function<void()> callback,
  ...
  if (useDefaultErrorTrigger) {
    if (*config.deferErrorTrigger) {
      ...
      config.funcTriggerOnError = [](const CollTraceEvent&) { std::abort(); };
    } else {
      config.funcTriggerOnError = [loggerName = std::string{config.loggerName}](
                                      const CollTraceEvent& event) {
...
CommsMaybeVoid WatchdogPlugin::dispatchAsyncError(
    const CollTraceEvent& curEvent) noexcept {
  try {
    ...
    config_.funcScheduleAsyncError(
