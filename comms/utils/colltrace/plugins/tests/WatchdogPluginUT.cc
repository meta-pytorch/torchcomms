...
class WatchdogPluginTest : public ::testing::Test {
 protected:
  ...
  [[noreturn]] static void throwSchedulingFailure(
      const std::function<void()>&,
      std::chrono::milliseconds) {
    throw std::runtime_error("expected scheduling failure");
  }
...
TEST_F(WatchdogPluginTest, AsyncErrorDelayDoesNotBlockProgress) {
  ...
  WatchdogPluginConfig config{
      ...
      .funcTriggerOnError =
          [triggeredCollId](const CollTraceEvent& event) {
...
TEST_F(WatchdogPluginTest, AsyncErrorDelayUsesStableEventSnapshot) {
  ...
  WatchdogPluginConfig config{
      .funcIfError = mockErrorCheckTrue,
      .funcTriggerOnError =
          [triggeredSnapshot](const CollTraceEvent& event) {
...
TEST_F(WatchdogPluginTest, DeferredErrorMarksBeforeReturning) {
  ...
  WatchdogPluginConfig config{
      .funcIfError = mockErrorCheckTrue,
      .funcTriggerOnError =
          [triggerEntered](const CollTraceEvent&) {
            triggerEntered->set_value();
          },
      .funcMarkError =
          [&markerCallCount](const CollTraceEvent&) { ++markerCallCount; },
...
TEST_F(WatchdogPluginTest, CustomDispatchFailureRunsSynchronousFallbackOnce) {
  int markerCallCount = 0;
  ...
  plugin = std::make_unique<WatchdogPlugin>(std::move(config));
  auto event = createCollTraceEvent(1);

  const auto firstResult = plugin->collEventProgressing(event);
  const auto secondResult = plugin->collEventProgressing(event);

  EXPECT_TRUE(firstResult.hasError());
  EXPECT_TRUE(secondResult.hasValue());
  EXPECT_EQ(errorCheckCallCount.load(), 1);
  EXPECT_EQ(markerCallCount, 1);
  EXPECT_EQ(scheduleCallCount, 1);
  EXPECT_EQ(triggerCallCount.load(), 1);
  EXPECT_EQ(lastTriggeredCollId.load(), event.collRecord->getCollId());
}

TEST_F(WatchdogPluginTest, DefaultDispatchFailureAbortsSynchronously) {
  WatchdogPluginConfig config{
      .funcIfError = mockErrorCheckTrue,
      .asyncErrorDelay = std::chrono::seconds{1},
      .deferErrorTrigger = true,
      .funcScheduleAsyncError = throwSchedulingFailure,
  };
  plugin = std::make_unique<WatchdogPlugin>(std::move(config));
  auto metadata = std::make_unique<::testing::NiceMock<MockCollMetadata>>();
  ON_CALL(*metadata, toDynamic())
      .WillByDefault(
          Return(static_cast<folly::dynamic>(folly::dynamic::object())));
  CollTraceEvent event;
  event.collRecord = std::make_shared<CollRecord>(1, std::move(metadata));

  EXPECT_DEATH(
      { (void)plugin->collEventProgressing(event); },
      "COMM FATAL: FatalError: Collective.*AsyncError");
}
