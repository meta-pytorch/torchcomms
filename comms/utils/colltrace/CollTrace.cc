...
void
  ...
  try {
    folly::getGlobalCPUExecutor()->add([sp] { delete sp; });
  } catch (...) {
...
std::optional<CapturedCollDescription> CollTrace::describeCapturedCollective(
  ...
  uint32_t largest = 0;
  {
    ...
    for (const auto& [_, state] : graphStateMap_) {
      // A destructed graph is gone as far as the contract is concerned; its
      // state only lingers until the next pollGraphEvents() removes it.
      // Answering from it hands a caller a binding it may cache for a graph
...
bool CollTrace::waitFlush(
  ...
  std::unique_lock lock(flushState_.mutex);
  // Same predicate as the unbounded overload, so cancellation still ends the
  // wait. A cancelled thread reads as completed here: it will never ack the
  // generation, and reporting that as a timeout would blame the deadline for a
  // tracer that is shutting down.
