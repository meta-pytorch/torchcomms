...
void CollTrace::pollGraphEvents(
  ...
  if (pollResult.entriesLost > 0) {
    ...
    for (auto& [_, event] : inFlightReplays_) {
      terminateEvent(*event, CollTraceTerminalReason::TrackingOverflow);
      graphReplayEvents_.push_back(std::move(event));
    }
    inFlightReplays_.clear();
    progressingGraphCollectives_.clear();
  }
