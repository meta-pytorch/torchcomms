...
// absent from the torchcomms OSS build tree. Gate it like fbclock below.
...
bool isPtpEnabled() {
  ...
  if (env == nullptr) {
    // Fall back to the compile-time literal of the yaml `default:` field
    // rather than a hardcoded constant, so the code default tracks the
    // canonical cvar default automatically. We cannot use the runtime
    // NCCL_USE_PTP / NCCL_USE_PTP_DEFAULTCVARVALUE symbols here because
    ...
    return NCCL_USE_PTP_DEFAULT_LITERAL;
  }
