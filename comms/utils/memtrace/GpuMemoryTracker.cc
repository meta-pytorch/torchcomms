...
void addAllocationUsage(GpuMemoryUsage& usage, uint64_t accountedBytes) {
...
void removeAllocationUsage(GpuMemoryUsage& usage, uint64_t accountedBytes) {
  usage.currentBytes -= accountedBytes;
...
bool GpuMemoryTracker::recordAllocation(
  ...
  try {
    ...
    addAllocationUsage(resourceUsage, accountedBytes);
    addAllocationUsage(componentUsage, accountedBytes);
    if (isPrimsComponent(component)) {
      addAllocationUsage(state->prims, accountedBytes);
    }
    addAllocationUsage(state->total, accountedBytes);
...
void GpuMemoryTracker::recordFree(
  ...
  try {
    ...
    auto& resourceUsage = state->resources[resourceIndex(resource)];
    auto& componentUsage = state->components[componentIndex(component)];
    removeAllocationUsage(resourceUsage, accountedBytes);
    removeAllocationUsage(componentUsage, accountedBytes);
    if (isPrimsComponent(component)) {
      removeAllocationUsage(state->prims, accountedBytes);
    }
    removeAllocationUsage(state->total, accountedBytes);
    --state->activeAllocations;
  } catch (...) {
  ...
  try {
    {
      ...
      if (gpuMemoryTracker) {
        gpuMemoryTracker->recordFree(
            allocation->resource, allocation->accountedBytes);
      }
    }
  } catch (...) {
    return;
