...
TEST(GpuMemoryTrackerTest, AttributesUsageAndTracksPeak) {
  ...
  recordGpuMemoryAllocation(
      GpuMemoryResourceType::kNvlP2pDataStaging, 0x1000, 128);
  recordGpuMemoryAllocation(
      GpuMemoryResourceType::kIbSendRecvPeerBulk, 0x2000, 256);
...
TEST(GpuMemoryTrackerTest, AccountsUnclassifiedMemoryOutsidePrims) {
  auto gpuMemoryTracker = std::make_shared<GpuMemoryTracker>(kContext);
  ScopedGpuMemoryContext scope{gpuMemoryTracker.get()};
  recordGpuMemoryAllocation(GpuMemoryResourceType::kUnclassified, 0x1050, 128);
...
TEST(
  ...
  ScopedGpuMemoryContext scope{gpuMemoryTracker.get()};
  recordGpuMemoryAllocation(
      GpuMemoryResourceType::kNvlP2pDataStaging, 0x1100, 128);
  recordGpuMemoryFree(0x1100);
  recordGpuMemoryAllocation(
      GpuMemoryResourceType::kIbSendRecvPeerBulk, 0x1200, 256);
  recordGpuMemoryAllocation(
      GpuMemoryResourceType::kMcclAllGatherOverlapCounters, 0x1300, 64);
...
TEST(GpuMemoryTrackerTest, RejectsDuplicateBackingWithoutDoubleCounting) {
  ...
  ScopedGpuMemoryContext scope{gpuMemoryTracker.get()};
  recordGpuMemoryAllocation(
      GpuMemoryResourceType::kCommonTransportDispatchTable, 0x3000, 64);
  recordGpuMemoryAllocation(
      GpuMemoryResourceType::kIbgdaAtomicReturnSink, 0x3000, 64);
...
TEST(GpuMemoryTrackerTest, FreeLooksUpBackingAndOnlyRecordsOnce) {
  ...
  ScopedGpuMemoryContext scope{gpuMemoryTracker.get()};
  recordGpuMemoryAllocation(
      GpuMemoryResourceType::kIbgdaPeerTransportArray, 0x4000, 128);
...
TEST(GpuMemoryTrackerTest, MissingFreeKeepsAllocationAccounted) {
  ...
  ScopedGpuMemoryContext scope{gpuMemoryTracker.get()};
  recordGpuMemoryAllocation(
      GpuMemoryResourceType::kCommonTransportDispatchTable, 0x4500, 128);
...
TEST(GpuMemoryTrackerTest, NullTrackerIsNoOp) {
  recordGpuMemoryAllocation(
      GpuMemoryResourceType::kCommonTransportDispatchTable, 0x5000, 64);
...
TEST(GpuMemoryTrackerTest, UnspecifiedResourceIsNotTracked) {
  ...
  recordGpuMemoryAllocation(GpuMemoryResourceType::kCount, 0x5050, 64);
...
TEST(GpuMemoryTrackerTest, ScopedTrackerRestoresNestedBinding) {
...

  {
    ...
    recordGpuMemoryAllocation(
        GpuMemoryResourceType::kCommonTransportDispatchTable, 0x5100, 64);
    {
      ScopedGpuMemoryContext innerScope{innerTracker.get()};
      recordGpuMemoryAllocation(
          GpuMemoryResourceType::kIbEagerControlBuffer, 0x5200, 128);
    }
    recordGpuMemoryAllocation(GpuMemoryResourceType::kNvlP2pSignal, 0x5300, 32);
  }
  recordGpuMemoryAllocation(
      GpuMemoryResourceType::kIbgdaAtomicReturnSink, 0x5400, 256);
...
TEST(GpuMemoryTrackerTest, NullScopeShadowsAndRestoresOuterBinding) {
  auto gpuMemoryTracker = std::make_shared<GpuMemoryTracker>(kContext);
  {
    ScopedGpuMemoryContext outerScope{gpuMemoryTracker.get()};
    {
      ...
      recordGpuMemoryAllocation(
          GpuMemoryResourceType::kCommonTransportDispatchTable, 0x5450, 64);
    }
    recordGpuMemoryAllocation(
        GpuMemoryResourceType::kCommonTransportDispatchTable, 0x5460, 32);
  }
...
TEST(GpuMemoryTrackerTest, FreeUsesRegisteredOwnerAcrossThreads) {
  ...
  auto unrelatedTracker = std::make_shared<GpuMemoryTracker>(kContext);
  {
    ...
    recordGpuMemoryAllocation(
        GpuMemoryResourceType::kIbgdaPeerTransportArray, 0x5500, 256);
  }
...
TEST(GpuMemoryTrackerTest, FailedFreeKeepsAllocationRegistered) {
  auto gpuMemoryTracker = std::make_shared<GpuMemoryTracker>(kContext);
  {
    ScopedGpuMemoryContext scope{gpuMemoryTracker.get()};
    recordGpuMemoryAllocation(
        GpuMemoryResourceType::kIbSlotCounter, 0x5550, 64);
  }
...
TEST(GpuMemoryTrackerTest, OverlappingFreeSuccessIsAccountedOnce) {
  auto gpuMemoryTracker = std::make_shared<GpuMemoryTracker>(kContext);
  {
    ScopedGpuMemoryContext scope{gpuMemoryTracker.get()};
    recordGpuMemoryAllocation(
        GpuMemoryResourceType::kIbSlotCounter, 0x5560, 64);
  }
...
TEST(GpuMemoryTrackerTest, FailedOverlappingFreeKeepsOtherOperationInProgress) {
  ...
  auto newTracker = std::make_shared<GpuMemoryTracker>(kContext);
  {
    ...
    recordGpuMemoryAllocation(
        GpuMemoryResourceType::kIbSlotCounter, 0x5570, 64);
  }
...

  {
    ...
    recordGpuMemoryAllocation(
        GpuMemoryResourceType::kIbgdaPeerTransportArray, 0x5570, 128);
  }
  finishGpuMemoryFree(
...
TEST(GpuMemoryTrackerTest, ReusedBackingKeepsNewAllocationRegistered) {
  ...
  auto newTracker = std::make_shared<GpuMemoryTracker>(kContext);
  {
    ScopedGpuMemoryContext scope{oldTracker.get()};
    recordGpuMemoryAllocation(
        GpuMemoryResourceType::kIbSlotCounter, 0x5580, 64);
  }
  ...
  ASSERT_NE(oldGeneration, 0);
  {
    ScopedGpuMemoryContext scope{newTracker.get()};
    recordGpuMemoryAllocation(
        GpuMemoryResourceType::kIbgdaPeerTransportArray, 0x5580, 128);
  }
  finishGpuMemoryFree(
...
TEST(GpuMemoryTrackerTest, ReusedBackingDoesNotInflatePeak) {
  auto gpuMemoryTracker = std::make_shared<GpuMemoryTracker>(kContext);
  ScopedGpuMemoryContext scope{gpuMemoryTracker.get()};
  recordGpuMemoryAllocation(GpuMemoryResourceType::kIbSlotCounter, 0x5590, 64);

  const auto oldGeneration = beginGpuMemoryFree(0x5590);
  ASSERT_NE(oldGeneration, 0);
  recordGpuMemoryAllocation(
      GpuMemoryResourceType::kIbgdaPeerTransportArray, 0x5590, 128);
  finishGpuMemoryFree(
...
TEST(GpuMemoryTrackerTest, CurrentTrackerIsThreadLocal) {
  auto gpuMemoryTracker = std::make_shared<GpuMemoryTracker>(kContext);
  {
    ...
    std::thread allocationThread{[] {
      recordGpuMemoryAllocation(
          GpuMemoryResourceType::kIbSlotCounter, 0x5600, 64);
    }};
...
TEST(GpuMemoryTrackerTest, RejectsDuplicateBackingAcrossTrackers) {
  ...
  recordGpuMemoryAllocation(
      GpuMemoryResourceType::kCommonTransportDispatchTable, 0x5700, 64);
  {
    ScopedGpuMemoryContext duplicateScope{duplicateTracker.get()};
    recordGpuMemoryAllocation(
        GpuMemoryResourceType::kIbgdaAtomicReturnSink, 0x5700, 128);
...
TEST(GpuMemoryTrackerTest, AllocationRegistryDoesNotOwnTracker) {
  std::weak_ptr<GpuMemoryTracker> weakTracker;
  {
    ...
    ScopedGpuMemoryContext scope{gpuMemoryTracker.get()};
    recordGpuMemoryAllocation(
        GpuMemoryResourceType::kCommonTransportDispatchTable, 0x5800, 64);
  }
  ...
  recordGpuMemoryAllocation(GpuMemoryResourceType::kIbSlotCounter, 0x5800, 128);
