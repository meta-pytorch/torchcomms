...
#include "absl/log/check.h" // @manual=fbsource//third-party/abseil-cpp:log__check
...
// nccl_cvars.h is generated by the internal ncclx-cvars Buck target and
// is absent from both the torchcomms OSS build tree and the
// hrdw_ring_buffer conda feedstock build. Gate just the canonical
// default literal on it — PrecisionClock itself lives under
// `comms/utils/` and self-degrades (fbclock when `__has_include` finds
// it, std::chrono::system_clock otherwise), so it's always includable
// and callable.
...
inline constexpr bool
    NCCL_COLLTRACE_FATAL_ON_CALIBRATION_FAILURE_DEFAULT_LITERAL = true;
#endif
...
std::chrono::system_clock::time_point GlobaltimerCalibration::toWallClock(
  ...
  if (a.host_time.time_since_epoch().count() == 0) {
    ...
    // an obvious sentinel for downstream readers; throttle to at most one
    // log per 5 s so a hot timestamping path doesn't flood the log.
...
GlobaltimerCalibration::GlobaltimerCalibration() {
  ...
  *mapped_ptr_ = 0;
  ...
  // wait for a periodic refresh to populate the anchor. Read inline via
  // EnvUtils::getBoolEnv with the cvar's yaml default literal as the
  // fallback — we don't go through the runtime cvar machinery here
  // because GpuClockCalibration initializes lazily on first call to
  // get(), and many of those call paths fire before ncclCvarInit() runs.
  if (!refresh()) {
    if (::meta::comms::getBoolEnv(
            "NCCL_COLLTRACE_FATAL_ON_CALIBRATION_FAILURE",
            NCCL_COLLTRACE_FATAL_ON_CALIBRATION_FAILURE_DEFAULT_LITERAL)) {
      LOG(FATAL)
  ...
  CHECK_NE(a.host_time.time_since_epoch().count(), 0)
...
// (mapped_ptr_); anchor_ is mutex-guarded so readers via toWallClock() remain
// safe across a refresh.
bool GlobaltimerCalibration::refresh() {
  ...
  // Inline RAII so we don't drag in absl/cleanup just for the
  // single-callsite scope guard. Copy/move deleted explicitly to
  // satisfy `cppcoreguidelines-special-member-functions` (Rule of
  // Five) — this guard owns a reference and a release-on-destruct
  // contract that doesn't survive any of the implicit copies.
  struct InProgressGuard {
    std::atomic_flag& flag;
    // Explicit ctor because the user-declared copy/move below make this
    // class non-aggregate, so brace-init can't fall back to the implicit
    // member-wise initialization.
    explicit InProgressGuard(std::atomic_flag& f) noexcept : flag(f) {}
    ~InProgressGuard() {
      flag.clear(std::memory_order_release);
    }
    InProgressGuard(const InProgressGuard&) = delete;
    InProgressGuard(InProgressGuard&&) = delete;
    InProgressGuard& operator=(const InProgressGuard&) = delete;
    InProgressGuard& operator=(InProgressGuard&&) = delete;
  } guard{refresh_in_progress_};
  ...
  // Read host_time *after* the kernel completes so the host timestamp is
  // no earlier than the device reading. `precisionNow()` is fbclock-
  // aligned in the BUCK build and self-degrades to
  // std::chrono::system_clock::now() in the feedstock build (PrecisionClock.cc
  // gates the fbclock branch on `__has_include`).
  {
    std::lock_guard<std::mutex> lock(anchor_mutex_);
    anchor_ = Anchor{
