...
struct FlightRecorder {
  ...
  FlightRecorder() {
    max_entries_ = env_to_value("TORCHCOMM_FR_BUFFER_SIZE", 2000);
    capture_cpp_stack_ = env_to_value("TORCHCOMM_FR_CPP_STACK", false);
...
class FlightRecorderHook {
 ...
 private:
  ...
  // Map from op_id to reset_epoch to pass the correct reset_epoch when retiring
  std::unordered_map<size_t, size_t> op_id_to_reset_epoch_;
