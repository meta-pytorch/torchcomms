...
class TorchCommMPI : public TorchCommBackend,
                     public std::enable_shared_from_this<TorchCommMPI> {
 public:
  static constexpr std::string_view kBackendName = "mpi";

  TorchCommMPI();
  ~TorchCommMPI() override;

  TorchCommMPI(const TorchCommMPI&) = delete;
  TorchCommMPI(TorchCommMPI&&) = delete;
  TorchCommMPI& operator=(const TorchCommMPI&) = delete;
  TorchCommMPI& operator=(TorchCommMPI&&) = delete;
 ...
 private:
  ...
  static MPI_Datatype toMPIDatatype(at::ScalarType type);
  static MPI_Op toMPIReduceOp(const ReduceOp& op);
  void mpiCheck(int result, const char* msg);
