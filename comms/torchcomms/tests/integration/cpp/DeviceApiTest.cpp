...
void DeviceApiTest::testDevicePut(int count, at::ScalarType dtype) {
  ...
  // Get device window with signals (one signal per rank for synchronization)
  int signal_count = num_ranks_;
  auto* dev_win = win->get_device_window(signal_count, -1, 1);
  ASSERT_NE(dev_win, nullptr) << "Device window should not be null";

  // TODO: The current NCCL GIN implementation requires that both source
  ...
  auto* src_win =
      dynamic_cast<torch::comms::TorchCommWindowNCCLX*>(src_base_win.get());
  ASSERT_NE(src_win, nullptr) << "Source window should be TorchCommWindowNCCLX";

  // Create RegisteredBuffer pointing to the source window's nccl_orig_win_
  torch::comms::device::RegisteredBuffer src_buf;
  src_buf.base_ptr = src_tensor.data_ptr();
  src_buf.size = count * src_tensor.element_size();
  src_buf.backend_window = src_win->get_nccl_orig_window();
  ASSERT_NE(src_buf.backend_window, nullptr)
  ...
  // Launch put kernel on put_stream
  {
    ...
    torch::comms::device::test::launchDevicePutKernelWithOffsets(
        dev_win,
        src_buf,
        src_offset,
        dst_offset,
  ...
  // Wait for signal 0 to be incremented (indicating put completed to us)
  {
    ...
    torch::comms::device::test::launchDeviceWaitSignalKernel(
  ...
  {
    c10::cuda::CUDAStreamGuard guard(put_stream);
    torch::comms::device::test::launchDeviceResetSignalKernel(
