...
class NativeDeviceMeshTest(unittest.TestCase):
    ...
    def test_scatter_backward(self) -> None:
        """Test that scatter backward is gather."""
        comm = torchcomms.new_comm(
            self.backend, self.device, name="scatter_backward_test"
        )
        ...
        try:
            ...
            # Output tensor with requires_grad
            output = torch.zeros(
                10, 5, device=self.device, dtype=torch.float32, requires_grad=True
            )
            ...
            # Verify forward result
            expected = torch.full((10, 5), float(rank + 1), device=self.device)
            self.assertTrue(torch.allclose(result, expected))
            ...
            # Scatter backward is gather - gradients should be gathered to rank 0
            if rank == 0:
                for i, t in enumerate(scatter_list):
                    self.assertIsNotNone(t.grad)
                    expected_grad = torch.full_like(t.grad, float(i + 1))
                    self.assertTrue(torch.allclose(t.grad, expected_grad))
            else:
    ...
    def test_gather_backward(self) -> None:
        """Test that gather backward is scatter.

        Gather: all ranks send to dst rank
        Backward: dst rank scatters gradients back to all ranks
        """
        comm = torchcomms.new_comm(
            self.backend, self.device, name="gather_backward_test"
        )
        ...
        try:
            ...
            # Gather list on ALL ranks to maintain autograd connection
            # Only dst rank will receive actual data, but non-dst ranks need
            # tensors in the list for gradient flow
            gather_list = [
                torch.zeros(10, 5, device=self.device, dtype=torch.float32)
                for _ in range(world_size)
            ]
            ...
            # All ranks call backward - they all have world_size tensors in result
            # Use different gradient values to verify scatter
            grads = [
                torch.full((10, 5), float(i + 1), device=self.device)
                for i in range(world_size)
            ]
            torch.autograd.backward(result, grads)

            self._sync_device()

            # Verify gradient on input tensor
            # Each rank should receive grad[rank] from dst rank via scatter
            self.assertIsNotNone(t.grad)
            expected_grad = torch.full((10, 5), float(rank + 1), device=self.device)
            self.assertTrue(
                torch.allclose(t.grad, expected_grad),
                f"Rank {rank}: expected grad {rank + 1}, got {t.grad[0, 0].item()}",
            )
        finally:
            comm.finalize()
