...
class NodeRankLayoutTest(unittest.TestCase):
    """Tests for NodeRankLayout, including subcommunicators created via split().

    Hardcoded for a 1-node x 4-rank layout so the expected node-rank layout is
    known exactly without an independent oracle.
    """
    ...
    def setUp(self):
        ...
        # The node-rank layout reports two launcher-assigned id lists for the on-node
        ...
        if self.num_ranks != EXPECTED_NUM_RANKS:
            ...
            self.tearDown()
            self.skipTest(
                f"NodeRankLayoutTest requires exactly {EXPECTED_NUM_RANKS} ranks on a "
                f"single node, got {self.num_ranks}"
            )

        # Skip on backends that do not implement get_node_rank_layout(). The
        # base backend raises, surfaced as a RuntimeError on the Python side.
        try:
            self.torchcomm.get_node_rank_layout()
        except RuntimeError as e:
            # tearDown won't run on a setUp skip, so clean up explicitly to
            # finalize the comm created above.
            self.tearDown()
            self.skipTest(f"backend does not implement get_node_rank_layout(): {e}")
    ...
    def _assert_layout(
        ...
        """Assert comm.get_node_rank_layout() matches the expected layout.
        ...
        layout = comm.get_node_rank_layout()
        self.assertEqual(
            layout.num_nodes, num_nodes, f"num_nodes mismatch for {description}"
        )
        self.assertEqual(
            layout.local_node_ranks,
            local_node_ranks,
            f"local_node_ranks mismatch for {description}",
        )
        self.assertEqual(
            len(layout.local_node_global_rank_ids),
            local_node_ranks,
            f"local_node_global_rank_ids length mismatch for {description}",
        )
        self.assertEqual(
            len(layout.local_node_local_rank_ids),
            local_node_ranks,
        ...
        if expected_rank_ids is not None:
            if self.global_rank_ids_predictable:
                self.assertEqual(
                    sorted(layout.local_node_global_rank_ids),
                    expected_rank_ids,
                    f"local_node_global_rank_ids mismatch for {description}",
                )
            if self.local_rank_ids_predictable:
                self.assertEqual(
                    sorted(layout.local_node_local_rank_ids),
                    expected_rank_ids,
                    f"local_node_local_rank_ids mismatch for {description}",
                )
        # All ranks share one node, so the layout is always uniform and single.
        self.assertTrue(layout.uniform, f"expected uniform layout for {description}")
        self.assertTrue(
            layout.is_single_node(), f"expected single-node for {description}"
        )
    ...
    def test_contiguous_split_layout(self):
        ...
        child = self.torchcomm.split(ranks, name="contiguous_layout_comm")
    ...
    def test_non_contiguous_split_layout(self):
        ...
        child = self.torchcomm.split(ranks, name="noncontig_layout_comm")
    ...
    def test_single_rank_split_layout(self):
        """A communicator split down to a single rank: 1 node, 1 rank."""
        child = self.torchcomm.split([self.rank], name="single_rank_layout_comm")
    ...
    def test_multi_level_split_layout(self):
        ...
        first_comm = self.torchcomm.split(first_ranks, name="first_level_layout_comm")
        ...
        second_comm = first_comm.split(second_ranks, name="second_level_layout_comm")
    ...
    def test_chained_intra_node_split(self):
        """Build an intra-node comm from the node-rank layout, then split it
           again using the remap-by-position rule.
        ...
        layout = self.torchcomm.get_node_rank_layout()
        node_world_ranks = layout.local_node_global_rank_ids
    ...
    def test_compute_node_rank_layout_disabled(self):
        """With compute_node_rank_layout off, get_node_rank_layout() raises.
        ...
        get_node_rank_layout() — the call raises rather than returning
        stale/default data. std::logic_error surfaces as RuntimeError on the
        Python side.
        ...
        wrapper = TorchCommTestWrapper(hints={"compute_node_rank_layout": "false"})
        try:
            with self.assertRaises(RuntimeError):
                wrapper.get_torchcomm().get_node_rank_layout()
        finally:
            wrapper = None

    def test_compute_node_rank_layout_enabled_explicitly(self):
        """Setting the hint to true matches the default-on behavior."""
        wrapper = TorchCommTestWrapper(hints={"compute_node_rank_layout": "true"})
        try:
            self._assert_layout(
                ...
                local_node_ranks=EXPECTED_NUM_RANKS,
                expected_rank_ids=list(range(EXPECTED_NUM_RANKS)),
                description="world communicator with compute_node_rank_layout=true",
            )
        finally:
            wrapper = None
