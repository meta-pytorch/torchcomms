...
class LazyP2PTest(unittest.TestCase):
    ...
    def setUp(self):
        # This test always drives the 'nccl-lazy' backend regardless of the
        # harness TEST_BACKEND, so restrict it to the nccl configuration to
        # avoid forcing nccl-lazy under other (e.g. non-CUDA) backends.
        if os.getenv("TEST_BACKEND") != "nccl":
            self.skipTest("LazyP2PTest only runs for the nccl backend")
