...
TEST_F(AllToAllvDynamicTest, DispatchCombineExample) {
  ...
  std::vector<size_t> input_chunk_sizes(num_ranks_, chunk_size);

  // Send all chunks: input_chunk_indices = [0, 1, 2, 3, 4, 5, 6, 7]
  std::vector<size_t> input_chunk_indices(num_ranks_);
  ...
  // rank 7
  std::vector<size_t> input_chunk_count_per_rank(num_ranks_, 1);

  size_t maxSendcount = chunk_size * num_ranks_; // Total send buffer size
  ...
  // Create input tensor with rank-specific pattern
  auto options = at::TensorOptions().dtype(at::kFloat).device(device_type_);
  at::Tensor input_tensor =
      at::zeros({static_cast<int64_t>(maxSendcount)}, options);
  ...
  for (size_t chunk_id = 0; chunk_id < input_chunk_sizes.size(); chunk_id++) {
    size_t length = input_chunk_sizes[chunk_id];
  ...
  for (int sender_rank = 0; sender_rank < num_ranks_; sender_rank++) {
    ...
    size_t expected_offset = rank_ * chunk_size;
  ...
  at::Tensor allgather_input_chunk_sizes_gpu =
      at::zeros({num_ranks_ * num_ranks_}, size_t_options);
  ...
  const size_t num_chunks = num_ranks_ * num_ranks_; // 64 chunks total
  ...
  at::Tensor combine_input_chunk_sizes_gpu =
      at::full({static_cast<int64_t>(num_chunks)}, chunk_size, size_t_options);
  ...
  at::Tensor combine_input_chunk_indices_gpu =
      at::arange(0, num_ranks_, size_t_options) * num_ranks_ + rank_;

  at::Tensor combine_input_chunk_count_per_rank_gpu =
      at::ones({num_ranks_}, size_t_options);

  // Create output tensor: 8k elements (1024 * 8)
  const size_t combine_output_size = chunk_size * num_ranks_; // 8192
  at::Tensor combine_output_tensor =
      at::zeros({static_cast<int64_t>(combine_output_size)}, options);
