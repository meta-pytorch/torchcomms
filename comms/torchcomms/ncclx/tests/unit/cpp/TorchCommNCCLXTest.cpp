...
TEST_F(TorchCommNCCLXTest, GlobalRegisterAddressFailure) {
  // Test: Verify global_register_address throws exception on NCCL failure
  ...
  // Execute: call global_register_address and expect exception
  EXPECT_THROW(
      TorchCommNCCLX::global_register_address(
          TorchCommNCCLX::AddressWithLen{test_addr, test_len},
          nccl_mock_.get()),
...
TEST_F(TorchCommNCCLXTest, GlobalDeregisterAddressFailure) {
  // Test: Verify global_deregister_address throws exception on NCCL failure
  ...
  // Execute: call global_deregister_address and expect exception
  EXPECT_THROW(
      TorchCommNCCLX::global_deregister_address(
          TorchCommNCCLX::AddressWithLen{test_addr, test_len},
          nccl_mock_.get()),
...
TEST_F(
  ...
  // Execute: simulate memory allocation - should throw due to registration
  // failure
