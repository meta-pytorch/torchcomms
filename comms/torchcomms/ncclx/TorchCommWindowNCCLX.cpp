...
TorchCommWindowNCCLX<Backend>::~TorchCommWindowNCCLX() noexcept {
  ...
  if (device_window_initialized_) {
    Backend::destroy_device_window(nccl_comm_, nccl_api_, device_window_);
    device_window_initialized_ = false;
  }
...
TorchCommWindowNCCLX<Backend>::register_local_buffer(const at::Tensor& tensor) {
  ...
  CHECK_EQ(
      nccl_api_->commWindowRegister(
          buf.base_ptr, buf.size, local_comm_, &local_win, NCCL_WIN_DEVICE_API),
      ncclSuccess)
...
TorchCommWindowNCCLX<Backend>::get_device_window(
  ...
  device_window_initialized_ = true;
...
void TorchCommWindowNCCLX<Backend>::checkRequestSizeAndThrow(
  ...
  if (input_size > win_size_) {
    throw std::runtime_error(
        fmt::format(
            "[TorchCommWindowNCCLX]: Requested size ({} bytes) exceeds window size ({} bytes)",
            input_size,
            win_size_));
  }
}
