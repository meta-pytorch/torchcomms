...
//
// Pass-by-Value Design:
//   NCCLGinBackend::create_device_window() returns TorchCommDeviceWindow BY
//   VALUE, following NCCL's pattern. There is NO cudaMalloc or cudaMemcpy
//   involved - the struct is passed by value to kernels and CUDA copies it
//   automatically.
...
TEST_F(NCCLGinBackendTest, CreateDeviceWindowSuccessReturnsValidStruct) {
  ...
  //   2. Returns TorchCommDeviceWindow struct BY VALUE (no cudaMalloc!)
  ...
  // create_device_window() returns TorchCommDeviceWindow BY VALUE - no
  // pointers!
...
TEST_F(NCCLGinBackendTest, DestroyDeviceWindowSuccess) {
  ...
  //
  // Note: No cudaFree expected - we don't cudaMalloc the ncclDevComm struct!
  ...
  // Expect only devCommDestroy - no cudaFree!
  EXPECT_CALL(*nccl_mock_, devCommDestroy(fake_nccl_comm_, _))
      .WillOnce(Return(ncclSuccess));

  NCCLGinBackend::destroy_device_window(
      fake_nccl_comm_, nccl_mock_.get(), device_window);

  // Verify struct is reset
  EXPECT_EQ(device_window.window_, nullptr);
  EXPECT_EQ(device_window.base_, nullptr);
  EXPECT_EQ(device_window.size_, 0);
  EXPECT_EQ(device_window.rank_, 0);
  EXPECT_EQ(device_window.num_ranks_, 0);
}
...
TEST_F(NCCLGinBackendTest, DestroyDeviceWindowWithNullCommIsNoOp) {
  ...
  EXPECT_NO_THROW(
      NCCLGinBackend::destroy_device_window(
          nullptr, nccl_mock_.get(), device_window));
}
...
TEST_F(NCCLGinBackendTest, DestroyDeviceWindowWithNullApiIsNoOp) {
  ...
  EXPECT_NO_THROW(
      NCCLGinBackend::destroy_device_window(
          fake_nccl_comm_, nullptr, device_window));
}
...
TEST_F(NCCLGinBackendTest, FullLifecycleCreateAndDestroy) {
  ...
  // Destroy - only devCommDestroy, no cudaFree!
