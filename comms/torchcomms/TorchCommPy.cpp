...
Persistent collectives allow separating the setup cost from execution cost.
The init call is expensive (allocates resources) but the exec call is cheap.
...
This is the fast path - can be called many times with the same handle.
...
Must be called when done with the persistent operation to release resources.
