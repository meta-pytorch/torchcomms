...
# Only use the conda installation when the static archive is present
# (e.g., installed by build_ncclx.sh). If conda only provides libgflags.so,
# skip find_package (which would find conda's shared cmake config) and build
# statically from source via FetchContent.
if(EXISTS "${CONDA_INCLUDE}/gflags/gflags.h" AND EXISTS "${CONDA_LIB}/libgflags.a")
...
elseif(EXISTS "${CONDA_INCLUDE}/gflags/gflags.h")
    # Conda has headers but only libgflags.so — go straight to FetchContent
    # to avoid find_package discovering the conda shared-lib cmake config.
    message(STATUS "Conda gflags has no libgflags.a — building gflags statically via FetchContent")
...
# Skipped for the conda-static path which creates gflags::gflags directly.
