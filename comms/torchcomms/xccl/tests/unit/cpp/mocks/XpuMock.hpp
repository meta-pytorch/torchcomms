...
class XpuMock : public XpuApi {
 public:
  ...
  MOCK_METHOD(
      ...
      (xpuStream_t & stream, unsigned int flags, int priority),
      (override));
  MOCK_METHOD(
      xpu_result_t,
      streamDestroy,
  ...
  MOCK_METHOD(
      xpu_result_t,
      streamWaitEvent,
      (const xpuStream_t& stream, xpuEvent_t& event, unsigned int flags),
      (override));
  ...
  MOCK_METHOD(
      xpu_result_t,
      streamIsCapturing,
      (const xpuStream_t& stream, xpuStreamCaptureStatus* pCaptureStatus),
      (override));
  MOCK_METHOD(
      xpu_result_t,
  ...
  MOCK_METHOD(
      xpu_result_t,
      memcpyAsync,
      (void* dst, const void* src, size_t count, const xpuStream_t& stream),
      (override));
  ...
  MOCK_METHOD(
      ...
      (xpuEvent_t & event, unsigned int flags),
      (override));
  MOCK_METHOD(
      xpu_result_t,
      eventDestroy,
      (const xpuEvent_t& event),
      (override));
  MOCK_METHOD(
      xpu_result_t,
      eventRecord,
      (xpuEvent_t & event, const xpuStream_t& stream),
      (override));
  ...
  void setupDefaultBehaviors() {
    ...
    ON_CALL(*this, getDeviceProperties(_, _))
        .WillByDefault(Return(XPU_SUCCESS));
    ...
    ON_CALL(*this, getCurrentXPUStream(_)).WillByDefault([](int device_index) {
      return c10::xpu::XPUStream::unpack3(
          0, device_index, c10::DeviceType::XPU);
    });
  }
};
