...
class XcclMock : public XcclApi {
 public:
  ...
  MOCK_METHOD(
      onecclResult_t,
      commGetAsyncError,
      (onecclComm_t comm, onecclResult_t* asyncError),
      (override));
  MOCK_METHOD(
      onecclResult_t,
      commSplit,
  ...
  MOCK_METHOD(
      onecclResult_t,
      commRegister,
      (onecclComm_t comm, void* buffer, size_t size, void** handle),
      (override));
  MOCK_METHOD(
      onecclResult_t,
      commDeregister,
      (onecclComm_t comm, void* handle),
      (override));
  ...
  MOCK_METHOD(
      onecclResult_t,
      commUserRank,
      (const onecclComm_t comm, int* userRank),
      (override));
  MOCK_METHOD(
      onecclResult_t,
      commCount,
      (const onecclComm_t comm, int* count),
      (override));
  ...
  void setupDefaultBehaviors() {
    ...
    ON_CALL(*this, commInitRankConfig(_, _, _, _, _))
        .WillByDefault(DoAll(
            SetArgPointee<0>(reinterpret_cast<onecclComm_t>(0x1234)),
    ...
    ON_CALL(*this, broadcast(_, _, _, _, _, _, _))
        .WillByDefault(Return(onecclSuccess));
    ON_CALL(*this, bcast(_, _, _, _, _, _))
        .WillByDefault(Return(onecclSuccess));
    ...
    ON_CALL(*this, reduceScatter(_, _, _, _, _, _, _))
        .WillByDefault(Return(onecclSuccess));
    ON_CALL(*this, allToAll(_, _, _, _, _, _))
        .WillByDefault(Return(onecclSuccess));
