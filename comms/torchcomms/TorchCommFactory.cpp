...
std::shared_ptr<TorchCommBackend> TorchCommFactory::create_backend(
  ...
  if (impl) {
    // Check if init_dynamic_regime is requested but backend doesn't support it
    ...
    if (options.init_dynamic_regime && !impl->supportsDynamicRegime()) {
      throw std::runtime_error(
          "[TorchComm]: init_dynamic_regime=true is not supported by backend: " +
          backend +
          ". Only backends that implement supportsDynamicRegime() can use "
          "dynamic regime for fault tolerance.");
    }
