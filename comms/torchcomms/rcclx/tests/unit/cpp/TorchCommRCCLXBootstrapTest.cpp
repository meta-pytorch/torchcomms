...
class TorchCommRCCLXBootstrapTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    TorchCommRCCLXBootstrap::getRCCLXStoreKey(); // This increments counter
    initial_counter_ = TorchCommRCCLXBootstrap::getRCCLXStoreKeyCounter();
  }
  ...
  std::unique_ptr<TorchCommRCCLXBootstrap> createBootstrap(
      c10::intrusive_ptr<c10d::Store> store = nullptr) {
    if (!store) {
      store = store_;
    }
    return std::make_unique<TorchCommRCCLXBootstrap>(
        store, device_, rcclx_mock_, hip_mock_, kTimeout);
  }
...
TEST_F(TorchCommRCCLXBootstrapTest, StaticMethodsStoreKeyGeneration) {
  // Test that store key generation works correctly with counter
  std::string prefix = TorchCommRCCLXBootstrap::getRCCLXStoreKeyPrefix();
  EXPECT_EQ(prefix, "rcclx_storekey_");

  int counter_before = TorchCommRCCLXBootstrap::getRCCLXStoreKeyCounter();
  std::string key1 = TorchCommRCCLXBootstrap::getRCCLXStoreKey();
  int counter_after = TorchCommRCCLXBootstrap::getRCCLXStoreKeyCounter();
  ...
  std::string key2 = TorchCommRCCLXBootstrap::getRCCLXStoreKey();
  int final_counter = TorchCommRCCLXBootstrap::getRCCLXStoreKeyCounter();
...
TEST_F(TorchCommRCCLXBootstrapTest, GetRankAndSizeFromEnvironment) {
  ...
  EXPECT_CALL(*rcclx_mock_, commInitRankConfig(_, 4, _, 1, _))
      .WillOnce(DoAll(
...
TEST_F(TorchCommRCCLXBootstrapTest, GetRankAndSizeEnvironmentVariablesMissing) {
  // Don't set environment variables
  EXPECT_THROW(createBootstrap(), std::runtime_error);
}

TEST_F(TorchCommRCCLXBootstrapTest, ExchangeUniqueIdRank0) {
  ...
  EXPECT_CALL(*rcclx_mock_, getUniqueId(_))
      .WillOnce(DoAll(SetArgPointee<0>(expected_id), Return(ncclSuccess)));

  EXPECT_CALL(*rcclx_mock_, commInitRankConfig(_, 2, _, 0, _))
      .WillOnce(DoAll(
          SetArgPointee<0>(reinterpret_cast<ncclComm_t>(0x3000)),
          Return(ncclSuccess)));
...
TEST_F(TorchCommRCCLXBootstrapTest, ExchangeUniqueIdNonRank0) {
  ...
  EXPECT_CALL(*rcclx_mock_, commInitRankConfig(_, 2, _, 1, _))
      .WillOnce(DoAll(
          SetArgPointee<0>(reinterpret_cast<ncclComm_t>(0x3000)),
          Return(ncclSuccess)));
...
TEST_F(TorchCommRCCLXBootstrapTest, CreateNcclCommGetUniqueIdFailure) {
  setupRankAndSize(0, 2);
  ...
  EXPECT_CALL(*rcclx_mock_, getErrorString(ncclInvalidArgument))
...
TEST_F(TorchCommRCCLXBootstrapTest, CreateNcclCommInitRankConfigFailure) {
  setupRankAndSize(0, 2);
...
TEST_F(TorchCommRCCLXBootstrapTest, ExchangeUniqueIdInvalidStoreData) {
  setupRankAndSize(1, 2);
...
TEST_F(TorchCommRCCLXBootstrapTest, CreateNcclCommWithNullStore) {
  setupRankAndSize(0, 2);
  ...
  auto bootstrap = std::make_unique<TorchCommRCCLXBootstrap>(
      nullptr, device_, rcclx_mock_, hip_mock_, kTimeout);
  ...
  EXPECT_CALL(*rcclx_mock_, allReduce(_, _, _, _, _, _, _))
...
TEST_F(TorchCommRCCLXBootstrapTest, CleanupTCPStoreBarrierFailure) {
  setupRankAndSize(0, 2);
  setenv("MASTER_ADDR", "localhost", 1);
  setenv("MASTER_PORT", "0", 1);
