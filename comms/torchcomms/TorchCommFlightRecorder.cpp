...
template <typename EventType>
void FlightRecorder<EventType>::record(
    size_t pg_id,
...
template <typename EventType>
void FlightRecorder<EventType>::recordWithResetEnabled(
    size_t pg_id,
    const std::tuple<std::string, std::string>& pg_name,
    size_t collective_seq_id,
    size_t p2p_seq_id,
...
template <typename EventType>
std::vector<typename FlightRecorder<EventType>::Entry>
FlightRecorder<EventType>::dump_entries() {
...
// returns std::nullopt.
std::optional<typename FlightRecorder<EventType>::Entry>
FlightRecorder<EventType>::getEntry(
...
template <typename EventType>
std::optional<typename FlightRecorder<EventType>::Entry>
FlightRecorder<EventType>::getEntry(std::optional<size_t> id) {
...
FlightRecorder<EventType>::getPgConfig() {
...
FlightRecorder<EventType>::getPgConfigJson() {
...
template <typename EventType>
const c10::Dict<c10::IValue, c10::IValue>
FlightRecorder<EventType>::getPgStatus() {
...
template <typename EventType>
const std::map<std::string, std::map<std::string, std::string>>
FlightRecorder<EventType>::getPgStatusJson() {
  std::map<std::string, std::map<std::string, std::string>> result;
  for (const auto& [pg_id, status] : all_pg_status_) {
...
// exists.
    ...
    std::string fileNamePrefix = env_to_value(
        "TORCHCOMM_FR_DUMP_TEMP_FILE", defaultLocation.string().c_str());
...
FlightRecorderHook::FlightRecorderHook(size_t max_entries) {
  recorder_ = FlightRecorder<c10::Event>::get();
  recorder_->max_entries_ = max_entries;
  recorder_->enabled_ = max_entries > 0;
}
...
void FlightRecorderHook::onPreHook(
  ...
  bool isP2P = isP2POp(args.name);

  size_t collective_seq_id = 0;
  size_t p2p_seq_id = 0;
  if (isP2P) {
    p2p_seq_id = p2p_seq_id_++;
    collective_seq_id_ = 0;
  } else {
    collective_seq_id = collective_seq_id_++;
    p2p_seq_id_ = 0;
  }
  ...
  // Store the current reset_epoch so we can pass it to retire_id later
  {
    std::lock_guard<std::mutex> guard(recorder_->mutex_);
    op_id_to_reset_epoch_[args.op_id] = recorder_->reset_epoch_;
  }
...
void FlightRecorderHook::onPostHook(const TorchComm::PostHookArgs& args) {
  ...
  // Look up the reset_epoch for this op_id and pass it to retire_id
  size_t reset_epoch = 0;
  auto it = op_id_to_reset_epoch_.find(args.op_id);
  if (it != op_id_to_reset_epoch_.end()) {
    reset_epoch = it->second;
    op_id_to_reset_epoch_.erase(it);
  }
  recorder_->retire_id(args.op_id, reset_epoch, false);
}
...
size_t FlightRecorderHook::size() const {
  std::lock_guard<std::mutex> guard(recorder_->mutex_);
  return recorder_->entries_.size();
}
